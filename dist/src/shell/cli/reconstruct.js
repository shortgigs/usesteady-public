...
import { renderReconstructHelpText, renderReconstructJson, renderReconstructText, } from "../reconstruct-render.js";
export function resolveReconstructStoreDir(env = process.env) {
    return resolveStoreDir(env);
}
export function isEphemeralStoreDir(storeDir) {
    return storeDir === DEFAULT_STORE_DIR;
}
export function parseReconstructArgs(argv) {
    let help = false;
    let ucpRootId;
    for (let i = 0; i < argv.length; i += 1) {
        ...
        if (tok.startsWith("-"))
            return "usage-error";
        if (ucpRootId !== undefined)
            return "usage-error";
        ucpRootId = tok;
    }
    if (help)
        return { mode: "help" };
    if (ucpRootId === undefined || ucpRootId.trim().length === 0)
        return "usage-error";
    return { mode: "reconstruct", ucpRootId: ucpRootId.trim() };
}
function primaryTaskInputFromPayload(p) {
    const fromInputs = p.taskInputs?.find((input) => typeof input === "string" && input.trim().length > 0);
    return fromInputs?.trim() ?? "";
}
function resolveUcpRootForWorkflowPayload(storeDir, p) {
...
function workflowPayloadReferencesUcpRoot(storeDir, p, ucpRootId) {
    if (p.sessionRefs.some((ref) => ref.intentId === ucpRootId))
        return true;
    const derived = resolveUcpRootForWorkflowPayload(storeDir, p);
    return derived === ucpRootId;
}
function findWorkflowRunIdForIntentRoot(storeDir, intentRootId) {
    let envelopes;
    try {
    ...
    for (const env of envelopes) {
        if (env.type !== "ucp.workflow_run.v1")
            continue;
        const p = env.payload;
        ...
        if (!Array.isArray(p.sessionRefs))
            continue;
        if (!workflowPayloadReferencesUcpRoot(storeDir, p, intentRootId))
            continue;
...
function buildWorkflowSummary(storeDir, workflowRunId) {
...
export function buildReconstructReport(storeDir, ucpRootId) {
    const gaps = [];
    ...
    const chainEnvelopes = getChain(storeDir, ucpRootId);
    const entries = chainEnvelopes.map((env) => ({
        id: env.id,
        type: env.type,
        ts: env.ts,
    }));
    ...
    if (intentEnv !== undefined) {
        const payload = intentEnv.payload;
 ...
 */
export function getBlockingGaps(gaps) {
 ...
 */
export function resolveLatestWorkflowUcpRoot(storeDir, options) {
    ...
    for (const h of histories) {
        ...
        const match = envelopes.find((env) => env.type === "ucp.workflow_run.v1"
            && env.payload.workflowRunId === h.workflowRunId);
        if (match === undefined || match.type !== "ucp.workflow_run.v1")
            continue;
 ...
 */
export function resolveLatestCompletedUcpRoot(storeDir, options) {
...
export function compareChainVerification(recomputed, claimed) {
...
export function explainIntegrityMismatch(comparison) {
...
function assertReconstructableBase(report, reasons) {
...
export function assertFailurePathReconstructReport(report, scenario) {
    const reasons = [];
...
export function auditMultiStepGovernance(storeDir, workflowRunId, primaryChainEnvelopeCount) {
    const audit = getWorkflowAuditRecord(storeDir, workflowRunId);
    if (audit === null)
        return null;
    const tasks = audit.tasks.map((t) => {
        const intentId = t.session?.intentId ?? null;
        let chainEnvelopeCount = 0;
        let approvalConfirmed = false;
        if (intentId !== null) {
            const chain = buildSessionChain(storeDir, intentId);
            chainEnvelopeCount = getChain(storeDir, intentId).length;
            const handoff = chain.cursorHandoff?.payload;
            approvalConfirmed = handoff?.confirmedByHuman === true;
        }
        else if (t.outcome === "accepted") {
            // FS fast-path (e.g. rename): approval occurred at task_ready; no intent chain.
    ...
    return {
        ...
        confirmed_approval_handoffs: tasks.filter((t) => t.approval_confirmed).length,
        primary_chain_envelope_count: primaryChainEnvelopeCount,
        tasks,
    };
}
export function assertMultiStepCodeChangeCertReport(report, storeDir, options) {
    ...
    if (report.chain.envelope_count < minChainCount) {
        reasons.push(`chain_count ${report.chain.envelope_count} < ${minChainCount} — governance evidence chain too short`);
    }
    if (report.chain_verification === null) {
        ...
        reasons.push(`integrity entry_ids length ${report.chain_verification.entry_ids.length} < ${minChainCount}`);
    }
    if (report.workflow === null) {
    ...
    const governance = auditMultiStepGovernance(storeDir, workflowRunId, report.chain.envelope_count);
    if (governance === null) {
        reasons.push("governance audit record missing");
        return { ok: false, reasons };
    }
    if (governance.confirmed_approval_handoffs < minConfirmedApprovals) {
        reasons.push(`approval chain incomplete: ${governance.confirmed_approval_handoffs} confirmed approvals ` +
            `< ${minConfirmedApprovals} required (one per task)`);
    }
    if (governance.intent_backed_tasks < 2) {
        reasons.push(`expected at least 2 intent-backed replace tasks, got ${governance.intent_backed_tasks}`);
    }
    if (reasons.length > 0) {
        return { ok: false, reasons };
    }
...
export function assertReconstructionCertReport(report) {
    const reasons = [];
    assertReconstructableBase(report, reasons);
...
export function runReconstruct(args, storeDir = resolveReconstructStoreDir(), format = "text") {
    if (args.mode === "help") {
...
export function listKnownUcpRoots(storeDir) {
    const roots = new Set();
    for (const h of getWorkflowHistories(storeDir)) {
        const audit = getWorkflowAuditRecord(storeDir, h.workflowRunId);
        if (audit === null)
            continue;
        for (const task of audit.tasks) {
            const intentId = task.session?.intentId;
