...
export function truthfulInputDisplay(taskSpec) {
    ...
    const esc = (v) => escapeControlForInline(v.replace(/\\/g, "\\\\").replace(/"/g, '\\"'));
 ...
 */
function operationKindLabel(op) {
    switch (op) {
 ...
 */
function createDirEffectsText(target) {
...
export function operationPreviewLines(taskSpec) {
    ...
    if (op === "rename") {
        if (target === undefined || taskSpec.newPath === undefined)
            return [];
        return [
            "  Preview:",
            `    Operation: ${operationKindLabel(op)}`,
 ...
 * unstructured tasks.
 */
export function operationReviewSummaryLines(taskSpec) {
    if (taskSpec.structuredReplace !== undefined) {
        const sr = taskSpec.structuredReplace;
        return [
            `       Target:  ${sr.filePath}`,
    ...
    const target = taskSpec.targetFiles && taskSpec.targetFiles.length > 0
        ? taskSpec.targetFiles[0]
        : undefined;
    if (op === "append_file" || op === "prepend_file" || op === "write_file") {
        if (target === undefined)
            return [];
        const content = taskSpec.content ?? "";
        return [
 ...
 */
function sessionDetailLines(run) {
    ...
    const lines = [];
    if (d.changeSummary)
        lines.push(`  Understood: ${d.changeSummary}`);
    if (d.targetFiles && d.targetFiles.length)
 ...
 */
function intakeGuidanceLines(run) {
...
export function renderWorkflowFrame(run, opts) {
    const { phase, display } = run;
    switch (phase) {
        // ── Pre-flight review (WF-R4) ─────────────────────────────────────────
        case "reviewing": {
            const total = run.spec.tasks.length;
            const runtime = (t) => t.runtime ?? run.spec.defaultRuntime ?? "cursor";
            ...
            const taskBlocks = run.spec.tasks.map((t, i) => {
            ...
            for (let i = 0; i < taskBlocks.length; i += 1) {
                if (i > 0)
                    taskLines.push("    ─────");
                for (const ln of taskBlocks[i])
                    taskLines.push(ln);
            }
            const breakGlassLines = run.mode === "break_glass"
                ? [
            ...
            return {
                ...
                prompt: { kind: "confirm", question: "Start workflow? (y/n)" },
            };
        }
        ...
        case "task_ready": {
            ...
            const prevLine = prevTask && prevTask.outcome !== "pending"
            ...
            return {
                ...
                prompt: { kind: "confirm", question: "Approve this task? (y/n)" },
            };
        }
        case "task_conflict": {
            ...
            return {
                ...
                prompt: { kind: "confirm", question: "Accept warning and approve task? (y/n)" },
            };
        }
        case "task_scope": {
        ...
        case "task_advisory": {
            ...
            const positions = run.currentSession?.runtime === "claude"
            ...
            for (let i = 0; i < positions.length; i++) {
                ...
                if (!rec)
                    continue;
                positionLines.push(`  Position ${i + 1} [${rec.position.kind}]  id: ${rec.modelPositionId.slice(0, 16)}…`);
            ...
            return {
                ...
                prompt: { kind: "confirm", question: "Proceed despite the model position? (y/n)" },
            };
        }
        case "task_failed": {
            ...
            return {
                ...
                prompt: { kind: "choose", question: "Action:", choices },
            };
        }
