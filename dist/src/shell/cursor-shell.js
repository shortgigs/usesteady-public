 ...
 */
import { approve, reject, acceptConflict, answerScope, deliver, } from "../product/cursor-product-session.js";
// ─── Confirm advance (y/n) ────────────────────────────────────────────────────
/**
 ...
 */
export async function advanceCursorOnConfirm(state, yes, plugin, storeDir) {
    if (!yes)
        return reject(state);
 ...
 */
export async function advanceCursorOnChoice(state, choiceIdx, plugin, storeDir) {
    const candidates = state.scopeQuestion?.candidates ?? [];
    const file = candidates[choiceIdx - 1];
    if (!file)
        return state;
    // answerScope → "approved", then deliver immediately.
    const narrowed = answerScope(state, file);
    return deliver(narrowed, plugin, storeDir);
}
//# sourceMappingURL=cursor-shell.js.map