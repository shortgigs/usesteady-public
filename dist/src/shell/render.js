...
function phaseLabel(phase, runtime) {
...
function promptForPhase(phase, candidates) {
    switch (phase) {
        case "prepared":
 ...
 */
export function renderCursorFrame(state) {
 ...
 * No session state is modified.
 */
export function renderClaudeFrame(state) {
    const { phase, display } = state;
    const lines = [
        "",
