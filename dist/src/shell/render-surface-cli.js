 ...
 * behavior-identical.
 */
export function renderSurfaceCli(surface) {
    switch (surface.type) {
...
function renderSafety(s) {
    ...
    const footer = s.detectorId === "evasion_or_rule_bypass"
        ? [
...
// ─── system_will ───────────────────────────────────────────────────────────────
function renderWill(s) {
    const lines = ["", "  SYSTEM WILL", `  -> ${s.headline}`];
    const opLine = willOperationLine(s.operation);
    if (opLine)
        lines.push(`     ${opLine}`);
    if (s.preview)
        lines.push(...previewLines(s.preview));
    lines.push("");
    return lines;
}
function willOperationLine(op) {
...
function previewLines(diff) {
    const out = [`     ${diff.filePath}`];
...
// ─── work_plan ─────────────────────────────────────────────────────────────────
function renderWorkPlan(s) {
    const wp = s.workPlan;
    const lines = ["", "  WORK PLAN", `  Goal: ${wp.goal}`];
...
//# sourceMappingURL=render-surface-cli.js.map