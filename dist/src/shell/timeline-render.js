...
function renderTable(view) {
    const lines = [];
    lines.push("");
    lines.push("  #  Task                 State          Started     Ended       Outcome");
...
export function renderTimelineIncompleteText(runId) {
    const lines = [];
    lines.push("");
    ...
    lines.push("  Timeline cannot show a step-by-step narrative without a terminal workflow record.");
    lines.push("  Per-task delivery envelopes may exist in the store, but v1.1 does not use them");
...
export function renderTimelineText(view) {
    if (view.kind === "empty") {
    ...
    const lines = [];
    lines.push("");
    lines.push(`  Workflow: ${view.workflowName}  (run id: ${view.workflowRunId})`);
...
export function renderTimelineJson(view) {
    if (view.kind === "empty") {
...
export function renderTimelineHelpText() {
    return ("\n" +
...
//# sourceMappingURL=timeline-render.js.map