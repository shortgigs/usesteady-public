...
function renderList(summaries) {
    ...
    for (let i = 0; i < summaries.length; i++) {
        ...
        lines.push(`  ${i + 1}.  [${shortId(s.workflowRunId)}]  ${row}`);
        choices.push(row);
    }
    lines.push("");
    return {
...
// ─── Detail view ──────────────────────────────────────────────────────────────
function renderSessionChainLines(session) {
    const lines = [];
    ...
    if (session.cursorHandoff) {
        ...
        if (session.cursorReceipt) {
            ...
            const kind = p.refusalKind ?? "refused";
            const sub = p.scopeQuestionKind;
...
function renderDetail(record) {
    ...
    lines.push(`  run id     ${shortId(record.workflowRunId)}`);
 ...
 */
export function renderHistoryFrame(view) {
    switch (view.kind) {
