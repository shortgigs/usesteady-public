...
import { isolateIntentFromConfirmed, isolateIntentFromRatifiedGoal, } from "./isolated-intent.js";
import { parseAnalysisAsk } from "./business-intent-routing.js";
import { FORBIDDEN_TEMPLATE_ACTIONS, getTemplate, PHASE_TEMPLATES, resolveOutcome, resolveTarget, } from "./work-plan-templates.js";
const TYPE_PATTERNS = [
...
function findSpanInGoalText(goalText, pattern) {
...
function classifyDeliverableFromGoal(goalText, goalSourceRef) {
...
const PREAMBLE_PATTERN = /^(i\s+(?:want|need|would like|'d like)\s+to\s+|help\s+me\s+(?:to\s+)?|we\s+(?:need|want)\s+to\s+|please\s+)/i;
const PURPOSE_CLAUSE_PATTERN = /\s+(?:to|in order to|so(?:\s+that)?|so\s+we\s+can)\s+\S.*$/i;
/**
 ...
 */
const REQUESTER_FRAME_PATTERN = /^(?:(?:i|we)\s+(?:need[s]?|want[s]?|would like|are looking for)\s+|(?:our|the)\s+(?:\w+\s+){0,4}(?:need[s]?|want[s]?)\s+)/i;
/**
 ...
 */
const PROPER_NOUN_SUBJECT_FRAME_PATTERN = /^([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)?)\s+needs?\s+/;
/**
 ...
 */
const BIC1_FRAME_PATTERN = /^it\s+would\s+(?:help|be\s+great)\s+if\s+/i;
const BIC1_CAPABILITY_PATTERN = /^(.+?)\s+(could|got)\s+(.+)$/i;
...
function passesMinimumResidual(residual) {
 ...
 */
export function normalizeSubject(rawInput) {
    ...
    if (!hasBusinessFrame) {
        ...
        if (first) {
            ...
            const isVerbCandidate = /^[a-zA-Z-]+$/.test(alpha) && !NON_VERB_OPENERS.has(first.lower);
            if (isVerbCandidate) {
...
function extractSubject(rawInput) {
    return normalizeSubject(rawInput).subject;
}
function computePlanHash(deliverableType, goal, tasks, nextAction) {
    const payload = [
        deliverableType,
        goal,
        tasks.map(t => [
...
function isGrounded(goalText, classification) {
 ...
 */
function buildAnalysisWorkPlan(ask, sourceSpan) {
    ...
    const tasks = template.phases.map((phase, idx) => ({
        id: phase.id,
        action: phase.action,
 ...
 */
export function generateWorkPlanFromConfirmed(confirmed) {
    ...
    if (analysisAsk !== null) {
        const analysisSpan = hasRatifiedGoal
            ? {
                ref: "rawInput",
    ...
    const classification = classifyDeliverableFromGoal(isolated.goalText, isolated.goalSourceRef);
    ...
    const planHash = computePlanHash(classification.type, goal, tasks, nextAction);
    return {
        goal,
        sourceContractRef: "rawInput",
        tasks,
...
export function assertPlanDeterminism(confirmed, runs = 3) {
...
export function assertTemplatesForbiddenClean() {
...
//# sourceMappingURL=work-plan-generator.js.map