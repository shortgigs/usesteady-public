 ...
 */
export function validateTokenAgainstSpec(token, spec, workspaceRoot) {
    // 1. Spec hash match.
    ...
    if (currentHash !== token.workflow_spec_hash) {
        return {
            ...
            message: `Resume token was issued for spec hash ${token.workflow_spec_hash.slice(0, 12)}…, `
    ...
    if (spec.tasks.length !== token.total_task_count) {
        return {
            ...
            message: `Resume token expects ${token.total_task_count} tasks; current spec has ${spec.tasks.length}.`,
        };
    }
    // 3. Bounds check on completed_task_count.
    if (!Number.isInteger(token.completed_task_count)
        || token.completed_task_count < 0
        || token.completed_task_count > token.total_task_count) {
        return {
            ok: false,
            code: "corrupt_resume_token",
            message: `Resume token has out-of-range completed_task_count `
    ...
    if (token.completed_task_count === token.total_task_count) {
        return {
            ...
            message: `Resume token reports all ${token.total_task_count} tasks already complete. `
                + `Nothing to resume.`,
        };
    }
    ...
    if (token.completed_task_summaries.length !== token.completed_task_count) {
        return {
            ok: false,
            code: "corrupt_resume_token",
            message: `Resume token's completed_task_summaries length (${token.completed_task_summaries.length}) `
    ...
    for (let i = 0; i < token.completed_task_count; i++) {
        ...
        if (specOpType !== summary.op_type) {
            return {
                ...
                message: `Resume token's task ${i} op_type "${summary.op_type}" does not match `
    ...
    if (tokenRoot !== currentRoot) {
        return {
            ...
            message: `Resume token was issued in workspace ${tokenRoot}; `
 ...
 */
export function validateTokenForResume(tokenPath, spec, workspaceRoot) {
    ...
    switch (r.kind) {
        ...
        case "unsupported_format":
            return {
                ...
                message: `Resume token format "${r.seen}" is not supported by this CLI version. `
                    + `Expected "usesteady.resume-token.v1".`,
            };
    }
}
//# sourceMappingURL=resume-token-validator.js.map