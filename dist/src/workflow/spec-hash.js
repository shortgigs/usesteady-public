 ...
 */
function canonicalWorkPlanTaskPayload(task) {
    return {
        id: task.id,
        action: task.action,
        target: task.target,
...
function canonicalWorkPlanPayload(plan) {
    return {
        goal: plan.goal,
...
function canonicalTaskPayload(task) {
    return {
        input: task.input,
 ...
 */
function canonicalSpecPayload(spec) {
    const payload = {
        name: spec.name,
 ...
 */
export function computeWorkflowSpecHash(spec) {
 ...
 */
export function verifyWorkflowSpecHash(spec, expected) {
    const actual = computeWorkflowSpecHash(spec);
    if (actual === expected)
        return { ok: true };
    return { ok: false, expected, actual };
}
//# sourceMappingURL=spec-hash.js.map