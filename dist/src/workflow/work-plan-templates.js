...
function tpl(deliverableType, phases, goal, terminal) {
    return { deliverableType, phases, goalPattern: goal, terminalOutcomePattern: terminal };
}
export const PHASE_TEMPLATES = {
    characterization: tpl("characterization", [
    ...
    ], s => `Complete characterization of ${s}`, s => `Published characterization report for ${s}`),
    research_initiative: tpl("research_initiative", [
    ...
    ], s => `Establish research initiative for ${s}`, s => `Research plan artifact defined for ${s}`),
    program_migration: tpl("program_migration", [
    ...
    ], s => `Plan migration program for ${s}`, s => `Cutover plan drafted for ${s}`),
    capability_setup: tpl("capability_setup", [
    ...
    ], s => `Define capability setup for ${s}`, s => `Integration points identified for ${s}`),
    refactor_scope: tpl("refactor_scope", [
    ...
    ], s => `Plan refactor scope for ${s}`, s => `Refactor scope reviewed for ${s}`),
    structure_scaffold: tpl("structure_scaffold", [
    ...
    ], s => `Scaffold project structure for ${s}`, s => `Scaffold layout verified for ${s}`),
    ...
    ], (s, goalVerb = "Create") => `${goalVerb} ${s}`, s => `Work plan defined for ${s}`),
    ...
    analysis_findings: tpl("analysis_findings", [
    ...
    ], s => `Produce an explanation of ${s}`, s => `Findings documented: an evidence-backed answer to ${s}`),
};
...
export function resolveTarget(targetKey, subject) {
...
export function resolveOutcome(outcomeKey) {
    return OUTCOME_LABELS[outcomeKey] ?? outcomeKey;
}
export function getTemplate(type) {
    return PHASE_TEMPLATES[type];
}
//# sourceMappingURL=work-plan-templates.js.map