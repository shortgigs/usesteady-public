 ...
 */
export function evaluateOCDForHandoff(artifact, policy) {
 ...
 */
export function evaluatePathsAgainstOCDPolicy(allowedFiles, policy) {
    const rulesFired = [];
    ...
    for (const filePath of allowedFiles) {
        ...
        const matchesAnySafeGlob = (candidate) => policy.writeSafeGlobs.some((safeGlob) => matchesGlob(candidate, safeGlob));
        const originalSafe = matchesAnySafeGlob(normalizedPath);
        const canonicalSafe = canonicalPath === null ? false : matchesAnySafeGlob(canonicalPath);
        ...
        if (!isSafe) {
            ...
            conflictsDetected.push(`File '${filePath}' is outside the safe write zone (write_safe_globs).`);
        }
    }
    ...
    if (allowedFiles.length > 0 &&
        ...
        conflictsDetected.push(`Too many files in scope: ${allowedFiles.length} (threshold: ${policy.maxFilesApprovalThreshold}).`);
    }
 ...
 */
export function validateHProvidedPath(filePath, workspaceRoot) {
    ...
    if (canonicalFilePath !== filePath) {
        ...
        if (!resolvedCanonical.startsWith(resolvedWorkspace + path.sep)
 ...
 */
export function applyOCDClearance(artifact, ocdClearance) {
 ...
 */
export function acceptOCDConflict(artifact) {
    return {
        ...artifact,
        ocdClearance: {
 ...
 */
function canonicalizePathForCheck(normalizedPath) {
    try {
...
//# sourceMappingURL=ocd-evaluator.js.map