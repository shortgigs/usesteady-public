...
import { CAPABILITY_REGISTRY_VERSION, capabilityIdForObservationCode, } from "./proposal-bindings.js";
...
const CREATED_FROM = "diagnostic_record";
const CATALOG_IDS = new Set(EXECUTION_CAPABILITY_CATALOG_V1.map((c) => c.capability_id));
...
function buildProposal(diagnostic, obs, capability_id) {
...
function computeOverall(proposals) {
 ...
 */
export function generateRemediationProposalsFromDiagnostic(diagnostic, catalog = EXECUTION_CAPABILITY_CATALOG_V1) {
    const catalogIds = new Set(catalog.map((c) => c.capability_id));
    const seenCodes = new Set();
    const proposals = [];
    for (const obs of sortObservations([...diagnostic.observations])) {
        if (obs.code === "effective_state_fail_closed")
            continue;
        if (seenCodes.has(obs.code))
            continue;
