...
function looksLikePath(s) {
    ...
    if (/\.[a-zA-Z0-9]{1,10}$/.test(s))
        return true; // has extension
    if (s.includes("/") || s.includes("\\"))
        return true; // path separator
    if (/^[a-zA-Z0-9_\-.]+$/.test(s))
        return true; // clean identifier
    return false;
}
// ─── Helpers ──────────────────────────────────────────────────────────────────
 ...
 */
const POLITE_PREFIX = /^(?:please\s+|can\s+you\s+|could\s+you\s+|i\s+need\s+(?:to\s+)?|i\s+want\s+to\s+)/i;
...
const CREATE_DIR_REVERSED = [
    /^(?:create|make|add)\s+(?:a\s+)?(?<p>.+?)\s+(?:folder|directory|dir)$/i,
];
function matchCreateDir(raw) {
...
const CREATE_FILE_REVERSED = [
    /^(?:create|make|add)\s+(?:a\s+)?(?<p>.+?)\s+file$/i,
];
function matchCreateFile(raw) {
 ...
 */
export function intentToChange(intent) {
    switch (intent.kind) {
        case "create_dir":
            return {
