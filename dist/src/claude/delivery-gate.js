...
import { createClaudeHandoffEnvelope, createClaudeReceiptEnvelope, createClaudeRefusedEnvelope, createModelAdvisoryEnvelope, createModelEvidenceBasisEnvelope, createRetiredPositionReassertionEnvelope, } from "../ucp/envelope.js";
import { deriveDeliveryEvidenceBasis } from "./evidence-basis.js";
import { classifyIncomingAdvisory, compareGrounds, groundsIdFromBasis, } from "./objection-retirement.js";
    ...
    async deliver(artifact, opts) {
        ...
        if (response.kind === "advisory") {
            ...
            if (validPositions.length === 0) {
                ...
                const envelopeByPositionId = new Map();
                for (const position of validPositions) {
                    ...
                    if (previousEnvelope !== undefined &&
                        ...
                        const refusedEnvelope = createClaudeRefusedEnvelope({
                            ...
                            errorMessage: "One advisory content identity resolved to conflicting envelope identities.",
                            messageOrigin: "adapter",
                        }, { parentId: handoffEnvelope.id, rootId: artifact.intentId });
                        ...
                        return {
                            ...
                            errorCode: "conflicting_advisory_identity",
                            message: "One advisory content identity resolved to conflicting envelope identities.",
                        };
                    }
                    ...
                    records.push(record);
                    const classification = classifyIncomingAdvisory({ modelPositionId, groundsId: incomingGroundsId }, retiredRefs);
                    if (classification === "retired_reassertion") {
                        const match = retiredRefs.find((r) => r.modelPositionId === modelPositionId)
                            ...
                            ?? retiredRefs[0];
                        const groundsRelation = match === undefined
                            ? "unknown"
 ...
 */
function validateAdvisoryPositions(positions, artifact) {
