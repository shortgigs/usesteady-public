...
import { canonicalModelAdvisoryEvent, } from "./types.js";
 ...
 * output.
 */
export function deriveDeliveryEvidenceBasis(request) {
    const artifact = request.artifact;
    const sources = [
        {
            source: "governed_task_spec",
            availability: "available_and_corresponded",
            detail: "Task category, summary, and structured edit specification were supplied " +
        ...
        artifact.allowedFiles.length > 0
            ? {
                ...
                availability: "available_and_corresponded",
                detail: `${artifact.allowedFiles.length} allowed file name(s) were listed in the ` +
            ...
            },
        {
            ...
            availability: "not_provided",
            detail: "No file contents were included in the model input. The V1 delivery " +
                "contract builds the model input from the governed task specification only.",
        },
        {
            source: "tool_retrieval",
            availability: "not_provided",
            detail: "No in-process tool execution or read-file round-trip occurred; " +
                "delivery is a single request/response.",
        },
    ];
    if (request.priorAdvisories && request.priorAdvisories.length > 0) {
        sources.push({
            ...
            availability: "available_and_corresponded",
            detail: `${request.priorAdvisories.length} previously persisted advisory position(s) ` +
    ...
    if (request.retiredAdvisories && request.retiredAdvisories.length > 0) {
        sources.push({
            ...
            availability: "available_and_corresponded",
            detail: `${request.retiredAdvisories.length} retired model position(s) were ` +
 ...
 */
export function classifyAdvisoryEvidenceBasis(storeDir, advisoryEnvelopeId) {
    ...
    for (const id of childIds) {
        ...
        if (env.type === "ucp.model_evidence_basis.v1") {
            return { status: "established", envelope: env };
        }
    }
...
export function validateMappedRetirementEnvelopePair(input) {
    ...
    let e;
    try {
        o = input.readEnvelopeById(input.storeDir, input.relation.model_position.position_hash);
    }
    catch {
        return { ok: false, code: "mapped_store_unavailable" };
    }
    if (o === null)
        return { ok: false, code: "mapped_o_reference_absent" };
    if (o.id !== input.relation.model_position.position_hash) {
        return { ok: false, code: "mapped_o_reference_corrupt" };
    }
    try {
        e = input.readEnvelopeById(input.storeDir, input.relation.resolving_evidence.evidence_basis_id);
    }
    catch {
        return { ok: false, code: "mapped_store_unavailable" };
    }
    if (e === null)
        return { ok: false, code: "mapped_e_reference_absent" };
    ...
    const op = o.payload;
    if (typeof op.artifactId !== "string" ||
...
//# sourceMappingURL=evidence-basis.js.map