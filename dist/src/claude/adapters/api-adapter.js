...
export class ClaudeApiAdapter {
    ...
    async receive(request) {
        ...
        const userMessage = buildUserMessage(artifact.taskSpec, request.priorAdvisories, request.retiredAdvisories);
...
function buildSystemPrompt(artifact) {
    ...
    lines.push(`OCD CLEARANCE: ${artifact.ocdClearance === "clear"
        ...
        : "conflict_accepted — human has reviewed and accepted the conflict."}`, "", "═══ SCOPE CLARIFICATION PROTOCOL ═══", "", "If the target file is ambiguous or not in allowedFiles, you MUST call", "request_scope_clarification with real file path candidates.", "Do NOT guess file paths. Do NOT widen scope beyond allowedFiles.", "", "═══ MODEL ADVISORY PROTOCOL ═══", "", "If you CAN technically perform the task but have a material concern about it", "(a risk, a recommendation against, significant uncertainty, or a better", "alternative), you MUST call model_advisory_position INSTEAD of executing.", "An advisory pauses the action for human review; it does not decide anything.", "Call it at most once per response (multiple positions in one call are not", "supported — choose the single most important position).", "If you have no material concern, proceed with the task normally.", "", "═══ TASK ═══");
    lines.push("", `Execution domain: ${artifact.executionDomain}`);
    return lines.join("\n");
}
// ─── User message ─────────────────────────────────────────────────────────────
function buildUserMessage(taskSpec, priorAdvisories, retiredAdvisories) {
    const lines = [
    ...
    if (priorAdvisories !== undefined && priorAdvisories.length > 0) {
        lines.push("");
        lines.push("Note: you previously recorded an advisory position on this exact action,", "and the authorized human reviewer explicitly chose to proceed despite it:");
    ...
    if (retiredAdvisories !== undefined && retiredAdvisories.length > 0) {
        lines.push("");
        lines.push("Note: the following model position(s) were retired by an authorized human", "on named resolving evidence. They are not standing. Do not reassert them", "on the same recorded grounds:");
        for (const retired of retiredAdvisories) {
            const ev = retired.resolvingEvidenceIds.join(", ");
            lines.push(`  - [${retired.position.kind}] id=${retired.modelPositionId} evidence=${ev}`, `    ${retired.position.explanation}`);
        }
    }
    lines.push("");
 ...
 */
const MODEL_ADVISORY_TOOL = {
    name: "model_advisory_position",
    description: "Call this when you can technically perform the task but have a material position " +
...
function resolveTools(allowedTools) {
    const tools = [SCOPE_CLARIFICATION_TOOL, MODEL_ADVISORY_TOOL];
...
// ─── Response mapping ─────────────────────────────────────────────────────────
function mapApiResponse(response, artifact, model) {
 ...
 */
function extractAdvisoryPositions(response, artifact, model) {
    const kinds = new Set(MODEL_ADVISORY_KINDS);
    const positions = [];
    let sawAdvisoryCall = false;
    for (const block of response.content) {
        if (block.type !== "tool_use")
            continue;
        if (block.name !== "model_advisory_position")
            continue;
