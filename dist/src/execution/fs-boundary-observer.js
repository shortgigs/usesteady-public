 ...
 */
export function makeFsBoundaryObserver(inner, sink) {
    return {
        async executeFsOp(op) {
            ...
            try {
                ...
                console.error("[fs-boundary-observer] sink append failed (record absent → correspondence NOT ESTABLISHED at verification):", err instanceof Error ? err.message : String(err));
            }
 ...
 */
export function makeJsonlBoundarySink(filePath) {
    let prepared = false;
    return {
        append(record) {
            if (!prepared) {
...
//# sourceMappingURL=fs-boundary-observer.js.map