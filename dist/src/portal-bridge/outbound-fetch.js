...
const setDefaultCACertificates = tls["setDefaultCACertificates"];
...
export function enableOutboundSystemCaIfNeeded() {
    if (outboundCaConfigured)
        return;
    const explicit = process.env[OUTBOUND_SYSTEM_CA_ENV]?.trim();
    if (explicit === "0")
        return;
    const shouldEnable = explicit === "1" || (explicit === undefined && process.platform === "win32");
    if (!shouldEnable)
        return;
...
export function resolveOutboundFetch(override) {
    if (override)
        return override;
