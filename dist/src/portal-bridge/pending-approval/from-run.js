...
function changeTypeFor(action) {
    return action === "other" ? null : action;
}
function affectedResourcesFor(task, action) {
    const changeType = changeTypeFor(action);
    if (changeType === null)
        return undefined; // run_command / unresolved: no file resource
    ...
    const path = task.structuredReplace?.filePath ??
        task.newPath ??
 ...
 */
export function pendingApprovalInputFromRun(run, requestedAt) {
    ...
    const advisoryRecords = run.phase === "task_advisory" && session?.runtime === "claude"
        ? (session.state.advisoryPositions ?? [])
        : [];
    const advisoryCycle = run.phase === "task_advisory" && session?.runtime === "claude"
        ? (session.state.advisoryCycle ?? 1)
        : 0;
    const advisoryPairs = new Set();
...
//# sourceMappingURL=from-run.js.map