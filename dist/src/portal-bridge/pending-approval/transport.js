...
import { validatePendingApprovalPayload } from "./payload.js";
import { parseApprovalDecisionResponse, } from "./decision.js";
const DEFAULT_TIMEOUT_MS = 10_000;
function trimBase(base) {
    return base.replace(/\/+$/, "");
}
function emitUrl(base) {
    return `${trimBase(base)}/api/v1/pending-approvals`;
}
function decisionsUrl(base, runId) {
 ...
 */
export async function emitPendingApproval(payload, opts) {
 ...
 */
export async function pollApprovalDecision(runId, opts) {
    const fetchFn = resolveFetch(opts);
    if (!fetchFn)
        return { ok: false, reason: "fetch_unavailable" };
    if (!opts.url || !opts.url.trim())
