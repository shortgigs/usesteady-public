...
import { validateMappedRetirementEnvelopePair, } from "../../claude/evidence-basis.js";
import { AUTHORITY_ASSERTION_SCHEMA, AUTHORITY_ASSERTION_V2_SCHEMA, AUTHORITY_DECISION_RELATION_RETIREMENT, AUTHORITY_DECISION_RELATION_SUPERSESSION, DECISION_ORIGIN_SCHEMA, RETIREMENT_AUTHORITY_ASSERTION_V3_SCHEMA, RETIREMENT_DECISION_ORIGIN_V2_SCHEMA, } from "./types.js";
function isObject(v) {
...
function isCleanString(v, nonEmpty = true) {
    return (typeof v === "string" &&
        !v.includes("\u0000") &&
        (!nonEmpty || v.length > 0));
}
...
function parseRetirementRelation(raw) {
    if (!isObject(raw) || Object.keys(raw).sort().join(",") !== "model_position,resolving_evidence")
        return null;
...
export function canonicalRetirementRelations(relations) {
...
export function parseRetirementDecisionOriginV2(raw) {
    if (!isObject(raw) ||
    ...
    for (const field of ["origin_id", "decision_id", "authority_subject_id", "organization_id", "run_id"]) {
        if (!isCleanString(raw[field]))
            return null;
    }
    if (raw["approver_display"] !== null && !isCleanString(raw["approver_display"], false))
        return null;
    const parsed = relations.map(parseRetirementRelation);
    if (parsed.some((relation) => relation === null))
        return null;
    const typed = parsed;
    ...
    if (o.size !== typed.length || e.size !== typed.length)
        return null;
    ...
    if (canonicalRetirementRelations(typed) !== storedTuples)
        return null;
    return raw;
}
export function parseRetirementAuthorityAssertionV3(raw) {
    if (!isObject(raw) || raw["schema"] !== RETIREMENT_AUTHORITY_ASSERTION_V3_SCHEMA)
        return { ok: false };
...
function parseDecisionOrigin(raw) {
    if (!isObject(raw) || raw["schema"] !== DECISION_ORIGIN_SCHEMA)
        return null;
    const actor = raw["authenticated_actor"];
    const context = raw["authorized_context"];
    if (!isObject(actor) || actor["provider"] !== "supabase" || !isCleanString(actor["subject_id"]))
        return null;
    if (!isObject(context) ||
        (context["role"] !== "org:admin" && context["role"] !== "org:member") ||
        context["capability"] !== "approve_steps")
    ...
    if (strings.some((value) => !isCleanString(value)))
        return null;
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(raw["origin_id"]))
        return null;
    if (raw["approver_display"] !== null && !isCleanString(raw["approver_display"], false))
        return null;
    if (raw["decision"] !== "approve" && raw["decision"] !== "reject")
        return null;
    if (!isCleanString(raw["decided_at"]) || !isIso(raw["decided_at"]))
        return null;
    if (!Number.isSafeInteger(raw["step_index"]) || raw["step_index"] < 0)
        return null;
    if (!Number.isSafeInteger(raw["gate_cycle"]) || raw["gate_cycle"] < 0)
        return null;
    if (!isHex64(raw["decision_basis"]))
        return null;
    ...
    if (relation !== null &&
        ...
        return null;
    if (!Array.isArray(raw["model_positions"]) ||
    ...
    for (const item of raw["model_positions"]) {
        if (!isObject(item) || !isHex64(item["model_position_id"]) || !isHex64(item["position_hash"]))
            return null;
        modelPositions.push({
    ...
    if (relation === null && (modelPositions.length !== 0 || resolvingIds.length !== 0))
        return null;
    if (relation === AUTHORITY_DECISION_RELATION_SUPERSESSION && (modelPositions.length === 0 || resolvingIds.length !== 0))
        return null;
    if (relation === AUTHORITY_DECISION_RELATION_RETIREMENT && (modelPositions.length === 0 || resolvingIds.length === 0))
        return null;
    return raw;
}
function parseV2(raw) {
    if (raw["schema"] !== AUTHORITY_ASSERTION_V2_SCHEMA)
        return null;
    const payload = raw["payload"];
    const signature = raw["signature"];
    if (!isObject(payload) || payload["version"] !== AUTHORITY_ASSERTION_V2_SCHEMA)
        return null;
    const origin = parseDecisionOrigin(payload["decision_origin"]);
    if (!origin || !isHex64(payload["decision_origin_hash"]) || !isCleanString(payload["key_id"]))
        return null;
...
export function parseAuthorityAssertion(raw) {
    if (!isObject(raw))
        return { ok: false };
    if (raw["schema"] !== AUTHORITY_ASSERTION_SCHEMA)
        return { ok: false };
    const p = raw["payload"];
    if (!isObject(p))
        return { ok: false };
    if (p["version"] !== AUTHORITY_ASSERTION_SCHEMA)
        return { ok: false };
    if (!isString(p["decision_id"]) || p["decision_id"].length === 0)
        return { ok: false };
    if (!isString(p["organization_id"]) || p["organization_id"].length === 0)
        return { ok: false };
    if (!isString(p["authority_subject_id"]) || p["authority_subject_id"].length === 0) {
        return { ok: false };
    }
    if (!isStringOrNull(p["approver_display"]))
        return { ok: false };
    if (p["decision"] !== "approve" && p["decision"] !== "reject")
        return { ok: false };
    if (!isIso(p["decided_at"]))
        return { ok: false };
    if (!isStringOrNull(p["run_id"]))
        return { ok: false };
    if (!isStringOrNull(p["thread_id"]))
        return { ok: false };
    if (!isNonNegIntOrNull(p["step_index"]))
        return { ok: false };
    if (!isString(p["decision_basis"]) || p["decision_basis"].length === 0)
        return { ok: false };
    if (!isString(p["key_id"]) || p["key_id"].length === 0)
        return { ok: false };
    ...
    if (p["decision_relation"] !== undefined) {
        ...
        if (rel !== AUTHORITY_DECISION_RELATION_SUPERSESSION &&
            rel !== AUTHORITY_DECISION_RELATION_RETIREMENT) {
            return { ok: false };
        }
        const mps = p["model_positions"];
        if (!Array.isArray(mps) || mps.length === 0 || mps.length > 8)
            return { ok: false };
        for (const mp of mps) {
            if (!isObject(mp))
                return { ok: false };
            if (!/^[0-9a-f]{64}$/.test(String(mp["model_position_id"])))
                return { ok: false };
            if (!/^[0-9a-f]{64}$/.test(String(mp["position_hash"])))
                return { ok: false };
        }
        if (rel === AUTHORITY_DECISION_RELATION_RETIREMENT) {
            const ev = p["resolving_evidence_ids"];
            if (!Array.isArray(ev) || ev.length === 0 || ev.length > 16)
                return { ok: false };
            for (const id of ev) {
...
function verifyV2(assertion, trust, expected) {
    ...
    try {
        const valid = ed25519Verify(null, Buffer.from(payloadCanonical, "utf8"), createPublicKey({ key: Buffer.from(pinned, "base64"), format: "der", type: "spki" }), Buffer.from(assertion.signature.signature, "base64"));
        if (!valid)
    ...
    try {
        if (canonicalModelPositions(origin.model_positions) !==
            canonicalModelPositions(expected.model_positions ?? [])) {
            return { ok: false, reason: "binding_mismatch:model_positions" };
        }
        if (canonicalizeJson([...origin.resolving_evidence_ids].sort()) !==
...
export function verifyDecisionOriginAuthorityAssertionV2(raw, trust, expected) {
    const parsed = parseDecisionOriginAuthorityAssertionV2(raw);
    if (!parsed.ok)
        return { ok: false, reason: "malformed_assertion" };
    return verifyV2(parsed.assertion, trust, expected);
}
export function verifyRetirementAuthorityAssertionV3(raw, trust, expected, readback) {
    ...
    try {
        const valid = ed25519Verify(null, Buffer.from(canonicalizeJson(assertion.payload), "utf8"), createPublicKey({ key: Buffer.from(pinned, "base64"), format: "der", type: "spki" }), Buffer.from(assertion.signature.signature, "base64"));
        if (!valid)
            return { ok: false, reason: "invalid_signature" };
    }
    catch {
    ...
    const scalarChecks = [
        ...
        bindingMismatch("decision_id", origin.decision_id, expected.decision_id),
        bindingMismatch("approver_display", origin.approver_display, expected.approver_display),
        bindingMismatch("decision_relation", origin.decision_relation, expected.decision_relation),
    ];
    ...
    if (expectedRelations === undefined ||
 ...
 */
export function verifyAuthorityAssertion(raw, trust, expected) {
    ...
    let valid = false;
    try {
        valid = ed25519Verify(null, Buffer.from(canonical, "utf8"), createPublicKey({ key: Buffer.from(pinned, "base64"), format: "der", type: "spki" }), Buffer.from(assertion.signature.signature, "base64"));
    }
    catch {
        return { ok: false, reason: "invalid_signature" };
    }
    ...
    if (expected.authority_subject_id !== undefined) {
        checks.push(bindingMismatch("authority_subject_id", payload.authority_subject_id, expected.authority_subject_id));
    }
    if (expected.approver_display !== undefined) {
        checks.push(bindingMismatch("approver_display", payload.approver_display, expected.approver_display));
    }
    ...
    const expectsRelation = expectedRelation !== undefined ||
        (expectedPositions !== undefined && expectedPositions.length > 0);
    const carriesRelation = payload.decision_relation !== undefined || payload.model_positions !== undefined;
    if (expectsRelation) {
