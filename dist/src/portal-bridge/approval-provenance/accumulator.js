...
export function createBridgeApprovalProvenanceState() {
    return { lastBridgeApproval: null };
}
function toBridgeApprovalAuthority(authority) {
 ...
 */
export function recordBridgeApprovalDecision(state, entry, authority) {
 ...
 */
export function accumulateBridgeApprovalFromRemote(state, remote) {
    const base = state ?? createBridgeApprovalProvenanceState();
    if (remote === null || remote.decision !== "yes")
        return base;
 ...
 */
export function resolveExecutionReturnApprovalProvenance(bridgeState, storeDir, workflowRunId) {
