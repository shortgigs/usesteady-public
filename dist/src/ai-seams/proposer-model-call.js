 ...
 */
export function resolveProposerProvider(env = process.env) {
 ...
 */
export function parseProposerProviderPreference(raw) {
    if (typeof raw !== "string")
        return null;
    const v = raw.trim().toLowerCase();
    if (v === "xai" || v === "direct" || v === "grok")
        return "xai";
    if (v === "moonshot" || v === "kimi")
        return "moonshot";
    if (v === "anthropic")
        return "anthropic";
    if (v === "openrouter")
 ...
 */
export function parseProposerTransportPreference(raw) {
    return parseProposerProviderPreference(raw);
}
function isOrgPinned(env) {
 ...
 */
export function envWithProposerProviderPreference(base, preference) {
    if (preference == null)
        return base;
    const next = {
        ...base,
 ...
 */
export function envWithProposerTransportPreference(base, preference) {
    if (preference == null)
        return base;
    const normalized = preference === "direct"
        ? "xai"
...
export function envFromProposerTransportHeader(headerValue, base = process.env) {
    return envWithProposerProviderPreference(base, parseProposerProviderPreference(headerValue));
}
/** Runtime metadata label for work-item records (not authorship provenance). */
export function proposerRuntimeLabel(env = process.env) {
    const provider = resolveProposerProvider(env);
    if (provider === "anthropic") {
        return DEFAULT_ANTHROPIC_PROPOSER_MODEL;
    }
    if (provider === "moonshot") {
        const model = env["USESTEADY_MOONSHOT_MODEL"]?.trim() || DEFAULT_MOONSHOT_PROPOSER_MODEL;
...
function moonshotApiKey(env) {
    const key = env["MOONSHOT_API_KEY"]?.trim() || env["KIMI_API_KEY"]?.trim() || "";
    return key.length > 0 ? key : undefined;
}
function xaiApiKey(env) {
 ...
 */
export function isProposerProviderConfigured(env = process.env) {
    const provider = resolveProposerProvider(env);
    ...
    if (provider === "moonshot") {
        if (pinned)
            return moonshotApiKey(env) !== undefined;
        return (moonshotApiKey(env) !== undefined || anthropicApiKey(env) !== undefined);
    }
    if (provider === "openrouter") {
...
function openrouterModel(env) {
    return (env["USESTEADY_OPENROUTER_MODEL"]?.trim() ||
 ...
 */
export function isOperationalProposerFailure(err) {
    if (err == null)
        return true; // missing key / unavailable treated as operational
    ...
    if (/timeout|timed out|etimedout|econnreset|econnrefused|enotfound|network|socket hang up|fetch failed/.test(msg)) {
        return true;
    }
...
async function callAnthropic(options, env, system, user) {
...
async function callOpenAiCompatible(options, args, system, user) {
    if (!args.apiKey) {
...
async function callXai(options, env, system, user) {
...
async function callMoonshot(options, env, system, user) {
    return callOpenAiCompatible(options, {
 ...
 */
async function callOpenRouter(options, env, system, user) {
    const referer = env["USESTEADY_OPENROUTER_HTTP_REFERER"]?.trim() ||
        "https://usesteady.dev";
...
async function moonshotWithAnthropicFallback(options, env, system, user) {
 ...
 */
export function createProposerModelCall(options) {
    ...
    if (provider === "moonshot") {
        ...
        return async (system, user) => moonshotWithAnthropicFallback(options, env, system, user);
    }
    if (provider === "openrouter") {
        return async (system, user) => {
 ...
 */
export function createProposerModelCallFromTransportHeader(maxTokens, headerValue, base = process.env) {
    return createProposerModelCall({
        maxTokens,
        env: envFromProposerTransportHeader(headerValue, base),
    });
}
//# sourceMappingURL=proposer-model-call.js.map