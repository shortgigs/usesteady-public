...
import { authorizedDeltaPaths, classifyCollateral, diffScopeSnapshots, snapshotFsScope, } from "./scope-snapshot.js";
/**
...
function isInsideRoot(rootReal, p) {
    if (p === rootReal)
        return true;
 ...
 */
export function makeFsRealityProbe(workspaceRoot) {
    const rootResolved = resolve(workspaceRoot);
    return (execution) => {
        const results = execution.results ?? [];
        const claimedRan = results
        ...
        if (execution.observationScope !== undefined && execution.scopeSnapshotPre !== undefined) {
            ...
            const collateral = classifyCollateral(diffScopeSnapshots(execution.scopeSnapshotPre, post), authorized);
            scopeObservation = {
                scope,
                snapshotPost: post,
        ...
        if (collateralMismatch !== null)
            mismatches.push(collateralMismatch);
        ...
        return {
            ...
            detail: `independently verified ${claimedRan.length} actuated op(s) present and correct on disk` +
...
//# sourceMappingURL=fs-reality-probe.js.map