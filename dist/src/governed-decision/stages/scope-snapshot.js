 ...
 */
export function deriveObservationScope(ops) {
    const scopes = new Set();
    for (const op of ops) {
        if (!FS_OP_KINDS.has(op.kind))
            continue;
        const paths = op.kind === "rename_file" ? [op.path, op.toPath] : [op.path];
 ...
 */
function walkInto(abs, rel, entries, seen) {
    if (seen.has(rel))
        return;
    seen.add(rel);
    let st;
    ...
    if (st.isSymbolicLink()) {
        ...
        entries.push(targetHash === undefined
 ...
 */
export function snapshotFsScope(workspaceRoot, scopeRoots) {
...
export function diffScopeSnapshots(pre, post) {
 ...
 */
export function authorizedDeltaPaths(ops) {
 ...
 */
export function classifyCollateral(diff, authorized) {
    const unauthorized = (paths) => paths.filter((p) => !authorized.has(p));
    return {
...
export function makeFsScopeSensor(workspaceRoot) {
...
//# sourceMappingURL=scope-snapshot.js.map