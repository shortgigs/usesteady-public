...
export function buildKernelResultFromRun(run, succeeded, errorCode) {
    const total_steps = run.spec.tasks.length;
    const executed_steps = run.tasks.filter(t => t.outcome === "accepted").length;
    const failedIdx = run.tasks.findIndex(t => t.outcome !== "accepted" && t.outcome !== "pending");
...
export function computeKernelChecksum(ir, result) {
...
export function buildKernelArtifact(ir, result) {
    return {
        version: "kernel.v1",
        ir,
        result,
