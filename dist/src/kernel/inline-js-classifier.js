 ...
 */
export function isReplayableInlineJsShape(body) {
    ...
    if (containsDeniedInlineJsToken(trimmed))
        return false;
    if (containsDeniedInlineJsSyntax(trimmed))
        return false;
 ...
 */
export function containsDeniedInlineJsToken(body) {
 ...
 */
export function containsDeniedInlineJsSyntax(body) {
 ...
 * path needs).
 */
export function isK6NodeECommandShape(command) {
 ...
 * Pure function.
 */
export function extractK6Body(command) {
    if (typeof command !== "string")
        return null;
    const m = command.trim().match(K6_NODE_E_PATTERN);
    return m ? (m[1] ?? null) : null;
}
//# sourceMappingURL=inline-js-classifier.js.map