 ...
 */
export function runReplay(artifactPath) {
    ...
    try {
        ...
        throw new Error(`kernel.replay: artifact is not valid JSON (${artifactPath}): ` +
            (err instanceof Error ? err.message : String(err)));
    }
    if (!isArtifactShape(parsed)) {
        throw new Error(`kernel.replay: artifact is missing required fields {ir, result, checksum} (${artifactPath})`);
    }
