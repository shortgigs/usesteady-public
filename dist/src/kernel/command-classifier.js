 ...
 */
import { isK6NodeECommandShape, extractK6Body, isReplayableInlineJsShape, } from "./inline-js-classifier.js";
 ...
 */
export function isReplayableCommandShape(command) {
 ...
 * to call this.
 */
export function classifyInScopeCommand(command) {
    const trimmed = command.trim();
    ...
    if (ECHO_PATTERN.test(trimmed))
        return { kind: "echo", command: trimmed };
    throw new Error(`classifyInScopeCommand called with non-in-scope command: ${JSON.stringify(command)}. ` +
