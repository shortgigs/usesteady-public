...
import { makeBoundaryJsonlSink, makeReplayFsBoundaryObserver, observeInvocation, } from "../execution/boundary-observer.js";
...
function buildFsChangeForReplay(spec) {
...
// function directly.
export function executeStructuredReplace(spec, workspaceRoot) {
...
export function executeRunCommand(spec, workspaceRoot) {
 ...
 */
function fsChangePathsAreContained(op) {
...
async function executeOneTask(spec, plugin, workspaceRoot, sink) {
    ...
    if (spec.structuredReplace && !spec.operationType) {
        if (sink) {
            return observeInvocation(executeStructuredReplace, sink, "replay_structured_replace.execute")(spec, workspaceRoot);
        }
    ...
    if (spec.operationType === "run_command") {
        if (sink) {
            return observeInvocation(executeRunCommand, sink, "replay_run_command.execute")(spec, workspaceRoot);
        }
    ...
    const result = await plugin.executeFsOp(op);
...
function buildReplayKernelResult(totalSteps, outcomes) {
    ...
    const capturedErrorCode = failedIdx === -1 ? null : outcomes[failedIdx].errorCode;
...
function loadArtifact(artifactPath) {
    ...
    try {
        ...
        throw new Error(`kernel.execution-replay: artifact is not valid JSON (${artifactPath}): `
            + (err instanceof Error ? err.message : String(err)));
    }
    if (!isArtifactShape(parsed)) {
        throw new Error(`kernel.execution-replay: artifact is missing required fields {ir, result, checksum} (${artifactPath})`);
    }
    return parsed;
}
function isArtifactShape(value) {
    if (value === null || typeof value !== "object")
        return false;
    const obj = value;
    return (typeof obj["checksum"] === "string"
 ...
 * 0 / 1 / 3 respectively.
 */
export async function runExecutionReplay(artifactPath, opts) {
    ...
    if (integrity.replay !== "match") {
        throw new Error(`kernel.execution-replay: refusing to execute a tampered artifact (`
    ...
    const classification = classifyForReplay(artifact.ir);
    ...
    try {
        ...
        const plugin = makeReplayFsBoundaryObserver(new CursorInProcessAdapter(sandbox.path), sink);
...
//# sourceMappingURL=execution-replay.js.map