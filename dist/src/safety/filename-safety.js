 ...
 *              the whole path).
 */
export function validateFilenameSafety(path) {
    if (typeof path !== "string") {
        return {
            ok: false,
            code: "canonicalization_failed",
            detail: "Filename input is not a string — UseSteady refused this safely " +
    ...
    if (offenders.length > 0) {
        return {
            ...
            detail: buildOffenderDetail("Filename", offenders, "Filenames with invisible, bidirectional, or control codepoints are " +
    ...
    if (whitespaceSegment !== null) {
        ...
        return {
            ...
            detail: `Filename "${path}" contains a path segment "${whitespaceSegment}" ` +
    ...
    try {
        ...
        return {
            ok: false,
            code: "canonicalization_failed",
            detail: `Filename "${path}" could not be canonicalized — UseSteady refused ` +
    ...
    if (canonical !== path) {
        return {
            ...
            detail: `Filename "${path}" canonicalizes to a different string (NFKC ` +
 ...
 */
export function validateReplacementTextSafety(text) {
    if (typeof text !== "string") {
        return {
            ok: false,
            code: "canonicalization_failed",
            detail: "Replacement text input is not a string — UseSteady refused this " +
    ...
    if (offenders.length > 0) {
        return {
            ...
            detail: buildOffenderDetail("Replacement text", offenders, "Replacement text containing invisible, bidirectional, or control " +
 ...
 */
export function classifyNormalizationDivergence(oldValue, content) {
    if (containsCovertCodepoint(oldValue))
        return "hidden_codepoints_in_oldvalue";
    if (containsCovertCodepoint(content))
        return "hidden_codepoints_in_content";
 ...
 */
export function describeNormalizationDivergence(cause, oldValue, filePath) {
    switch (cause) {
        case "unicode_normalization_mismatch":
            return (`Unicode normalization mismatch for "${oldValue}" in "${filePath}": ` +
        ...
        case "hidden_codepoints_in_oldvalue":
            return (`Search string "${oldValue}" contains invisible or bidirectional ` +
        ...
        case "hidden_codepoints_in_content":
            return (`File "${filePath}" contains invisible or bidirectional codepoints ` +
        ...
        case "other":
            return (`Match-count mismatch for "${oldValue}" in "${filePath}" between ` +
...
function collectOffenders(input, rules) {
    const out = [];
 ...
 */
function buildOffenderDetail(subjectLabel, offenders, guidance) {
    ...
    const lead = `${subjectLabel} contains ${count} ${classSummary} ` +
        `codepoint${count === 1 ? "" : "s"} ` +
...
function describeClasses(classes) {
    const order = ["bidi-control", "zero-width", "control"];
...
function containsCovertCodepoint(s) {
    return (ZERO_WIDTH_CODEPOINTS.test(s) ||
 ...
 */
function suggestReservedNameRename(candidate) {
 ...
 */
function findWhitespaceBoundarySegment(path) {
    ...
    for (const seg of segments) {
        if (seg.length === 0)
            continue; // skip empties from `/foo`, `foo/`, `foo//bar`
        if (seg.trim() !== seg)
            return seg;
    }
    return null;
 ...
 */
export function validateProtectedPath(path) {
    ...
    for (const seg of segments) {
        ...
        if (seg.toLowerCase() === ".git") {
            return {
                ...
                detail: `Refusing operation on protected path "${path}". The "${seg}" ` +
 ...
 */
function matchWindowsReservedName(path) {
    if (path.length === 0)
        return null;
 ...
 */
function matchReservedDeviceNameInSegment(segment) {
    if (segment.length === 0)
        return null;
    let trimmed = segment;
...
//# sourceMappingURL=filename-safety.js.map