 ...
 */
import { createProposerModelCall, isProposerProviderConfigured, } from "../ai-seams/proposer-model-call.js";
import { DIRECT_CAPABLE_FAMILIES, GUIDED_ONLY_FAMILIES, REQUIRED_SLOTS, isDirectCapable, } from "./intent-families.js";
...
function isValidFamily(v) {
    return (typeof v === "string" &&
        [...DIRECT_CAPABLE_FAMILIES, ...GUIDED_ONLY_FAMILIES].includes(v));
}
...
function sanitizeMissingSlots(raw) {
...
function validate(raw) {
    if (!isValidFamily(raw["intent_family"]))
        return null;
    const family = raw["intent_family"];
...
export async function classifyIntent(input) {
    if (!isProposerProviderConfigured())
        return null;
    try {
        const text = await classifyModelCall(SYSTEM_PROMPT, input);
        if (text === null)
            return null;
        const raw = JSON.parse(extractJsonText(text));
        return validate(raw);
    }
    catch {
        return null;
