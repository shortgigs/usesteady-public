 ...
 */
function looksLikeSingleActionRequest(rawInput) {
 ...
 */
function extractLikelyName(rawInput) {
...
function buildSuggestions(rawInput) {
    ...
    if (!looksLikeSingleActionRequest(rawInput))
        return [];
    ...
    if (/\b(?:to|into)\s+\S*\.[a-z0-9]+\s*$/i.test(lower))
        return [];
    ...
    if (isCreate) {
        if (!name)
            return []; // can't suggest without a name
        if (isDir && !isFile) {
 ...
 */
export class LocalRecoveryAdapter {
    async invoke(skill, rawInput) {
...
//# sourceMappingURL=local-recovery-adapter.js.map