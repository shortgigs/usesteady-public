 ...
 */
export function buildRegistry(accepted) {
    const sorted = [...accepted].sort(compareSkills);
 ...
 */
function makeParserRejection(filePath, err) {
 ...
 */
export async function scanSkillsDirectory(baseDir) {
    ...
    try {
        ...
        throw new SkillLoadError('SCAN_FAILED', baseDir, cause instanceof Error ? cause.message : String(cause));
    }
