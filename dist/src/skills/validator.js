 ...
 */
import { KIND_SCHEMA_MAP, REQUIRED_BODY_SECTIONS, } from './types.js';
// ---------------------------------------------------------------------------
...
function isSkillKind(v) {
    return (v === 'interpretation' ||
        v === 'workflow_generation' ||
        v === 'recovery');
}
function isSkillOutputSchema(v) {
    return (v === 'usesteady.intent_candidates.v1' ||
...
// ---------------------------------------------------------------------------
function checkRequiredString(fm, field, issues) {
...
function checkVersion(fm, issues) {
    ...
    if (value === undefined || value === null) {
        ...
        issues.push({
            type: 'invalid_field',
            field: 'version',
            message: `Field "version" must be a semantic version string (e.g. "1.0.0"); ` +
...
function checkKind(fm, issues) {
    ...
    if (!isSkillKind(value)) {
        issues.push({
            type: 'invalid_kind',
            field: 'kind',
            message: `Field "kind" must be one of "interpretation", "workflow_generation", ` +
 ...
 */
function checkOutputSchema(fm, validatedKind, issues) {
    ...
    if (!isSkillOutputSchema(value)) {
        issues.push({
            type: 'invalid_field',
            field: 'output_schema',
            message: `Field "output_schema" must be one of the three supported schema identifiers; ` +
                `got ${JSON.stringify(value)}.`,
        });
        return;
    }
    ...
    if (validatedKind !== null) {
        ...
        if (value !== expectedSchema) {
            issues.push({
                ...
                field: 'output_schema',
                message: `Kind "${validatedKind}" requires output_schema "${expectedSchema}" ` +
                    `but got "${value}".`,
            });
        }
    }
...
const GUARDRAIL_CHECKS = [
    { field: 'requires_review', expected: true },
    { field: 'can_execute', expected: false },
    { field: 'can_call_tools', expected: false },
    { field: 'can_bypass_policy', expected: false },
    { field: 'can_bypass_approval', expected: false },
];
function checkGuardrails(fm, issues) {
    for (const { field, expected } of GUARDRAIL_CHECKS) {
        ...
        if (value === undefined || value === null) {
            issues.push({
                ...
                field,
                message: `Guardrail "${field}" is absent; ` +
            ...
            issues.push({
                type: 'guardrail_violation',
                field,
                message: `Guardrail "${field}" must be exactly ${JSON.stringify(expected)}; ` +
                    `got ${JSON.stringify(value)}.`,
            });
        }
    }
}
function checkTriggers(fm, issues) {
 ...
 */
function checkPriority(fm, issues) {
    ...
    if (typeof value !== 'number' || !Number.isFinite(value)) {
        issues.push({
            type: 'invalid_field',
            field: 'priority',
            message: `Field "priority" must be a finite number when present; ` +
                `got ${JSON.stringify(value)}.`,
        });
    }
}
 ...
 */
function checkBodySections(sections, issues) {
    for (const sectionName of REQUIRED_BODY_SECTIONS) {
        ...
        if (content === undefined) {
            issues.push({
                ...
                message: `Required body section "${sectionName}" is absent or has no content.`,
            });
        }
    }
}
 ...
 */
export function validateSkill(parsed, sourcePath) {
    ...
    if (typeof parsed.frontmatter !== 'object' ||
