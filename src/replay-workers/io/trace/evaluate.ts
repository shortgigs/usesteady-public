/**
 * Pure replay namespace trace-line per-scope I/O eligibility evaluator.
...
function decideIoEligibility(
  ...
  if (!isKnownMutationState(mutation.replay_worker_mutation_state)) {
    return {
      replay_worker_per_scope_io_eligibility_state: "blocked",
      reason:         "Replay namespace trace I/O eligibility policy fail-closed.",
      blocking_cause: "policy_ambiguous",
    };
  }

  if (mutation.replay_worker_mutation_scope !== REPLAY_NAMESPACE_TRACE_LINE_APPEND_SCOPE) {
    return {
      replay_worker_per_scope_io_eligibility_state: "blocked",
      reason:         "Per-scope I/O scope must exactly match replay_namespace_trace_line_append.",
      blocking_cause: "scope_mismatch",
    };
  }

  if (!isReplayNamespaceTraceLineAppendScope(mutation.replay_worker_mutation_scope)) {
    return {
      replay_worker_per_scope_io_eligibility_state: "blocked",
      reason:         "Replay worker mutation scope is not trace-line append.",
      blocking_cause: "scope_mismatch",
    };
  }
  ...
  return {
    ...
    reason:         "Replay worker mutation is eligible for replay-namespace trace-line append I/O.",
    blocking_cause: "none",
  };
}
