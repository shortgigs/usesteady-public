/**
 * Pure replay namespace single-path metadata per-scope I/O eligibility evaluator.
...
function decideIoEligibility(
  ...
  if (!isKnownMutationState(mutation.replay_worker_mutation_state)) {
    return {
      replay_worker_per_scope_io_eligibility_state: "blocked",
      reason:         "Replay namespace metadata I/O eligibility policy fail-closed.",
      blocking_cause: "policy_ambiguous",
    };
  }

  if (mutation.replay_worker_mutation_scope !== REPLAY_NAMESPACE_SINGLE_PATH_METADATA_SCOPE) {
    return {
      replay_worker_per_scope_io_eligibility_state: "blocked",
      reason:         "Per-scope I/O scope must exactly match replay_namespace_single_path_metadata.",
      blocking_cause: "scope_mismatch",
    };
  }

  if (!isReplayNamespaceSinglePathMetadataScope(mutation.replay_worker_mutation_scope)) {
    return {
      replay_worker_per_scope_io_eligibility_state: "blocked",
      reason:         "Replay worker mutation scope is not single-path metadata.",
      blocking_cause: "scope_mismatch",
    };
  }
  ...
  return {
    ...
    reason:         "Replay worker mutation is eligible for single-path metadata I/O.",
    blocking_cause: "none",
  };
}
...
/**
 * Pure evaluator — ReplayWorkerMutationRecord only; metadata scope only; no I/O.
 */
