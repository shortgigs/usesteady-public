 ...
 */
export function groundsIdFromBasis(
  artifactId: string,
  basis: DeliveryEvidenceBasis | undefined,
): GroundsId {
  if (basis === undefined) return "unknown";
  const sources = basis.sources
    .filter((s) => (ACTION_GROUND_SOURCES as readonly string[]).includes(s.source))
    .map((s) => ({ source: s.source, availability: s.availability }))
    .sort((a, b) => a.source.localeCompare(b.source));
  if (sources.length === 0) return "unknown";
  if (sources.some((s) => s.availability === "unknown")) return "unknown";
  return hashObject({ artifactId, sources });
}

export function compareGrounds(retired: GroundsId, incoming: GroundsId): GroundsRelation {
  if (retired === "unknown" || incoming === "unknown") return "unknown";
  return retired === incoming ? "same" : "new";
}

export function validateRetirementRequest(req: RetirementRequest): RetirementValidation {
  if (req.resolvingEvidenceIds.length === 0) {
    return { ok: false, reason: "empty_evidence" };
  }
  for (const id of req.resolvingEvidenceIds) {
    if (!SHA256_HEX.test(id)) return { ok: false, reason: "malformed_evidence_id" };
  }
 ...
 */
export function retirementAppliesTo(
  retirement: Pick<RetiredPositionRef, "modelPositionId">,
  objectionId: string,
): boolean {
...
export function deriveStanding(
  objectionId: string,
  retirements: readonly Pick<RetiredPositionRef, "modelPositionId">[],
): ObjectionStanding {
 ...
 */
export function classifyIncomingAdvisory(
  incoming: { readonly modelPositionId: string; readonly groundsId: GroundsId },
  retired: readonly Pick<RetiredPositionRef, "modelPositionId" | "groundsId">[],
): IncomingClassification {
  if (retired.length === 0) return "standing";
  ...
  if (incoming.groundsId === "unknown") return "retired_reassertion";
  for (const r of retired) {
    const rel = compareGrounds(r.groundsId, incoming.groundsId);
    if (rel === "unknown" || rel === "same") return "retired_reassertion";
  }
  return "standing";
}

export function canonicalResolvingEvidenceIds(ids: readonly string[]): readonly string[] {
  return [...ids].sort();
}
