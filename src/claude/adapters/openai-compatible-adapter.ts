 ...
 *   });
 */
export class OpenAiCompatibleAgentAdapter implements ClaudeAgentPlugin {
...
function isOpenAiCompatEnabled(): boolean {
...
function buildUserMessage(taskSpec: ClaudeTaskSpec): string {
  const lines: string[] = [
...
const SCOPE_CLARIFICATION_TOOL: OpenAI.ChatCompletionTool = {
  type: "function",
  function: {
...
const KNOWN_TOOLS: Record<string, OpenAI.ChatCompletionTool> = {
  str_replace_editor: {
    type: "function",
    function: {
...
function resolveTools(allowedTools: readonly string[]): OpenAI.ChatCompletionTool[] {
  const tools: OpenAI.ChatCompletionTool[] = [SCOPE_CLARIFICATION_TOOL];
...
function extractScopeQuestion(
  ...
  for (const call of toolCalls) {
    ...
    if (call.function.name !== "request_scope_clarification") continue;
