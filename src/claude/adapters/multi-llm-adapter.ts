...
import {
  ...
  type MultiLlmOptions,
  type NormalizedDecision,
  type ConsensusAuditRecord,
  type ConsensusRoundEntry,
  type PolicyEvaluation,
} from "./multi-llm-types.js";
import {
  appendConsensusAuditRecord,
  resolveConsensusAuditDir,
} from "./consensus-audit-sink.js";
 ...
 */
export class MultiLlmPlugin implements ClaudeAgentPlugin {
  private readonly primaryEntry:    NamedPlugin;
  private readonly secondaryPlugins: readonly NamedPlugin[];
  private readonly options:          MultiLlmOptions;

  constructor(
    primaryEntry:     NamedPlugin,
    secondaryPlugins: readonly NamedPlugin[],
    options:          Omit<MultiLlmOptions, keyof typeof DEFAULT_MULTI_LLM_OPTIONS> &
                      Partial<Pick<MultiLlmOptions, keyof typeof DEFAULT_MULTI_LLM_OPTIONS>>,
  ) {
  ...
  async receive(request: ClaudeDeliveryRequest): Promise<ClaudeDeliveryResponse> {
    ...
    const rounds:     ConsensusRoundEntry[] = [];

    let finalResponse:    ClaudeDeliveryResponse | undefined;
    let finalEvaluation:  PolicyEvaluation | undefined;
    // Snapshot of hashes from the last completed round — carried through to every audit write.
    let lastHashPrimary:  string | undefined;
    let lastHashesAll:    Record<string, string> = {};
    ...
    for (let roundNum = 1; roundNum <= this.options.maxRounds; roundNum++) {
      ...
      if (primaryResult === undefined) {
        ...
        const failRecord: ConsensusAuditRecord = {
      ...
      const allDecisions    = Array.from(results.entries()).map(([, r]) =>
        normalizeResponse(request, r.response),
      );
      ...
      if (
        evaluation.quorumState === "no_quorum" ||
      ...
      if (evaluation.quorumState === "primary_only") {
        ...
        const { results: reviewResults, timedOut: reviewTimedOut } =
          await this.runRound(allPlugins, reviewRequest, roundNum + 0.5, rounds);
        ...
        const reviewDecisions = Array.from(reviewResults.values()).map((r) =>
          normalizeResponse(reviewRequest, r.response),
        );
        ...
        const reviewHashesAll       = extractHashMap(
          reviewResults,
          (r) => normalizeResponse(reviewRequest, r.response).hash,
        );
        ...
        const failedClosedReason =
          "failReason" in reviewEval
  ...
  private async runRound(
    ...
    roundNum:  number,
    rounds:    ConsensusRoundEntry[],
  ): Promise<{
    results:  Map<string, { response: ClaudeDeliveryResponse; durationMs: number }>;
    timedOut: boolean;
  }> {
    ...
    const calls = plugins.map((named) =>
      this.callWithTimeout(named, request, roundNum, rounds, roundAbort.signal),
    );

    const settled = await Promise.allSettled(calls);
    clearTimeout(roundTimer);

    const results = new Map<string, { response: ClaudeDeliveryResponse; durationMs: number }>();
    let timedOut  = false;

    for (const outcome of settled) {
      if (outcome.status === "rejected") {
        // callWithTimeout caught and recorded the error — timeout or plugin threw
        if ((outcome.reason as { isTimeout?: boolean }).isTimeout === true) {
          timedOut = true;
        }
        continue;
      }
  ...
  ): Promise<{ name: string; response: ClaudeDeliveryResponse; durationMs: number } | null> {
    const start       = Date.now();
    const pluginAbort = new AbortController();

    const pluginTimer = setTimeout(
      () => pluginAbort.abort(),
      this.options.pluginTimeoutMs,
    );
    ...
    try {
      ...
      const response = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          pluginAbort.signal.addEventListener(
            "abort",
            () => {
              const err: Error & { isTimeout?: boolean } = new Error(
                `Plugin "${named.name}" did not respond within ${this.options.pluginTimeoutMs} ms.`,
              );
              err.isTimeout = true;
              reject(err);
            },
      ...
      const tagged: Error & { isTimeout?: boolean } = new Error(
        isTimeout ? "Plugin timed out" : String(err),
      );
  ...
  private findScopeResponse(
    results: Map<string, { response: ClaudeDeliveryResponse; durationMs: number }>,
  ): ClaudeDeliveryResponse | undefined {
    for (const { response } of results.values()) {
      if (response.kind === "refused_due_to_scope") return response;
    }
    return undefined;
  }
}
...
function buildReviewRequest(
  original:    ClaudeDeliveryRequest,
  priorOutputs: readonly NormalizedDecision[],
  errorCategory: "execution_error" | "hash_mismatch",
): ClaudeDeliveryRequest {
  const payload = buildReviewPayload(
    original.deliveryId,
    priorOutputs,
    errorCategory,
    `Prior dispositions: ${priorOutputs.map((d) => d.disposition).join(", ")}. ` +
  ...
  return {
    ...
    artifact: {
      ...
      taskSpec: {
        ...
        summary:
          `[REVIEW ROUND] ${original.artifact.taskSpec.summary}\n` +
 ...
 */
function isCapabilitySplit(
  rounds:        readonly ConsensusRoundEntry[],
  primaryPlugin: string,
): boolean {
  const primaryRound1 = rounds.find(
    r => r.pluginName === primaryPlugin && r.roundNumber === 1,
  );
  if (!primaryRound1 || primaryRound1.disposition !== "accepted") return false;

  const secondaryRound1 = rounds.filter(
    r => r.pluginName !== primaryPlugin && r.roundNumber === 1,
  );
  if (secondaryRound1.length === 0) return false;

  return secondaryRound1.every(
    r => r.operationClass === "execution_error" || r.disposition === "refused_error",
  );
}

// ─── Audit helpers ────────────────────────────────────────────────────────────

function writeAudit(record: ConsensusAuditRecord): void {
  ...
  try {
    process.stderr.write(
      `[consensus-audit] ${JSON.stringify(record)}\n`,
    );
  } catch {
    // Best-effort — do not throw, do not affect response
  }
 ...
 */
function extractHashMap(
  results:   Map<string, { response: ClaudeDeliveryResponse; durationMs: number }>,
  getHash:   (r: { response: ClaudeDeliveryResponse; durationMs: number }) => string,
 ...
 */
function hashAuditFields(
  primary: string | undefined,
  all:     Record<string, string>,
): { decisionHashPrimary?: string; decisionHashesAll?: Record<string, string> } {
  const out: { decisionHashPrimary?: string; decisionHashesAll?: Record<string, string> } = {};
  if (primary !== undefined && primary !== "") out.decisionHashPrimary = primary;
  if (Object.keys(all).length > 0)            out.decisionHashesAll   = all;
  return out;
}
