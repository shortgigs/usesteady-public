 ...
 */
function createDirAncestorDisclosure(target: string, workspaceRoot: string): string | null {
...
function isStyleOnly(path: string): boolean {
...
function isSharedLogicFile(path: string): boolean {
  return SHARED_LOGIC_PATTERNS.some(re => re.test(path));
}

function isCosmeticInput(input: string): boolean {
  return /\b(colour|color|text|copy|label|placeholder|spacing|margin|padding|font|style)\b/i.test(input);
}

function isRuntimeConfigFile(path: string): boolean {
  return /(?:^|\/)config\.(ts|tsx|js|jsx|mjs)$/i.test(path);
}

function inferConfigModule(filePath: string): string {
  const parts = filePath.replace(/\\/g, "/").split("/");
  const configIdx = parts.findIndex(p => /^config\./i.test(p));
  if (configIdx > 0) return parts[configIdx - 1]!;
  return "this";
}

function derivedRisk(
  op:    WorkflowTaskSpec["operationType"],
  files: string[],
  task?: WorkflowTaskSpec,
): ExecutionStep["risk_level"] {
  if (op === "delete_file") return "high";
  if (files.some(isHighRiskFile)) return "high";
  if (op === "rename" || files.length > 3) return "medium";
  if (files.every(isTestFile) && files.length > 0) return "low";
  if (files.every(isStyleOnly) && files.length > 0) return "low";
  if (task && isCosmeticInput(task.input)) return "low";
  if ((op === "replace" || op === undefined) && files.some(isRuntimeConfigFile)) return "medium";
  if ((op === "replace" || op === undefined) && files.some(isSharedLogicFile)) return "medium";
  if (files.some(f => /\.(tsx|jsx)$/.test(f))) return "medium";
  return "low";
}
...
function inferDeleteConsequences(filePath: string): string[] {
...
function derivedImpact(
  ...
  if (op === "delete_file") {
    ...
    const disclosure = files.length > 0
      ...
      : null;
    if (disclosure !== null) impact.push(disclosure);
  } else if (op === "replace") {
    ...
    if (files.length > 1) impact.push(`${files.length} files will be modified.`);
...
function derivedRiskNotes(
  op:    WorkflowTaskSpec["operationType"],
  task:  WorkflowTaskSpec,
  files: string[],
): string[] {
  const notes: string[] = [];
  if (op === "delete_file") notes.push("Check git status before approving — deletion cannot be undone without VCS.");
  if (op === "rename")      notes.push(`Verify no hard-coded references to the old path exist outside of ${files[0] ?? "this file"}.`);
  if (files.some(isHighRiskFile)) notes.push("This file type is commonly shared across environments — changes may have wide blast radius.");
  if ((op === "replace" || op === undefined) && files.some(isRuntimeConfigFile)) {
    notes.push(
      `Configuration values in ${files.filter(isRuntimeConfigFile).map(f => inferConfigModule(f)).join(", ")} module` +
      " control runtime behavior — the exact characters being changed are only visible at execution time.",
    );
  }
  if (files.length > 3)    notes.push("Large number of affected files — consider reviewing each file individually.");
  return notes;
}
...
function derivedWhy(
  ...
  const createDirSuffix = (() => {
    if (op !== "create_dir" || files.length === 0) return "";
  ...
  const consequence =
    op === "delete_file" ? (
      `The system will permanently delete ${files.join(", ")}.\n\n` +
    ...
    op === "replace"     ? (
      files.some(isRuntimeConfigFile)
...
export function specToExecutionSteps(
  spec:          WorkflowSpec,
  workspaceRoot: string,
): ExecutionStep[] {
  return spec.tasks.map(task => {
    const op    = task.operationType;
    const files = (task.targetFiles ?? []) as string[];

    const effectiveFiles: string[] = files.length > 0 ? files : (() => {
      if (!task.input) return [];
      const createFolder = task.input.match(
        /\b(?:create\s+folder|mkdir)\s+([\w./\\-]+)/i,
      );
      if (createFolder?.[1]) return [createFolder[1].replace(/\/$/, "")];
      const createFile = task.input.match(
        /\bcreate\s+file\s+([\w./\\-]+\.[a-zA-Z0-9]+)/i,
      );
      if (createFile?.[1]) return [createFile[1]];
      const inMatch = task.input.match(/\bin\s+([\w./\\-]+\.[a-zA-Z]{1,5})\s*$/i);
      if (inMatch?.[1]) return [inMatch[1]];
      const verbMatch = task.input.match(
        /\b(?:delete|remove|create|write|update|rename)\s+([\w./\\-]+\.[a-zA-Z]{1,5})\b/i,
      );
    ...
    const inferredOp = op ?? (() => {
      if (!task.input) return undefined;
      const lc = task.input.toLowerCase().trimStart();
      if (/^(?:delete|remove)\b/.test(lc))            return "delete_file" as const;
      if (/^(?:rename|move)\b/.test(lc))               return "rename"      as const;
      if (/^(?:mkdir|create\s+(?:folder|directory|dir))\b/.test(lc)) return "create_dir" as const;
      if (/^create\s+file\b/.test(lc)) return "write_file" as const;
      if (/^(?:create|add\s+file|write\s+file)\b/.test(lc)) return "write_file" as const;
      return undefined;
    })();

    const actionType: ExecutionStep["action_type"] =
      inferredOp === "create_dir" || inferredOp === "write_file" ? "create" :
