...
function suggestedActionMatchesCode(action: string, code: string): boolean {
...
export function renderWorkflowHealthSection(record: DiagnosticRecord): string {
  const lines: string[] = [
    "",
...
export function renderEntitlementHealthSummary(record: DiagnosticRecord): string {
  ...
  const entitlementActions = record.suggested_actions.filter((action) =>
    entitlement.some((o) => suggestedActionMatchesCode(action, o.code)),
  );
