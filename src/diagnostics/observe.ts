...
import {
  ...
  resolveOpsBaseUrl,
  resolveOrgIdForRefresh,
  type EffectiveState,
  type EntitlementCacheRecord,
} from "../shell/cli/entitlement-cache.js";
...
export function readUseSteadyConfig(): Record<string, unknown> | null {
...
export function probeIdentityObservations(
  env: NodeJS.ProcessEnv = process.env,
): DiagnosticObservation[] {
  const at = nowIso();
  const out: DiagnosticObservation[] = [];
...
export function probeRuntimeObservations(
  env: NodeJS.ProcessEnv = process.env,
): DiagnosticObservation[] {
  const at = nowIso();
  const out: DiagnosticObservation[] = [];
...
export function probePortalObservations(
  env: NodeJS.ProcessEnv = process.env,
): DiagnosticObservation[] {
  const at = nowIso();
  const out: DiagnosticObservation[] = [];
...
export function collectLocalObservations(
  ...
  cachePath?: string,
  now?: Date,
): { observations: DiagnosticObservation[]; effective: EffectiveState; cache: EntitlementCacheRecord | null } {
...
export async function collectIssuerObservations(
  env: NodeJS.ProcessEnv = process.env,
...
export async function collectObservations(input: {
  ...
  let issuerSnapshot: import("../shell/cli/entitlement-cache.js").IssuerSnapshot | null = null;
  const issuerEntitlementObs: DiagnosticObservation[] = [];
  ...
  const base = local.observations.filter(
    (o) => o.class !== "entitlement" && o.class !== "billing",
  );
