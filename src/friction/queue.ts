...
function rowToEntry(row: Record<string, unknown>): QueueEntry {
  const base = {
    id:        row["id"]        as string,
    event:     JSON.parse(row["event_json"] as string) as FrictionEvent,
 ...
 */
export function enqueue(event: FrictionEvent): string {
  const id  = randomUUID();
 ...
 */
export function listByStatus(status: QueueStatus): QueueEntry[] {
  try {
    const rows = db().prepare(
      "SELECT * FROM friction_queue WHERE status = ? ORDER BY created_at ASC"
 ...
 */
export function pendingEntries(limit = 20): QueueEntry[] {
  try {
    const rows = db().prepare(`
...
export function markFailed(id: string, error: string): void {
  try {
    const row = db().prepare(
      "SELECT attempts FROM friction_queue WHERE id = ?"
    ).get(id) as { attempts: number } | undefined;

    const nextAttempts = (row?.attempts ?? 0) + 1;
    const nextStatus: QueueStatus = nextAttempts >= MAX_ATTEMPTS ? "failed" : "pending";
 ...
 */
export function pruneSubmitted(keepDays = 30): number {
  try {
 ...
 */
export function queueStats(): Record<QueueStatus, number> {
  try {
    const rows = db().prepare(
      "SELECT status, COUNT(*) AS cnt FROM friction_queue GROUP BY status"
    ).all() as { status: QueueStatus; cnt: number }[];

    const stats: Record<QueueStatus, number> = { pending: 0, submitted: 0, failed: 0 };
