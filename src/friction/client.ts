...
function buildIssueBody(entry: QueueEntry, githubLogin: string): string {
  const { event } = entry;
  const ctx = event.context;

  const contextLines: string[] = [];
  if (ctx.phase)         contextLines.push(`- **Phase:** ${ctx.phase}`);
  if (ctx.runId)         contextLines.push(`- **Run ID:** \`${ctx.runId}\``);
  if (ctx.taskInputs?.length) {
    contextLines.push("- **Task inputs:**");
    for (const t of ctx.taskInputs) contextLines.push(`  - ${t}`);
  }
  if (ctx.failureNote)   contextLines.push(`- **Failure note:** ${ctx.failureNote}`);
  if (ctx.workspaceRoot) contextLines.push(`- **Workspace:** \`${ctx.workspaceRoot}\``);
  if (ctx.route)         contextLines.push(`- **Route:** \`${ctx.route}\``);
  if (ctx.errorMessage)  contextLines.push(`- **Error:** ${ctx.errorMessage}`);
...
function buildIssueTitle(entry: QueueEntry): string {
  const { event } = entry;
  const ctx = event.context;
 ...
 */
export async function flushQueue(config: FrictionConfig): Promise<number> {
  const auth = readAuth();
  if (!auth?.githubToken) return 0;   // nothing to do without a token
 ...
 */
export function scheduleFlush(config: FrictionConfig): void {
  setImmediate(() => {
