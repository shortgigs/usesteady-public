...
export async function swapLabel(
  ...
  await ghFetch(
    ...
    token,
    { method: "POST", body: JSON.stringify({ labels: [addLabel] }) },
  );
 ...
 */
export function parseIssueMetadata(body: string): Record<string, unknown> | null {
  try {
 ...
 */
export function parseSubmitterEmail(body: string): string | null {
 ...
 */
export function parseIssueContext(body: string): {
  kind?:        string;
  taskInputs?:  string[];
  failureNote?: string;
  phase?:       string;
  ...
  const taskInputs: string[] | undefined = taskSection1
    ? taskSection1.split("\n").map(l => l.replace(/^  - /, "").trim()).filter(Boolean)
    : undefined;

  const result: { kind?: string; taskInputs?: string[]; failureNote?: string; phase?: string; notes?: string } = {};
  ...
  if (failureNote !== undefined) result.failureNote = failureNote;
  if (taskInputs  !== undefined) result.taskInputs  = taskInputs;
