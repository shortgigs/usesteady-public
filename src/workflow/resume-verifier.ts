...
import {
  ...
  type ResumeVerificationReport,
  type TaskVerificationFinding,
  type TaskVerificationVerdict,
} from "./resume-verifier-types.js";
 ...
 */
function classifyOpType(opType: string): IdempotencyClass {
  switch (opType) {
...
function classifyTask(task: WorkflowTaskSpec): { opType: string; klass: IdempotencyClass } {
...
// ─── Per-op checkable verification ──────────────────────────────────────────

/**
 * Probe filesystem to decide already_done vs diverged for a `checkable`
 * task. Pure inspection — no writes. Returns one of:
 *
 *   { verdict: "already_done",        detail: "..." }
 *   { verdict: "task_state_diverged", detail: "..." }
 */
type CheckableProbe =
  | { readonly verdict: "already_done"; readonly detail: string }
  | { readonly verdict: "task_state_diverged"; readonly detail: string };

function probeCreate(task: WorkflowTaskSpec, workspaceRoot: string): CheckableProbe {
...
function probeDelete(task: WorkflowTaskSpec, workspaceRoot: string): CheckableProbe {
  const target = firstTargetPath(task);
  if (!target) {
...
function probeRename(task: WorkflowTaskSpec, workspaceRoot: string): CheckableProbe {
...
function probeCreateDir(task: WorkflowTaskSpec, workspaceRoot: string): CheckableProbe {
  const target = firstTargetPath(task);
  if (!target) {
...
function firstTargetPath(task: WorkflowTaskSpec): string | null {
 ...
 * aggregate.
 */
export function verifyResumePoint(
  token:          ResumeTokenV1,
  spec:           WorkflowSpec,
  workspaceRoot:  string,
): ResumeVerificationReport {
  const findings: TaskVerificationFinding[] = [];
  let alreadyDone        = 0;
  let diverged           = 0;
  let reconfirm          = 0;
  ...
  for (let i = 0; i < token.completed_task_count; i++) {
    ...
    let probe: CheckableProbe;
    switch (opType) {
      case "write_file":  probe = probeCreate(task, workspaceRoot);     break;
      case "delete_file": probe = probeDelete(task, workspaceRoot);     break;
      case "rename":      probe = probeRename(task, workspaceRoot);     break;
      case "create_dir":  probe = probeCreateDir(task, workspaceRoot);  break;
      default:
    ...
    if (probe.verdict === "already_done")              alreadyDone++;
    else if (probe.verdict === "task_state_diverged")  diverged++;
  }

  const aggregate: ResumeVerificationReport["aggregate"] =
    diverged > 0 ? "diverged" :
 ...
 * once.
 */
export function opTypeForTask(task: WorkflowTaskSpec): string {
  if (task.runtime === "claude") return "claude";
  if (task.structuredReplace !== undefined && (task.operationType === "replace" || task.operationType === undefined)) {
    return "replace";
  }
 ...
 */
export function idempotencyClassForTask(task: WorkflowTaskSpec): IdempotencyClass {
