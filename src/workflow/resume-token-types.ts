 ...
 */
export const RESUME_TOKEN_FORMAT_V1 = "usesteady.resume-token.v1" as const;
...
export type ResumeTokenV1 = {
  ...
  /**
   * Spec / workflow identity (`deriveWorkflowRunId`). Not the execution
   * instance. Same spec ⇒ same value.
   */
  readonly workflow_run_id:           string;
  /**
   * P-MED / F-02 — execution-instance identity. Token filename uses this
   * when present so run B cannot overwrite run A's file. Legacy tokens
   * omit it and keep using `workflow_run_id` as the filename key.
   */
 ...
 */
export type ResumeTokenRejectionCode =
  | "invalid_resume_token_format"
  | "invalid_resume_token"
  | "spec_hash_mismatch"
  | "task_count_mismatch"
...
export type ResumeTokenValidationResult =
  | { readonly ok: true; readonly token: ResumeTokenV1 }
  | { readonly ok: false; readonly code: ResumeTokenRejectionCode; readonly message: string };
