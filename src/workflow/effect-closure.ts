 ...
 */
export const CREATE_DIR_GENERIC_ANCESTOR_DISCLOSURE =
 ...
 */
export function formatAncestorDirs(ancestors: readonly string[]): string {
...
export function fsOpEffectDisclosure(op: FsChange): FsOpEffectDisclosure | undefined {
  if (op.operationType !== "create_dir") return undefined;
 ...
 */
function normalizeSegments(path: string): readonly string[] | null {
  const stack: string[] = [];
 ...
 */
function relativizeToScope(target: string, scopeRoot?: string): string | null {
  ...
  if (rootSegs === null || targetSegs === null) return null;
  if (targetSegs.length <= rootSegs.length) return null;
  for (let i = 0; i < rootSegs.length; i++) {
    if (rootSegs[i]!.toLowerCase() !== targetSegs[i]!.toLowerCase()) return null;
  }
