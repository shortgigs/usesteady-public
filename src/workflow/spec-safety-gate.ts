 ...
 */
function canonicalOpSafetyStrings(task: WorkflowTaskSpec): string[] {
  const out: string[] = [];
  const target = task.targetFiles?.[0];

  switch (task.operationType) {
    case "delete_file":  if (target) out.push(`delete ${target}`); break;
    case "create_dir":   if (target) out.push(`create folder ${target}`); break;
    case "write_file":   if (target) out.push(`create file ${target}`); break;
    case "append_file":  if (target) out.push(`append to ${target}`); break;
    case "prepend_file": if (target) out.push(`prepend to ${target}`); break;
    case "rename":
      if (target) out.push(`rename ${target} to ${task.newPath ?? ""}`.trim());
      break;
    case "replace":
      if (task.structuredReplace) out.push(`replace in ${task.structuredReplace.filePath}`);
      break;
    case "run_command":
  ...
  for (const f of task.targetFiles ?? []) out.push(f);
  if (typeof task.newPath === "string" && task.newPath.trim().length > 0) {
    out.push(task.newPath);
  }
  if (task.structuredReplace) out.push(task.structuredReplace.filePath);
 ...
 */
export function gateWorkflowSpecSafety(spec: WorkflowSpec): SpecSafetyVerdict {
  for (let i = 0; i < spec.tasks.length; i++) {
    const task = spec.tasks[i];
    if (!task) continue;

    const candidates: string[] = [task.input];
