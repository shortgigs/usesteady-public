...
export type NonGoalSegmentKind =
  | "evidence"
  | "example"
  | "constraint"
  | "context"
...
export class GoalIsolationDeclinedError extends Error {
  readonly code = "goal_isolation_declined" as const;
...
export function isGoalIsolationDeclinedError(
  err: unknown,
): err is GoalIsolationDeclinedError {
  return err instanceof GoalIsolationDeclinedError;
}

const GOAL_SENTENCE_PATTERNS: readonly RegExp[] = [
 ...
 */
const OPERATIVE_VERBS = new Set<string>([
  ...IMPERATIVE_VERBS,
...
const LABELLED_ASK_PATTERNS: readonly RegExp[] = [
  /^Objective:\s*(.+)/is,
  /^Goal:\s*(.+)/is,
...
type RawBlock = { text: string; start: number; end: number; blankBefore?: boolean };
...
function isGoalSentence(text: string): boolean {
...
function trimStagingManifest(text: string): string {
  let line = text.trim();
  const stageIdx = line.search(/\bStage only:/i);
  if (stageIdx > 0) line = line.slice(0, stageIdx).trim();
  const dashIdx = line.search(/\n-\s+(?:src|docs|tests|scripts|ui)\//);
  if (dashIdx > 0) line = line.slice(0, dashIdx).trim();
  return line;
}

function trimLabelledContent(text: string): string {
...
function extractLabelledAsk(text: string): string | null {
  const trimmed = text.trim();
  for (const pattern of LABELLED_ASK_PATTERNS) {
    const match = pattern.exec(trimmed);
    if (match?.[1]) return trimLabelledContent(match[1].trim());
    if (match?.[0]) return trimLabelledContent(match[0].trim());
  }
  if (/^OPEN\s+USESTEADY/i.test(trimmed)) return trimmed;
  if (/^Open:\s*USESTEADY/i.test(trimmed)) return trimLabelledContent(trimmed.replace(/^Open:\s*/i, "").trim());
  return null;
}

function extractImperativeAsk(text: string): string | null {
  let line = trimStagingManifest(stripAnswerPrefix(text));
  if (line.length === 0) return null;
  if (/^Stage only:/i.test(line)) return null;

  const first = firstToken(line);
  if (first && IMPERATIVE_VERBS.has(first.lower)) {
    return line.replace(/[.!]+$/, "").trim();
  }
  if (/^(?:push\s*\+\s*)?open\s+(?:PR|a\s+)/i.test(line)) return line.trim();
  if (/^Proceed with /i.test(line)) return line.trim();
  if (/^Run the prompt above now/i.test(line)) return line.trim();
  return null;
}
 ...
 */
function extractOperativeAsk(text: string): string | null {
  const line = trimStagingManifest(stripAnswerPrefix(text));
  if (line.length === 0) return null;
  if (/^Stage only:/i.test(line)) return null;
  if (isInterrogative(line)) return null;
...
function extractSectionAsk(text: string): string | null {
...
function isEvidenceBlock(text: string): boolean {
  const t = text.trim();
  if (/^<[^>]+>/.test(t) || /<\/?(?:title|meta|html|body|div|span|script|style)\b/i.test(t)) {
    return true;
  }
  if (/^(?:ERROR|WARN|INFO|Traceback|Exception|Stack trace)/i.test(t)) return true;
  if (/\bat\s+\S+\([^)]*\)/.test(t)) return true;
  if (/^\d{4}-\d{2}-\d{2}[T\s]/.test(t)) return true;
  if (/^(?:---|\*\*\*|===)/.test(t)) return true;
  return false;
}

function isContextBlock(text: string): boolean {
  return /^(?:I (?:copied|pasted|attached|shared|found)|Here is (?:our|the|my|a)|Below is|Attached (?:is|are)|For context)/i.test(
    text.trim(),
  );
}
...
function isInstructionBlock(text: string): boolean {
  const t = text.trim();
  return (
    /^(?:Explain your understanding|Ask questions before|Do not generate|Don't generate|Before acting|Do not (?:implement|execute|change))/i.test(t) ||
    /\bbefore (?:acting|you act|implementation)\b/i.test(t)
  );
}

function isExampleBlock(text: string): boolean {
  return /^(?:Here is an example|For example:|Example workflow:|Sample implementation:|Example:|Input:)/i.test(
    text.trim(),
  );
}

function isConstraintBlock(text: string): boolean {
  return /^(?:Do not|Don't|Must not|Without changing|No (?:runtime|governance|execution|portal))/i.test(
    text.trim(),
  );
}

function isStatusProse(text: string): boolean {
  const t = text.trim();
  ...
  if (/^#\s+P\d+/i.test(t)) return true;
  if (/^(?:Date Closed|Founder Approval|Status:\s*\*\*)/i.test(t)) return true;
  ...
  if (/\bPASS\b|\bFAIL\b|\bMERGED\b|\bCOMPLETE\b/i.test(t) && !extractImperativeAsk(t) && !extractLabelledAsk(t)) {
    if (/Layer|Status|cert|regression|fixture/i.test(t)) return true;
  }
  return false;
}

function isManifestLine(text: string): boolean {
  return /^-\s+(?:src|docs|tests|scripts|ui)\//.test(text.trim());
}

function isQuotedExampleLine(
  text: string,
  hasStrongerAsk: boolean,
): boolean {
  return hasStrongerAsk && isGoalSentence(text);
}

type BlockKind = NonGoalSegmentKind | "unknown";

function classifyNonAskBlock(text: string): BlockKind | "unknown" {
  if (isEvidenceBlock(text)) return "evidence";
  if (isContextBlock(text)) return "context";
  if (isInstructionBlock(text)) return "instruction";
  if (isExampleBlock(text)) return "example";
  if (isConstraintBlock(text)) return "constraint";
  if (isStatusProse(text)) return "context";
  if (isManifestLine(text)) return "instruction";
  return "unknown";
}

function splitRawInputBlocks(rawInput: string): readonly RawBlock[] {
...
const SECTION_BOUNDARY = new RegExp(
  `^(?:${SECTION_NAMES}):|^(?:${SECTION_NAMES})\\s*:?\\s*$`,
  "i",
);
...
function isLabelSpanBoundary(block: RawBlock): boolean {
  if (block.blankBefore) return true;
...
function isIllustrativeImperative(
  block: RawBlock,
  blocks: readonly RawBlock[],
  index: number,
): boolean {
  if (QUESTIONS_HEADER.test(block.text.trim())) return true;

  const prev = blocks[index - 1];
  const next = blocks[index + 1];
  const prevIsAdjacentFlow =
    !block.blankBefore && prev !== undefined && FLOW_LINE.test(prev.text.trim());
  const nextIsAdjacentFlow =
    next !== undefined && !next.blankBefore && FLOW_LINE.test(next.text.trim());
  return prevIsAdjacentFlow || nextIsAdjacentFlow;
}

function expandLabelledBlocks(blocks: readonly RawBlock[]): RawBlock[] {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (/^(?:Objective|Goal):?\s*$/i.test(block.text.trim())) {
      ...
      while (j < blocks.length) {
        const next = blocks[j]!;
        if (isLabelSpanBoundary(next)) break;
        parts.push(next.text);
        end = next.end;
        j++;
        if (parts.join(" ").length > LABEL_SPAN_MAX) break;
      }
      if (parts.length > 0) {
        expanded.push({
...
function collectAskCandidates(
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (isStatusProse(block.text) || isManifestLine(block.text)) continue;
...
function selectAskSpan(
  ...
  if (candidates.length > 0) {
    candidates.sort((a, b) => {
      ...
      if (a.priority >= 20 && a.index <= 1 && b.index > 1) return -1;
      if (b.priority >= 20 && b.index <= 1 && a.index > 1) return 1;
...
function isNoGoalMessage(rawInput: string, blocks: readonly RawBlock[]): boolean {
  if (
    /Closure Record/i.test(rawInput) &&
  ...
  if (/Decision:\s*approve\/merge\s+#\d+/i.test(rawInput)) {
    ...
    const buildable = asks.some(a =>
      /\b(?:implement|build|create|open|run|commit|authorize)\b/i.test(a.text),
    );
...
function classifyBulletKind(bullet: string): NonGoalSegmentKind {
  const trimmed = bullet.trim();
  if (GENERIC_REFLECTION_BULLETS.has(trimmed.toLowerCase())) return "context";
  const kind = classifyNonAskBlock(trimmed);
  if (kind === "unknown") {
    if (isEvidenceBlock(trimmed)) return "evidence";
    if (isGoalSentence(trimmed)) return "example";
    return "context";
  }
  return kind;
}

function segmentFromBullet(bullet: string, index: number): NonGoalSegment {
  const trimmed = bullet.trim();
  return {
...
function computeIsolationHash(goalText: string, segments: readonly NonGoalSegment[]): string {
  const payload = {
    goalText,
...
function blockIsSelectedAsk(
  block: RawBlock,
  askPick: { text: string; start: number; end: number },
): boolean {
 ...
 */
export function isolateIntentFromConfirmed(
  ...
  const nonGoalSegments: NonGoalSegment[] = [];
  const hasLabelledElsewhere = blocks.some(b => extractLabelledAsk(b.text) !== null);
  const hasSectionElsewhere = blocks.some(b => extractSectionAsk(b.text) !== null);
  const hasStrongerAsk = hasLabelledElsewhere || hasSectionElsewhere;

  for (const block of blocks) {
    if (blockIsSelectedAsk(block, askPick)) continue;
  ...
  const goalSourceRef: WorkPlanSourceRef = {
    ref: "rawInput",
    start: askPick.start,
    end: askPick.end,
  };
 ...
 */
export function isolateIntentFromRatifiedGoal(
  confirmed: ConfirmedUnderstandingV1,
): IsolatedIntent {
  ...
  if (rawInput.slice(ratified.start, ratified.end) !== ratified.text) {
    throw new GoalIsolationDeclinedError(
      "ratified goal is not a verbatim span of rawInput",
    );
  }
  ...
  for (const block of splitRawInputBlocks(rawInput)) {
    ...
    if (block.start < ratified.end && block.end > ratified.start) continue;
