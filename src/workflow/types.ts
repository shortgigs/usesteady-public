...
import type { EffectRefusalCode } from "./effect-decision.js";
...
export type WorkflowRunPhase =
  | "reviewing"      // spec loaded; H reviews task list before any session is created (Phase 11D)
  | "idle"           // reviewing confirmed; advanceWorkflow() launches the first task (internal)
  | "running"        // between tasks; advanceWorkflow() needed
  | "task_ready"     // current task prepared; awaiting H confirmation
  | "task_conflict"  // current task has OCD conflict; H must acceptConflict first
  | "task_approved"  // current task approved by H; ready to deliver
  | "task_scope"     // current task awaiting scope clarification from H
  | "task_advisory"  // model emitted structured advisory position(s); parked pending H supersession decision (P3 Phase 2)
  | "task_failed"    // current task reached non-accepted terminal; H must act
  | "completed"      // all tasks done (accepted or skipped); terminal
  | "stopped";       // H stopped or max-retry exceeded; terminal
...
export type WorkflowCurrentSession =
  | { readonly runtime: "cursor"; readonly state: CursorSessionState }
  | { readonly runtime: "claude"; readonly state: ClaudeSessionState };
...
export type WorkflowRun = {
   ...
   * same specification. Not an execution instance. P-MED / F-02.
   */
  readonly workflowRunId:   string;
  /**
   * Immutable execution-instance identity, minted at createWorkflowRun.
   * Same spec, two executions ⇒ two ids. Resume of A stays bound to A.
   * Optional only for pre-slice / test-constructed runs; production
   * createWorkflowRun always sets it.
   */
...
export type WorkflowTaskOutcome =
  ...
  | "accepted"          // delivery accepted by the runtime
  | "skipped"           // H explicitly skipped (failure action)
  | "skipped_by_intake" // intake mode !== "execute"; auto-skipped by coordinator
  | "planning_reviewed" // planning-review route completed (non-execution)
  | "stopped"           // workflow stopped at this task
  | "rejected";         // H explicitly rejected the task's session

// ─── Display ──────────────────────────────────────────────────────────────────

export type WorkflowDisplayErrorCode =
  | CanonicalErrorCode
  | "spec_hash_mismatch"
...
export type WorkflowDisplay = {
  ...
  readonly progress:     string;  // e.g., "Task 2 of 5"
  readonly currentLabel?: string;
  readonly taskSummary?: readonly WorkflowTaskSummaryLine[];
  readonly failureNote?: string;  // set when phase === "task_failed"
  /**
   ...
   */
  readonly errorCode?: WorkflowDisplayErrorCode;
};
...
export type FsPlugin = {
  executeFsOp(op: FsChange): Promise<{
    ...
    /**
     * P-EFFECT-RESULT — sha256 of the bytes actually present after a
     * successful create (`write_file`). Omitted on other ops and on
     * failure. Must be the read-back hash, not a pre-write representation.
     */
