 ...
 */
function extractFrontmatterAndBody(
  content: string,
  sourcePath: string,
): { readonly yaml: string; readonly body: string } {
  // Strip optional UTF-8 BOM so editors that add it don't break the parser.
  const text = content.startsWith('\uFEFF') ? content.slice(1) : content;

  if (!text.startsWith('---')) {
    throw new ValidationError(
      `${sourcePath}: missing frontmatter opening delimiter`,
      [
        {
  ...
  if (closingMatch === null) {
    throw new ValidationError(
      `${sourcePath}: missing frontmatter closing delimiter`,
      [
        {
          type: 'invalid_markdown',
          message:
            'No closing --- delimiter found; the frontmatter block is not terminated.',
        },
      ],
    );
  }
...
function parseScalar(raw: string): boolean | number | string {
  ...
  if (raw !== '' && !Number.isNaN(asNumber)) return asNumber;
...
// Internal parser state — tracks whether the current context is a sequence.
type ParserState =
  | { readonly mode: 'idle' }
  | { readonly mode: 'array'; readonly key: string };
...
function parseMinimalYaml(
  ...
  let state: ParserState = { mode: 'idle' };
  ...
  if (issues.length > 0) {
    throw new ValidationError(
      `${sourcePath}: YAML parse failed with ${issues.length} issue(s)`,
      issues,
    );
  }
...
// ---------------------------------------------------------------------------
// Step 3 — markdown section extractor
// ---------------------------------------------------------------------------

/** Internal representation of a heading found in the body. */
interface HeadingInfo {
  /** Number of leading `#` characters — 1 for `#`, 2 for `##`, etc. */
  readonly level: number;
  /** Heading text, lowercased and trimmed, for case-insensitive lookup. */
  readonly normText: string;
  /** Index in `body` of the first character of this heading line. */
  readonly lineStart: number;
  /** Index in `body` of the first character *after* this heading line. */
...
function extractSections(body: string): ParsedSections {
   ...
   */
  function getSection(normalisedName: string): string | undefined {
    const idx = headings.findIndex((h) => h.normText === normalisedName);
    if (idx === -1) return undefined;

    const heading = headings[idx];
    if (heading === undefined) return undefined; // noUncheckedIndexedAccess guard
...
export async function parseSkillFile(filePath: string): Promise<ParsedSkillFile> {
  ...
  const frontmatter: unknown = parseMinimalYaml(yaml, filePath);
