...
import type { LoadedSkillRegistry, RecoverySuggestionsOutput } from "./types.js";
...
export async function concreteSuggestions(
  rawInput: string,
  deps: SuggestionDeps,
): Promise<readonly { input: string; reason: string }[]> {
  const trimmed = rawInput.trim();
  if (!trimmed) return [];
  const trigger = { kind: "recovery" as const, failureReason: "unsupported" as const, rawInput: trimmed };
  if (!shouldConsultSkills(deps.registry, trigger)) return [];
  ...
  try {
    ...
    return result.accepted.flatMap((output) =>
      output.schema === "usesteady.recovery_suggestions.v1"
        ? (output as RecoverySuggestionsOutput).suggestions.map((s) => ({
            input:  s.input,
            reason: s.reason,
          }))
        : [],
