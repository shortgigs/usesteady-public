...
import {
  ...
  mkdirSync, renameSync, unlinkSync, lstatSync,
} from "node:fs";
...
import {
  ...
  validateReplacementTextSafety,
  validateProtectedPath,
  classifyNormalizationDivergence,
  describeNormalizationDivergence,
...
import type { ApprovedEffectiveBind } from "../../understand/interpretation/types.js";
import type { EffectRefusalCode } from "../../workflow/effect-decision.js";
...
import {
  ...
  type EffectReceiverMode,
} from "../../input/effective-resource.js";
import {
  applyAuthorizedCreate,
  authorizedCreateUtf8,
} from "../../input/effect-result.js";
 ...
 */
function safeCanonicalize(raw: string): string | null {
  try {
 ...
 * obfuscation).
 */
function pathsEqualByCanonical(a: string, b: string): boolean {
  if (a === b) {
 ...
 */
function matchesGlobOnBothViews(path: string, pattern: string): boolean {
...
export class CursorInProcessAdapter implements CursorEditorPlugin, FsPlugin {
  /**
  ...
  async receive(request: CursorDeliveryRequest): Promise<CursorResponse> {
    ...
    if (parsedChange.filePath) {
      ...
      targetFile = (result as { kind: "single_match"; file: string }).file;
    } else {
    ...
    const resolvedFile = targetFile as string;
    ...
    if (scopeConstraint.allowedFiles.length > 0) {
      const allowed = scopeConstraint.allowedFiles.some(
        (entry) => pathsEqualByCanonical(entry, resolvedFile),
      );
      if (!allowed) {
        return {
          kind:       "refused_due_to_execution_error",
    ...
    const effective = resolveEffectiveResource(this.workspaceRoot, resolvedFile);
    if (!effective.ok) {
      return {
        kind:       "refused_due_to_execution_error",
        deliveryId,
        artifactId: artifact.id,
        receivedAt: Date.now(),
        errorCode:  effective.reason === "unresolved"
          ? "effective_resource_unresolved"
          : effective.reason === "protected_resource"
            ? "prohibited_path"
            : "outside_workspace",
        detail:     effective.message,
      };
    }
    ...
    if (originalOccurrences !== canonicalOccurrences) {
      ...
      const detail =
        describeNormalizationDivergence(cause, oldValue, resolvedFile) +
    ...
    try {
      const refused = this.effectiveResourceRefusal(resolvedFile, "file", parsedChange.approvedEffective, "mutate");
      if (refused !== null) {
        return {
          kind: "refused_due_to_execution_error", deliveryId, artifactId: artifact.id,
          receivedAt: Date.now(), errorCode: refused.errorCode, detail: refused.detail,
        };
      }
      // Correspondence only — not an fd/O_NOFOLLOW lock. The write still
      // uses the lexical workspace join. A hop after this check remains
      // possible; do not treat this as atomic filesystem authority.
  ...
  private containmentRefusal(
    rawPath: string,
    fieldName: string,
  ): { readonly kind: "failed"; readonly errorCode: "outside_workspace"; readonly detail: string } | null {
    if (!pathEscapesWorkspace(rawPath)) return null;
    return {
      kind:      "failed",
      errorCode: "outside_workspace",
  ...
  /**
   * P-F01 — effective-receiver check before any write.
   * Lexical containment is checked first (no fs). Then existing prefixes
   * are walked. Alias hops outside the workspace are `outside_workspace`.
   * Unreadable/cyclic hops are `effective_resource_unresolved`.
   * When an approval-bound abs is stamped, execution must resolve to
   * that same receiver — a new contained hop is `effective_resource_changed`.
   *
   * This is a refusal check only. The caller still joins the supplied
   * path so a final-segment file symlink keeps the existing
   * `merge_conflict` "refusing to edit symlink target" behavior.
   *
   * `mode` distinguishes existing-object mutations from authorized
   * absent-create. Neither mode claims the check-to-system-call interval.
   */
  private effectiveResourceRefusal(
    rawPath: string,
    fieldName: string,
    approved?: ApprovedEffectiveBind,
    mode: EffectReceiverMode = "create",
  ): {
    readonly kind: "failed";
    readonly errorCode: "outside_workspace" | "prohibited_path" | "effective_resource_unresolved" | "effective_resource_changed";
    readonly detail: string;
  } | null {
    const lexical = this.containmentRefusal(rawPath, fieldName);
    if (lexical !== null) return lexical;
    const correspondence = checkEffectReceiverIdentity(
      this.workspaceRoot,
      rawPath,
      approved,
      mode,
    ...
    if (correspondence.code === "effective_resource_changed") {
      return {
        kind: "failed",
        errorCode: "effective_resource_changed",
        detail: correspondence.message,
      };
    }
    if (correspondence.code === "effective_resource_unresolved") {
      return {
        kind: "failed",
        errorCode: "effective_resource_unresolved",
        detail: mode === "mutate" && approved === undefined
          ? correspondence.message
          : `${correspondence.message} (${fieldName}: ${rawPath})`,
      };
    }
    if (correspondence.code === "prohibited_path") {
      return {
        kind: "failed",
  ...
  private filenameSafetyRefusal(
    ...
    readonly kind: "failed";
    readonly errorCode: "invalid_filename_chars" | "invalid_filename_reserved_name";
    readonly detail: string;
  } | null {
    ...
    const errorCode: "invalid_filename_chars" | "invalid_filename_reserved_name" =
      result.code === "invalid_filename_reserved_name"
  ...
  private protectedPathRefusal(
    ...
    readonly kind: "failed";
    readonly errorCode: "prohibited_path";
    readonly detail: string;
  } | null {
  ...
  async executeFsOp(op: FsChange): Promise<{
    ...
    try {
      switch (op.operationType) {
        case "create_dir": {
          const refused = this.effectiveResourceRefusal(op.dirPath, "path", op.approvedEffective);
          if (refused !== null) return refused;
          const filenameRefused = this.filenameSafetyRefusal(op.dirPath, "path");
          if (filenameRefused !== null) return filenameRefused;
          const protectedRefused = this.protectedPathRefusal(op.dirPath, "path");
          if (protectedRefused !== null) return protectedRefused;
        ...
        case "write_file": {
          const refused = this.effectiveResourceRefusal(op.filePath, "file", op.approvedEffective);
          if (refused !== null) return refused;
          ...
          const authorized = authorizedCreateUtf8(op);
          if (!authorized.ok) {
            return {
              kind:      "failed",
              errorCode: "effect_result_mismatch",
              detail:    authorized.detail,
            };
          }
          mkdirSync(dirname(abs), { recursive: true });
          const readAbs = op.approvedEffective?.abs ?? abs;
          const corresponded = applyAuthorizedCreate({
            abs,
            readAbs,
            authorizedUtf8: authorized.utf8,
            hooks: {
              write: (target, utf8) => { writeFileSync(target, utf8, "utf8"); },
          ...
          if (!corresponded.ok) {
            return {
              kind:      "failed",
              errorCode: "effect_result_mismatch",
              detail:    corresponded.detail,
            };
          }
          return {
            kind: "accepted",
            resultingContentSha256: corresponded.actualSha256,
          };
        }
        case "append_file": {
          const protectedRefused = this.protectedPathRefusal(op.filePath, "file");
          if (protectedRefused !== null) return protectedRefused;
          const refused = this.effectiveResourceRefusal(op.filePath, "file", op.approvedEffective, "mutate");
          if (refused !== null) return refused;
          const abs = isAbsolute(op.filePath) ? op.filePath : join(this.workspaceRoot, op.filePath);
          if (this.isSymlinkPath(abs)) {
          ...
          if (existsSync(abs)) {
            ...
            if (!read.ok) return { kind: "failed", detail: read.detail };
            current = read.text;
          }
          mkdirSync(dirname(abs), { recursive: true });
          const finalRefusal = this.effectiveResourceRefusal(op.filePath, "file", op.approvedEffective, "mutate");
          if (finalRefusal !== null) return finalRefusal;
        ...
        case "rename": {
          ...
          if (protectedRefusedFrom !== null) return protectedRefusedFrom;
          const protectedRefusedTo = this.protectedPathRefusal(op.newPath, "to");
          if (protectedRefusedTo !== null) return protectedRefusedTo;
          const lexicalFrom = this.containmentRefusal(op.filePath, "from");
          if (lexicalFrom !== null) return lexicalFrom;
          const lexicalTo = this.containmentRefusal(op.newPath, "to");
          if (lexicalTo !== null) return lexicalTo;
          const refusedFrom = this.effectiveResourceRefusal(op.filePath, "from", op.approvedEffectiveFrom, "mutate");
          if (refusedFrom !== null) return refusedFrom;
          const refusedTo = this.effectiveResourceRefusal(op.newPath, "to", op.approvedEffectiveTo, "create");
          if (refusedTo !== null) return refusedTo;
          ...
          if (filenameRefusedTo !== null) return filenameRefusedTo;
          ...
          const finalFrom = this.effectiveResourceRefusal(op.filePath, "from", op.approvedEffectiveFrom, "mutate");
          if (finalFrom !== null) return finalFrom;
          const finalTo = this.effectiveResourceRefusal(op.newPath, "to", op.approvedEffectiveTo, "create");
          if (finalTo !== null) return finalTo;
        ...
        case "run_command": {
          // R0-B / #1086 — repeat the exact proposal-time command policy
          // immediately before process creation. This is defense in depth:
          // callers that bypass FeasibilityValidator still cannot regain
          // shell authority at the adapter boundary.
          const policy = parseAuthorizedRunCommand(op.command);
          if (!policy.ok) {
            return {
              kind: "failed",
          ...
          const exec = spawnSync(policy.executable, [...policy.args], {
  ...
  private searchForOldValue(
    ...
    for (const file of allowedFiles) {
      ...
      if (canonOldValue === null) continue;

      const canonContent = safeCanonicalize(content);
      if (canonContent === null) continue;
      if (!canonContent.includes(canonOldValue)) continue;

      matches.push(file);
    }

    if (matches.length === 0) return { kind: "not_found" };
    if (matches.length === 1) return { kind: "single_match", file: matches[0]! };
    return { kind: "multiple_matches", files: matches };
  }

  private isSymlinkPath(path: string): boolean {
    try {
  ...
  private readUtf8TextStrict(path: string):
    | { ok: true; text: string }
    | { ok: false; error: "missing" | "binary" | "read_failed"; detail: string } {
    if (!existsSync(path)) {
...
type SearchResult =
  | { kind: "not_found" }
  | { kind: "single_match"; file: string }
  | { kind: "multiple_matches"; files: string[] };
