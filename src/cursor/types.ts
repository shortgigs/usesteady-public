...
export type CursorOCDClearanceStatus =
  | "cleared"           // No policy conflicts. Proceeds to ready_for_confirmation.
  | "conflict_detected" // Policy fired. H must override or reject.
  | "conflict_accepted"; // H explicitly accepted. Proceeds to ready_for_confirmation.
...
// ─── OCD policy (inputs for the OCD evaluator) ────────────────────────────────
 ...
 */
export type CursorScopeQuestionKind =
  | "need_file_path"       // filePath absent; Cursor found candidates by searching for oldValue
  | "need_scope_selection" // filePath present; oldValue in multiple named scopes within the file
  | "ambiguous_old_value"; // oldValue appears multiple times in the same scope
...
export type CursorExecutionErrorCode =
  | "old_value_not_found"     // oldValue not present in the target file
  | "ambiguous_match"         // oldValue appears more than once; cannot safely target
  | "file_not_found"          // target file does not exist or cannot be read
  | "scope_outside_allowed"   // resolved target is not in allowedFiles (when non-empty)
  | "prohibited_pattern_match" // resolved target matches a prohibitedPattern
  | "parse_error"             // parsedChange absent or malformed; no edit directive
  | "merge_conflict"          // filesystem conflict during write
  | "outside_workspace"       // S2 / #36 defense-in-depth: resolved abs path is not under workspaceRoot
  | "invalid_replacement_chars" // Cluster B Iter 2 / public#52: replace `newValue` contains BIDI / zero-width / control codepoints
  | "prohibited_path"         // effective or supplied receiver is a protected path (e.g. .git internals)
  | "effective_resource_unresolved" // Existing fail-closed effective receiver refusal
  | "effective_resource_changed" // Approval-bound receiver no longer matches at effect time
  | "delivery_timeout";       // plugin did not respond within the delivery timeout

// ─── Workspace validation result (for H-provided paths) ───────────────────────

export type WorkspacePathValidationResult =
  | { readonly ok: true }
  | { readonly ok: false; readonly reason: "outside_workspace_boundary" | "invalid_path" };
