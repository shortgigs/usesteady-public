 ...
 */
export function narrowArtifactScope(
  artifact:        CursorHandoffArtifact,
  newAllowedFiles: readonly string[],
  newAllowedScopes?: readonly string[],
): CursorHandoffArtifact {
  const newScopeConstraint: CursorScopeConstraint = {
...
/** Attach a pre-approval observation and include it in the artifact's content address. */
export function bindArtifactEffectiveReceiver(
  artifact: CursorHandoffArtifact,
  bind: ApprovedEffectiveBind,
  filePath?: string,
): CursorHandoffArtifact {
  const parsedChange = artifact.changeSpec.parsedChange;
  if (parsedChange === undefined || artifact.eligibility === "approved_for_cursor") return artifact;
  const changeSpec = {
    ...artifact.changeSpec,
    parsedChange: {
      ...parsedChange,
  ...
  const id = hashObject({
    intentId: artifact.intentId, mode: "execute", changeSpec,
    responseId: artifact.responseId, scopeConstraint: artifact.scopeConstraint,
  });
  const { approvedAt: _approvedAt, ...pending } = artifact;
  return { ...pending, id, changeSpec, eligibility: "pending_confirmation" };
}

function buildChangeSpec(intake: IntakeResult, parsedChange?: ParsedChange): CursorChangeSpec {
...
function buildChangeSpecFromInterpretation(
...
function buildChangeSpecFromIntentInterpretation(
...
function buildInitialOCDClearance(): CursorOCDClearance {
