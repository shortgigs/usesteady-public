...
export type RealityProbe = (
  execution: ExecutionPayload,
) => RealityProbeResult | Promise<RealityProbeResult>;
 ...
 */
function buildWhatHappened(execution: ExecutionPayload): string {
 ...
 */
export function makeObservationPort(
  realityProbe: RealityProbe | null,
): (ctx: BaseStageContext) => Promise<DecisionSection<ObservationPayload>> {
  return async (ctx: BaseStageContext): Promise<DecisionSection<ObservationPayload>> => {
    try {
      ...
      const intendedVsActual: "match" | "deviation" = executionValue.ranWhatWasApproved
        ? "match"
        : "deviation";
      ...
      const realityVerdict: "agree" | "disagree" | "unknown" =
        probeResult === null ? "unknown" : probeResult.realityVerdict;
      const realityDetail =
        probeResult !== null && typeof probeResult.detail === "string"
          ? probeResult.detail
          : undefined;
      ...
      const scopeObservation =
        probeResult !== null && probeResult.scopeObservation !== undefined
 ...
 */
export const agreeingRealityProbe: RealityProbe = () => ({ realityVerdict: "agree" });
