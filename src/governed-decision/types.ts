...
export type DecisionSection<T> =
  | { readonly status: "connected"; readonly value: T }
  | { readonly status: "derived"; readonly value: T; readonly derivedFrom: readonly StageName[] }
  | { readonly status: "unavailable"; readonly reason: string };

// --- Stage payloads (minimal v1 shapes; fleshed out by Phase C fidelity slices) ---
...
// --- Explicit Invocation Lineage v1 (charter:
//     docs/architecture/USESTEADY_EXPLICIT_INVOCATION_LINEAGE_V1.md) ---
//
// A reference is a DECLARED dependency on prior/external truth. It is never
// inherited authority: it enters as a claim whose authority is re-certified in
// THIS invocation (INV-EIL-3). Its entry authority band is a function of KIND and
// whether the kernel could RE-VERIFY it in the current invocation (INV-EIL-5) --
// never of the reference's past or a caller's self-assertion.

export type ReferenceKind =
  | "prior_observation" // a prior cycle's observation (the existing --from loop)
  | "certified_decision" // a prior FINAL, human-ratified GovernedDecisionRecord
  | "verified_artifact" // an artifact re-verifiable in THIS invocation
  | "asserted_artifact"; // an externally supplied artifact, not re-verified
 ...
 */
export function referenceAuthorityBand(
  kind: ReferenceKind,
  verifiedInThisInvocation: boolean,
): AuthorityLevel {
  switch (kind) {
