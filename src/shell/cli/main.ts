...
import {
  buildRequiredProviderIds,
  getMissingProviders,
  isProviderLive,
}                                           from "../../utils/credentials.js";
import { isTerminal as isCursorTerminal }
  from "../../product/cursor-product-session.js";
import { isClaudeTerminal }
...
import { CursorInProcessAdapter }
  from "../../cursor/adapters/inprocess-adapter.js";
import { ClaudeStubAdapter }
  from "../../claude/adapters/stub-adapter.js";
import { ClaudeApiAdapter }
  from "../../claude/adapters/api-adapter.js";
import { OpenAiCompatibleAgentAdapter }
  from "../../claude/adapters/openai-compatible-adapter.js";
import {
  renderClaudeFrame,
  renderWorkflowFrame,
  renderHistoryFrame,
...
import type { ShellPrompt } from "../types.js";
import {
  createWorkflowRun,
  advanceWorkflow,
  applyFailureAction,
...
import {
  loadTeamPolicyPack,
  renderTeamPolicyRunBannerLines,
...
import {
  ...
  primaryTaskInput,
} from "../../portal-bridge/execution-return/index.js";
import {
  captureCertificationChainSnapshot,
  resolveReportUcpRootId,
} from "../../portal-bridge/execution-return/resolve-report-ucp-root.js";
import {
  postCoreUnderstandWorkflow,
  USESTEADY_CORE_URL_ENV,
} from "../../portal-bridge/post-core-understand-workflow.js";
import {
  resolvePortalApprovals,
  resolveRemoteApprovalDecision,
} from "../../portal-bridge/pending-approval/index.js";
import {
  createBridgeApprovalProvenanceState,
  recordBridgeApprovalDecision,
  resolveExecutionReturnApprovalProvenance,
} from "../../portal-bridge/approval-provenance/index.js";
...
import { finalizeErrorCode, type CanonicalErrorCode } from "../../kernel/error-codes.js";
import type { WorkflowSpec } from "../../workflow/types.js";
import type { WorkflowPlugins } from "../../workflow/types.js";
...
import type { TuiConsensusEntry } from "../tui/types.js";
import {
  classifyWorkflowTerminalOutcome,
  parseConfirmInput,
  type WorkflowTerminalRecord,
} from "./workflow-terminal.js";
import {
  buildExecutionSummary,
  renderExecutionSummaryLines,
...
// S3 / friction #40 — Input Surface Integrity for `run <spec.json>`.
// Auto-wrap `--json`/`batch`-shape JSON (single op or op array) into a
// WorkflowSpec so all three JSON-accepting input surfaces accept the
// same JSON op shapes. Imports are pure; no I/O at module-eval time.
import { jsonOpToIROperation } from "../../input/json-to-ir.js";
import { buildSpecTasksFromIROps } from "./spec-task-builder.js";
// S4 / friction #44 — WorkflowSpec NL synthesis. See spec-nl-synth.ts module
// JSDoc for the rationale; called from `loadWorkflowSpecFromFile` below to
// route `tasks[].input` through `normalizeNLToIR` at parse-stage so the
// coordinator never has to fall back to a separate NL parser.
...
function createController(autoYes: boolean): SessionController {
...
function ask(rl: SessionController, question: string): Promise<string> {
...
function sanitizeForConsole(line: string): string {
  let out = line;
...
function print(lines: readonly string[]): void {
  if (process.env["USESTEADY_QUIET"] === "true") return;
...
function printBanner(
  runtime: "cursor" | "claude" | "workflow" | "history" | "onboarding",
  autoYes: boolean = false,
): void {
  if (runtime === "onboarding") return;   // onboarding prints its own header
  ...
  // entrypoint.
  if ((runtime === "workflow" || runtime === "history") && autoYes) return;
...
function parseChoice(answer: string): number {
  return parseInt(answer.trim(), 10);
}

// ??? Cursor run loop ??????????????????????????????????????????????????????????

async function runCursorLoop(
  rl:                   SessionController,
  workspaceRoot:        string,
  firstInput?:          string,
  onboardingSessionId?: string,    // present when coming from guided onboarding
): Promise<void> {
  ...
  const plugin   = makeCursorBoundaryObserver(
    new CursorInProcessAdapter(workspaceRoot),
    makeBoundaryJsonlSink(join(storeDir, "boundary-observations.jsonl")),
  );

  let pendingInput:          string | undefined  = firstInput;
...
function buildClaudePlugin(): import("../../claude/delivery-gate.js").ClaudeAgentPlugin {
  ...
  if (
    process.env["USESTEADY_USE_LLM"] === "true" ||
...
async function runClaudeLoop(rl: SessionController): Promise<void> {
  ...
  const plugin   = makeClaudeBoundaryObserver(
    buildClaudePlugin(),
    makeBoundaryJsonlSink(join(storeDir, "boundary-observations.jsonl")),
  );
  ...
  if (missingDemo.length > 0) {
    print([
      `  ℹ Using demo mode (no API key configured for ${
        missingDemo.map(p => p.displayName).join(", ")
      }). Set ${
        missingDemo.map(p => {
  ...
  while (true) {
    ...
    session     = submitClaude(
      session, input, DEFAULT_CLAUDE_OCD_POLICY, DEFAULT_CLAUDE_TOOL_POLICY,
    );
...
async function handleClaudePrompt(
  rl:       SessionController,
  session:  ReturnType<typeof createClaudeSession>,
  prompt:   ShellPrompt,
  plugin:   import("../../claude/delivery-gate.js").ClaudeAgentPlugin,
  storeDir: string,
): Promise<ReturnType<typeof createClaudeSession>> {
  if (prompt.kind === "confirm") {
    const answer = await ask(rl, `  ${prompt.question} `);
 ...
 */
function countAuditLines(auditFile: string): number {
  try {
 ...
 */
function readAuditLinesSince(auditFile: string, offsetLines: number): AuditEntry[] {
  try {
    const content = fs.readFileSync(auditFile, "utf8");
    ...
    for (const line of newLines) {
      try {
        const stripped = line.startsWith("[consensus-audit] ")
          ...
          : line;
        const parsed = JSON.parse(stripped) as AuditEntry;
 ...
 */
function noQuorumReason(rounds: AuditEntry["rounds"]): string {
 ...
 */
function mapDisposition(raw: string | undefined): TuiConsensusEntry["disposition"] {
  switch (raw) {
 ...
 */
function mapPluginLabel(pluginName: string, isPrimary: boolean): string {
  if (pluginName === "claude")             return "Claude";
  if (pluginName === "openai-compatible")  return "Secondary";
  if (isPrimary)                           return pluginName.charAt(0).toUpperCase() + pluginName.slice(1);
  return "Secondary";
}
 ...
 */
function deriveConsensusEntries(
  auditFile:     string,
  offsetBefore:  number,
): readonly TuiConsensusEntry[] {
  ...
  const perPlugin = new Map<string, { disposition?: string }>();
  ...
  const result: TuiConsensusEntry[] = [];
 ...
 */
function printConsensusExplanation(auditFile: string, auditOffsetBefore: number): void {
...
function persistWorkflowTerminalOutcome(record: WorkflowTerminalRecord): void {
 ...
 */
async function buildWorkflowSpecInteractively(rl: SessionController): Promise<WorkflowSpec | null> {
  ...
  while (true) {
    ...
    tasks.push({
      ...
      ...(runtimeRaw === "claude" ? { runtime: "claude" as const } : {}),
    });
    taskNum++;
  }
 ...
 * remaining task still goes through the existing approval flow.
 *
 ...
 *     refuse. Opt-in acknowledges unverified prior completion and advances
 *     past those tasks; it does not reexecute the completed operations.
 */
...
async function runWorkflowLoop(
  ...
  workspaceRoot:   string,
  autoYes:         boolean = false,
  executionPolicy: ExecutionPolicy = "interactive",
  resumeOptions:   RunWorkflowResumeOptions = {},
  reportToPortal:  boolean = false,
  pendingApprovalBridge: boolean = false,
  ...
  const plugins: WorkflowPlugins = {
    ...
    fs:     makeFsBoundaryObserver(
      cursorAdapter,
      makeJsonlBoundarySink(join(storeDir, "fs-boundary-observations.jsonl")),
    ),
  ...
  let lastRecord: WorkflowTerminalRecord = {
  ...
  while (true) {
    ...
    if (specPath) {
      try {
        ...
        const specLoadCode: CanonicalErrorCode =
          err instanceof WorkflowSpecLoadError ? err.code : "invalid_op";
        print([`  Error: ${message}`]);
        if (autoYes) {
    ...
    // both surfaces consume. Terminal and fail-closed: no retry, exit 1.
    {
      ...
      if (verdict.verdict === "block") {
        ...
        if (surface) print([...renderSurfaceCli(surface)]);
    ...
    const requiredProviders = buildRequiredProviderIds(
      spec.tasks.map(t => t.runtime ?? spec.defaultRuntime ?? "cursor"),
    );
    ...
    if (missingLive.length > 0) {
      print([
        "",
        `  ✗ Workflow cannot start — missing API key for ${
          missingLive.length === 1 ? missingLive[0]!.displayName : `${missingLive.length} providers`
        }.`,
        "",
    ...
    if (missingDemoOnly.length > 0) {
      ...
      print([
        "",
        `  ℹ Using demo mode (no API key configured for ${
          missingDemoOnly.map(p => p.displayName).join(", ")
        }).`,
    ...
    const workflowPolicies = {
      ...DEFAULT_WORKFLOW_POLICIES,
      cursorPolicy: {
        ...DEFAULT_WORKFLOW_POLICIES.cursorPolicy,
        workspaceRoot,
      },
      workspaceRoot,
    };
    ...
    if (resumeOptions.resumeFromPath !== undefined) {
      const validation = validateTokenForResume(
        resumeOptions.resumeFromPath,
        spec,
        workspaceRoot,
      );
      if (!validation.ok) {
      ...
      if (report.aggregate === "needs_reconfirm" && !resumeOptions.reexecuteNonIdempotent) {
        print([
          ...
          `    The workflow includes non-idempotent ops (append/prepend/replace/` +
            `run/claude).`,
          `    Pass --reexecute-non-idempotent to acknowledge unverified prior ` +
            `completion and advance past those tasks without reexecuting them.`,
      ...
      // Continue A's execution instance. Do not mint a new id (P-MED / F-02).
      run = createWorkflowRun(spec, {
        executionInstanceId:
          validation.token.execution_instance_id ?? validation.token.workflow_run_id,
      });
      ...
      const newTasks = run.tasks.map((t, i) => {
        if (i >= completedCount) return t;
        const finding = report.findings[i];
        const outcome =
          finding?.verdict === "previously_skipped" ? ("skipped" as const) :
          finding?.verdict === "already_done"       ? ("accepted" as const) :
      ...
      print([
        ...
        `    Completed:  ${completedCount}/${spec.tasks.length} (per token; structural checks do not verify bytes)`,
        `    Unverified prior completions acknowledged: ${report.reconfirm}`,
        `    Remaining:  ${spec.tasks.length - completedCount} task(s)`,
        "",
      ]);
    }

    if (tuiEnabled) setTuiState(run);
    ...
    let capturedFailureNote: string | null = null;
    ...
    const writeTokenIfAdvanced = (currentRun: typeof run): void => {
      const completedNow = currentRun.tasks.filter(t =>
        t.outcome === "accepted" || t.outcome === "skipped" || t.outcome === "skipped_by_intake",
      ).length;
      if (completedNow === lastWrittenCompletedCount) return;
      if (completedNow === 0) return;
      try {
        const token = buildResumeToken({
          spec,
          workflowRunId:         currentRun.workflowRunId,
          ...(currentRun.executionInstanceId !== undefined ? { executionInstanceId: currentRun.executionInstanceId } : {}),
    ...
    while (!isWorkflowTerminal(run)) {
      ...
      if (executionPolicy === "batch_fail_fast") {
        ...
        const leakIdx = run.tasks.findIndex(t =>
        ...
        if (leakIdx !== -1 && run.phase !== "stopped") {
          ...
          if (!capturedFailureNote) capturedFailureNote = explicitFailureReason;
          run = { ...run, phase: "stopped" as const };
          if (tuiEnabled) setTuiState(run);
          break;
        }
      }
      ...
      if (!frame.prompt) {
        ...
        if (run.phase === "running") {
          run = bindEffectiveResourceOnReady(
            advanceWorkflow(
              run,
              workflowPolicies.cursorPolicy,
              workflowPolicies.claudeOCDPolicy,
              workflowPolicies.claudeToolPolicy,
            ),
            workspaceRoot,
          );
          if (tuiEnabled) setTuiState(run);
      ...
      if (frame.prompt.kind === "confirm") {
        ...
        if (approvalsConfig.enabled && !autoYes && run.mode !== "break_glass") {
          ...
          if (remote !== null) {
            ...
            if (remote.decision === "yes") {
              bridgeApprovalProvenance = recordBridgeApprovalDecision(
              ...
              remote.decision === "yes" &&
                ...
                ? {
                    ...
                    remote.authority.assertion.schema ===
                      ...
                      ? {
                          mappedRelations:
                            remote.authority.assertion.payload.decision_origin.retirement_basis_relations.map(
                              (relation) => ({
                                modelPositionId:
        ...
        const n = parseChoice(answer);
        if (isNaN(n)) {
          if (!autoYes) print(["  Invalid selection. Enter a number or exit."]);
          continue;
        }
    ...
    const _errorCode: string | null = finalizeErrorCode({
      capturedErrorCode,
      capturedFailureNote,
    ...
    // crashes the run.
    let _kernelArtifact: ReturnType<typeof buildKernelArtifact> | null = null;
    ...
    if (reportToPortal) {
      try {
        ...
        const approvalProvenance = resolveExecutionReturnApprovalProvenance(
          bridgeApprovalProvenance,
          storeDir,
          run.workflowRunId,
        );
        await reportRunToPortal({
          ...
          buildInput: {
            ...
            deliveryReportSummary: deliveryReportSummaryFromOutcomes(
              run.tasks.map((t) => t.outcome),
            ),
            approvalRecord:  {
            ...
            ...(() => {
              const ov = outcomeVerificationFromTasks(
                run.tasks.map((t) => t.outcomeVerification),
              );
    ...
    if (autoYes) return terminalRecord;
    ...
    if (!process.stdin.isTTY) return terminalRecord;
    ...
    if (specPath) {
      ...
      if (newPath.trim()) specPath = newPath.trim();
    } else {
      specPath = undefined;
    }
  }
...
async function runHistoryLoop(
  ...
  while (true) {
    ...
    if (!selected) { print(["  Invalid selection."]); continue; }

    const record = getWorkflowAuditRecord(storeDir, selected.workflowRunId);
    if (!record) { print(["  Audit record not found."]); continue; }
    ...
    if (detailFrame.prompt?.kind === "confirm") {
      ...
      if (back.trim().toLowerCase() === "exit") break;
...
async function main(): Promise<void> {
  ...
  const reportToPortal =
    process.env["USESTEADY_REPORT_TO_PORTAL"] === "1" || args.includes("--report-to-portal");
  ...
  try {
    if (runtime === "onboarding") {
      ...
      const positional = filteredArgs.slice(1).filter(
        a => a !== "--yes" && a !== "--report-to-portal" && a !== "--pending-approval-bridge",
      );
      ...
      const hasExplicitWorkspaceRoot =
        rawSpecPath !== undefined && positional.length >= 2;
      ...
      const resumeOpts: RunWorkflowResumeOptions = {
        ...(resumeFromPath !== undefined ? { resumeFromPath } : {}),
        reexecuteNonIdempotent,
      };
      const terminal = await runWorkflowLoop(
        rl, specPath, root, autoYes, executionPolicy, resumeOpts, reportToPortal, pendingApprovalBridge,
      );
...
// otherwise changes.
export const _runComplete: Promise<void> = main()
  .catch(err => {
    process.stderr.write(`\nFatal: ${String(err)}\n`);
    process.exit(1);
  })
