...
import {
  isNlRecoveryEligibleFailure,
  synthesizeStructuredFieldsFromNL,
} from "./spec-nl-synth.js";
...
function looksLikeJsonOp(raw: unknown): raw is Record<string, unknown> {
  return (
    typeof raw === "object" &&
    raw !== null &&
    !Array.isArray(raw) &&
...
function looksLikeJsonOpArray(raw: unknown): raw is Record<string, unknown>[] {
  return Array.isArray(raw) && raw.length > 0 && raw.every(looksLikeJsonOp);
}

function tryAutoWrapJsonOpsAsWorkflowSpec(raw: unknown): WorkflowSpec | null {
  ...
  const irOps = ops.map((op, i) => {
    ...
    if (irOp === null) {
      throw new WorkflowSpecLoadError(
        "invalid_op",
        ops.length === 1
...
const CONFLICTING_INPUT_SOURCES_MESSAGE =
  "Workflow task cannot define both input and structuredReplace. Use exactly one source of truth.";
...
const RUN_SPEC_ACCEPTED_SHAPES_MESSAGE =
  'Workflow spec was not recognized.\n' +
  '  Accepted shapes:\n' +
...
export function loadWorkflowSpecFromFile(filePath: string): WorkflowSpec {
  ...
  try {
    ...
    const nodeCode = (cause as NodeJS.ErrnoException | null)?.code;
    if (nodeCode === "ENOENT") {
      throw new WorkflowSpecLoadError(
        "file_not_found",
        `Workflow spec not found: ${absolutePath}`,
      );
    }
    if (nodeCode === "EACCES" || nodeCode === "EPERM") {
      throw new WorkflowSpecLoadError(
        "file_not_found",
        `Workflow spec not readable (permission denied): ${absolutePath}`,
      );
    }
    if (cause instanceof SyntaxError) {
      throw new WorkflowSpecLoadError(
        "invalid_json",
        `Workflow spec is not valid JSON: ${absolutePath}`,
      );
    }
    const reason = cause instanceof Error ? cause.message : String(cause);
    throw new WorkflowSpecLoadError(
      "invalid_op",
      `Could not read workflow spec at ${absolutePath}: ${reason}`,
    );
  }
  ...
  const tasks = (obj["tasks"] as unknown[]).map((t, i) => {
    ...
    const structuredReplace =
      sr && typeof sr === "object" && !Array.isArray(sr) &&
      typeof (sr as Record<string, unknown>)["oldValue"]  === "string" &&
      typeof (sr as Record<string, unknown>)["newValue"]  === "string" &&
      typeof (sr as Record<string, unknown>)["filePath"]  === "string"
        ? sr as { readonly oldValue: string; readonly newValue: string; readonly filePath: string }
        : undefined;
    ...
    if (hasExplicitInput && structuredReplace !== undefined) {
      ...
      const inputText = rawInput as string;
      const isJsonPlaceholder =
        /^replace "x" with "y" in .+$/.test(inputText);
      if (!isJsonPlaceholder && inputText !== canonical) {
        throw new WorkflowSpecLoadError(
          "conflicting_input_sources",
          `Task ${i + 1}: ${CONFLICTING_INPUT_SOURCES_MESSAGE}`,
        );
      }
    }
    ...
    if (runtime !== undefined && runtime !== "cursor" && runtime !== "claude") {
      throw new Error(
        `Task ${i + 1} has an invalid "runtime" value: ${JSON.stringify(runtime)}. ` +
    ...
    if (targetFiles !== undefined) {
      ...
      for (const f of targetFiles as unknown[]) {
    ...
    if (
      ...
      throw new Error(
        `Task ${i + 1} has an invalid "operationType" value: ${JSON.stringify(operationType)}.`
      );
    }
    ...
    const requestedOccurrence: "first" | undefined =
      rawRequestedOccurrence === "first" ? "first" : undefined;
    ...
    let synthOperationType:    typeof operationType    = operationType;
    let synthTargetFiles:      readonly string[] | undefined = undefined;
    let synthContent:          typeof content          = content;
    let synthNewPath:          typeof newPath          = newPath;
    let synthCommand:          typeof command          = command;
    let synthStructuredReplace: typeof structuredReplace = structuredReplace;
    let synthRequestedOccurrence: "first" | undefined  = undefined;
    ...
    if (!hasStructured) {
      ...
      if (!result.ok) {
        ...
        if (isNlRecoveryEligibleFailure(result)) {
          return {
            input: taskInput,
            ...(typeof task["label"] === "string" && task["label"] ? { label: task["label"] as string } : {}),
            ...(runtime ? { runtime: runtime as "cursor" | "claude" } : {}),
            ...(targetFiles ? { targetFiles: targetFiles as readonly string[] } : {}),
          };
        }
        throw new WorkflowSpecLoadError(
          "invalid_op",
          `Task ${i + 1} input could not be parsed as a supported NL operation: ` +
          `${result.reason}\n` +
      ...
      if (f.operationType    !== undefined) synthOperationType    = f.operationType;
      if (f.targetFiles      !== undefined) synthTargetFiles      = f.targetFiles;
      if (f.content          !== undefined) synthContent          = f.content;
      if (f.newPath          !== undefined) synthNewPath          = f.newPath;
      if (f.command          !== undefined) synthCommand          = f.command;
      if (f.structuredReplace !== undefined) synthStructuredReplace = f.structuredReplace;
      // F8-W4 — the mapper only ever yields "first" (non-"first" is a
      // load refusal); the === "first" guard is belt-and-braces typing.
      if (f.requestedOccurrence === "first") synthRequestedOccurrence = "first";
    }

    const finalTargetFiles: readonly string[] | undefined =
      (targetFiles as readonly string[] | undefined) ?? synthTargetFiles;
    ...
    return {
      ...
      ...(synthOperationType    ? { operationType: synthOperationType as "replace" | "create_dir" | "write_file" | "rename" | "delete_file" | "append_file" | "prepend_file" | "run_command" } : {}),
  ...
  if (defaultRuntime !== undefined && defaultRuntime !== "cursor" && defaultRuntime !== "claude") {
    throw new Error(
      `"defaultRuntime" has an invalid value: ${JSON.stringify(defaultRuntime)}. ` +
      `Expected "cursor" or "claude".`
    );
  }
  ...
  return {
    ...
    tasks,
    ...(defaultRuntime ? { defaultRuntime: defaultRuntime as "cursor" | "claude" } : {}),
    ...(typeof obj["maxRetries"] === "number" ? { maxRetries: obj["maxRetries"] as number } : {}),
  };
}
