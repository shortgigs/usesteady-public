 ...
 *                                   (a/../../etc), and POSIX/file
 *                                   symlink hops whose effective
 *                                   destination is outside the
 *                                   workspace (P-F01). Same code is also
 ...
 *                                   no telemetry signal.
 *     effective_resource_unresolved Effective destination could not be
 *                                   safely established (unreadable or
 *                                   cyclic hop). Refused before preview
 *                                   and before effect. P-F01.
 *     effective_resource_changed    Execution-time receiver is not the
 *                                   approval-bound receiver. Refused
 *                                   even if the new hop is still
 *                                   contained. Fresh preview required.
 *     effect_result_mismatch        Authorized create payload does not
 *                                   match the resulting file bytes, or
 *                                   correspondence could not be
 *                                   established. Success is refused.
 *                                   P-EFFECT-RESULT. Distinct from
 *                                   ADV-V2-037 (input to proposal).
...
async function askApproval(prompt: string): Promise<string> {
...
if (jsonIdx >= 0) rawArgs.splice(jsonIdx, jsonFlag !== null ? 2 : 1);
...
if (outputIdx >= 0) rawArgs.splice(outputIdx, outputMode !== null ? 2 : 1);
...
//
// Five input sources can provide direct input to the CLI. The contract is
// deterministic: at most ONE may be used per invocation. Combining any two is
// a conflict, not a silent-drop.
//
//   stdin     piped request via non-TTY stdin
//   --prompt  explicit prompt flag
//   --json    explicit json flag (structured op or array)
//   argv      positional argv carrying a direct request
//             (any subcommand token that is not a known pure subcommand and
//              is not "batch" — e.g. usesteady "rename a.ts to b.ts")
//   batch     the `batch <file.json>` subcommand (reads ops from a file)
//
// "Pure" subcommands (run, history, cursor, claude, setup, ui, help, report,
// admin, --help, -h, --version, -v) are orthogonal — they do not provide
// direct input and are not subject to this conflict check.
//
// Pre-fix bug: argv input silently drop-won over stdin/--prompt/--json.
// Users running `echo "..." | usesteady "rename ..."` or `usesteady "..."
// --prompt "..."` saw a single success result with only one of their two
// inputs actually executed, and zero diagnostic. This source contract
// detects every such pair up-front and fails explicitly with exit code 2.

type DirectInputSource = "stdin" | "--prompt" | "--json" | "argv" | "batch";

type InputSourceResolution =
  | { ok: true; source: DirectInputSource | null }
  | { ok: false; sources: readonly DirectInputSource[] };
...
function resolveDirectInputSource(params: {
  ...
  const sources: DirectInputSource[] = [];
  if (params.hasStdin)  sources.push("stdin");
  if (params.hasPrompt) sources.push("--prompt");
  if (params.hasJson)   sources.push("--json");
  if (params.hasArgv)   sources.push("argv");
  if (params.hasBatch)  sources.push("batch");
  if (sources.length > 1) return { ok: false, sources };
...
const PURE_SUBCOMMANDS = new Set<string>([
  "run", "history", "cursor", "claude",
  "setup", "ui", "help", "report", "admin",
  "replay",
  "capabilities",
  "quickstart",
  "templates",
...
const hasExplicitNonStdinSource =
  isBatchSubcommand || jsonFlag !== null || promptFlag !== null;
const shouldSampleDirectStdin =
  !isTTY && !isPureSubcommand && !(autoYes && hasExplicitNonStdinSource);
...
async function readStdinPiped(): Promise<string> {
  const ctrl = new SessionController({ mode: "one_shot", autoYes: false });
  try {
...
const HELP_TEXT = `
  ...
  Every request is validated before anything runs.
  You see SYSTEM WILL -- the declared operation and target -- then approve it. --yes skips per-step prompts.
  ...
  Usage:
    ...
    usesteady run spec.json --resume-from <token.json>
                                                  Resume a stopped run from its token
    usesteady run spec.json --resume-from <token.json> --reexecute-non-idempotent
                                                  Resume including non-idempotent completed tasks
    usesteady workflow resume-info <token.json> --spec spec.json
                                                  Inspect a resume token (no state mutation)
    ...
    usesteady doctor                              Check local install readiness (temporary store mutation; no governed effect)
    usesteady auth refresh                        Fetch Team entitlement projection (writes entitlement cache)
    ...
    usesteady timeline --last                     Chronological view of latest terminal run (no state mutation)
    usesteady timeline --run-id <id>              Chronological view of one terminal run (no state mutation)
    usesteady timeline --last --output json       Same view, machine-readable JSON (no state mutation)
    usesteady reconstruct <ucp_root_id>           Rebuild what happened from a UCP root (no governed effect; may persist bookkeeping)
    usesteady reconstruct <ucp_root_id> --output json   Machine-readable reconstruction report
    usesteady audit export --run-id <id>          Export one terminal run as JSON (no state mutation)
    usesteady audit export --run-id <id> --output file
                                                  Write export to ./audit-exports/<run-id>.json
                                                  (no governed effect; writes that file)
    usesteady audit summary --last                Operational summary of latest run (no state mutation)
    usesteady usage summary                       Local usage summary (no governed effect; may persist bookkeeping)
  ...
  Audit store (discoverability — not tamper evidence):
    Default audit store is ephemeral: the OS temp folder + usesteady-ucp.
    OS temp cleanup can delete it. That is not durable audit retention.
    Set USESTEADY_STORE_DIR to a directory outside the OS temp folder
    for configured local storage; retention is not guaranteed. usesteady doctor reports ephemeral vs
    configured-local. This slice does not change the default location.

  Safe resume:
    After a stopped run, a token is written under
    <workspace>/.usesteady/resume-tokens/.
    Inspect without executing:
      usesteady workflow resume-info <token.json> --spec <spec.json>
    Continue the same execution instance:
      usesteady run <spec.json> --resume-from <token.json>
    Non-idempotent completed tasks (append / prepend / replace / ...)
    refuse resume unless you also pass --reexecute-non-idempotent.
  ...
  --output json error codes (all documented, surfaced verbatim):
    ...
    Adapter-layer (surfaced verbatim from the cursor execution result):
      ...
      outside_workspace             Path resolves outside the workspace root.
                                    ...
                                    //srv/share/x), \`..\`-after-normalize
                                    (a/../../etc), and POSIX/file symlink
                                    hops whose effective destination is
                                    outside the workspace (P-F01). Distinct
                                    from scope_outside_allowed (workspace
                                    boundary vs. cursor-session allowedFiles).
                                    (S2 / friction #36; P-F01 hop.)
      effective_resource_unresolved Effective destination could not be
                                    safely established (unreadable or
                                    cyclic hop). Refused before preview
                                    and before effect. (P-F01.)
      effective_resource_changed    Execution-time receiver is not the
                                    approval-bound receiver. Refused even
                                    if still contained. Fresh preview
                                    required. (P-F01.)
      effect_result_mismatch        Authorized create payload does not
                                    match the resulting file bytes, or
                                    correspondence could not be
...
import {
  draftTaskToSpecTask as draftTaskToSpecTaskShared,
  type SpecTask as SpecTaskShared,
} from "./spec-task-builder.js";
type SpecTask = SpecTaskShared;
function draftTaskToSpecTask(
  ...
  _draftTaskToInput: (t: any) => string,
): SpecTask {
  return draftTaskToSpecTaskShared(t, i);
}

// ─── Result-file helpers (--output json plumbing) ─────────────────────────────
// USESTEADY_RESULT_PATH is a temporary file written by main.ts at the end of
// runWorkflowLoop and read once here.  It exists only for the lifetime of a
// single CLI invocation and is deleted after reading.

type WorkflowResult = {
  succeeded: boolean;
  errorCode: string | null;
  // Stabilization P0 / PR-2: strictly additive summary fields.
  // Present for every run produced by main.ts; older result-files
  // from pre-PR-2 builds will not carry these, so the reader must
  ...
  total_steps?:    number;
  // Issue #42 -- count of OCD conflict warnings accepted along the
  // run. Optional for the same reason as the five fields above:
  // pre-#42 result files do not carry the field, so readers must
  // tolerate its absence. main.ts always sets it (zero when no
  // warnings were accepted) for post-#42 runs.
...
async function readWorkflowResult(resultPath: string): Promise<WorkflowResult | null> {
  try {
    const { existsSync, readFileSync, rmSync } = await import("node:fs");
    if (!existsSync(resultPath)) return null;
    const data = JSON.parse(readFileSync(resultPath, "utf8")) as WorkflowResult;
...
async function emitJsonResult(resultPath: string): Promise<boolean> {
...
function workflowOutcomeReadErrorMessage(error: "missing" | "malformed" | "token_mismatch"): string {
  if (error === "missing") return "missing terminal outcome from delegated workflow run";
  if (error === "token_mismatch") return "terminal outcome token mismatch (stale or foreign outcome file)";
  return "malformed terminal outcome from delegated workflow run";
}

function frictionHintLine(): string {
...
function requestedOccurrenceForJsonError(validateError: {
  readonly operation: { readonly type: string; readonly args: Record<string, unknown> };
}): string | undefined {
  const op = validateError.operation;
  if (op.type !== "replace") return undefined;
  const req = op.args["requestedOccurrence"];
  if (req === undefined) return undefined;
  if (req === "first") return "first";
  if (req === "all")   return "all";
  if (
    typeof req === "object" &&
    req !== null &&
    "index" in (req as Record<string, unknown>) &&
    typeof (req as { index: unknown }).index === "number"
  ) {
    const n = (req as { index: number }).index;
    const mod100 = n % 100;
    const suffix =
...
function failInputSourceConflict(sources: readonly DirectInputSource[]): never {
...
function failUnsupportedDraftClauses(clauses: readonly string[]): never {
  if (jsonOutput) {
    process.stdout.write(JSON.stringify({
      success: false,
...
async function processDraftInput(
  input:    string,
  autoYes:  boolean,
  inputMode: "prompt_flag" | "stdin_pipe" | "argv",
): Promise<void> {
  ...
  const { translateIntent, formatDraftTask, draftTaskToInput } =
    await import("./draft/intent-to-tasks.js");
  ...
  if (!jsonOutput) {
    ...
    tasks.forEach((t, i) => {
      ...
      if (!t.needs_confirmation) {
        ...
        if (detail.length > 0) process.stdout.write(detail.join("\n") + "\n");
      }
    });
    if (uncertain.length > 0) {
      process.stdout.write([
...
async function processJsonInput(
  ...
  if (!parseResult.ok) {
    ...
    if (jsonOutput) process.stdout.write(JSON.stringify({ success: false, error: "invalid_json" }) + "\n");
    process.exit(2);
  }
  ...
  const { formatDraftTask, draftTaskToInput } =
    await import("./draft/intent-to-tasks.js");
  const { jsonOpToIROperation, diagnoseInvalidJsonOp } =
    await import("../../input/json-to-ir.js");
  const { irOperationToJsonDraftTask }   = await import("../../input/ir-to-draft.js");
  const { mapIROpToStructuredFields } = await import("./ir-to-spec-fields.js");
  ...
  const irOperations: import("../../input/ir.js").Operation[] = [];
  ...
  for (const op of ops) {
    ...
    if (irOp === null) {
      ...
      if (jsonOutput) process.stdout.write(JSON.stringify({ success: false, error: "invalid_op" }) + "\n");
      process.exit(2);
    }
    ...
    safetyInputs.push(
      task.action === "run_command" && typeof task.to === "string"
        ? task.to
    ...
    const createFields = irOp.type === "create" ? mapIROpToStructuredFields(irOp) : undefined;
    if (createFields && !createFields.ok) throw new Error("internal: create mapping failed");
    specTasks.push({
      ...draftTaskToSpecTask(task, i, draftTaskToInput),
      // ADV-V2-037: DraftTask predates create payloads. Preserve the
      // validated IR content when materializing the approval proposal.
      ...(createFields?.ok ? createFields.fields : {}),
    ...
    const shadowOp =
      task.action === "replace" && task.from !== undefined && task.to !== undefined && task.file !== undefined
        ? { type: "replace" as const, from: task.from, to: task.to, file: task.file }
        : task.action === "rename" && task.from !== undefined && task.to !== undefined
          ? { type: "rename" as const, from: task.from, to: task.to }
          : task.action === "create" && task.file !== undefined
            ? { type: "create" as const, file: task.file }
            : task.action === "delete" && task.file !== undefined
              ? { type: "delete" as const, file: task.file }
              : task.action === "run_command" && task.to !== undefined
                ? { type: "run" as const, to: task.to }
                : task.action === "append" && task.to !== undefined && task.file !== undefined
                  ? { type: "append" as const, to: task.to, file: task.file }
                  : task.action === "prepend" && task.to !== undefined && task.file !== undefined
                    ? { type: "prepend" as const, to: task.to, file: task.file }
                    : null;
    if (shadowOp !== null) {
  ...
  const _ir: import("../../input/ir.js").IR = {
  ...
  if (validateError !== null) {
    ...
    if (jsonOutput) {
      ...
      const payload: Record<string, unknown> = { success: false, error: validateError.code };
      if (requestedOcc !== undefined) payload["requested_occurrence"] = requestedOcc;
      process.stdout.write(JSON.stringify(payload) + "\n");
    }
    process.exit(1);
  }

  // P-F01 — preview copy names the effective receiver. Execution
  // specTasks keep the supplied path so execute can re-resolve the hop
  // and compare it to the approval-bound identity.
  const { applyEffectivePathsToTaskSpec } = await import("../../input/effective-resource.js");
  const previewRoot = process.cwd();
  ...
  if (!jsonOutput) {
    ...
    previewLines.forEach((line, i) => {
      ...
      const detail = entryApprovalDetailLines(
        applyEffectivePathsToTaskSpec(specTasks[i]!, previewRoot),
      );
      if (detail.length > 0) process.stdout.write(detail.join("\n") + "\n");
    });
  }
  ...
  if (ans !== "y" && ans !== "yes") {
    if (!jsonOutput) process.stdout.write("  Cancelled.\n" + frictionHintLine() + "\n");
    process.exit(0);
  }
  ...
  const { BATCH_FAIL_FAST_ENV, shouldBatchFailFast } =
    await import("./execution-policy.js");
...
async function delegateRawInputToWorkflow(
  nlInput: string,
  workflowName: string,
): Promise<never> {
  const spec = JSON.stringify({
 ...
 */
async function cliRecoverySurface(
  nlInput: string,
): Promise<import("../../workflow/routing-surface.js").RoutingSurface | undefined> {
  try {
    const [{ synthesizeFirstTaskFromNL, computeArtifactsForSpec }, { concreteSuggestions }, loader, { LocalRecoveryAdapter }, path, fs, url] =
      await Promise.all([
    ...
    const registry = (scan as { registry: ReturnType<typeof loader.buildRegistry> }).registry;
    ...
    const artifacts = await computeArtifactsForSpec({
      ...
      root: process.cwd(),
      suggest: (input: string) => concreteSuggestions(input, { registry, adapter }),
      repository: null,
    });
...
async function processNLInput(
  nlInput: string,
  autoYes: boolean,
  surface: "prompt" | "stdin" | "positional",
): Promise<void> {
  ...
  if (result.kind === "error") {
    ...
    if (err.code === "parse_error") {
      ...
      if (isTTY && !autoYes && !jsonOutput) {
        const { classifyClarifyRecoverability, reconstructClarifiedInput } =
          await import("../../input/clarify-recoverability.js");
      ...
      if (isTTY && !autoYes && !jsonOutput) {
        const [{ proposeCandidateUnderstanding }, { hydrateEnvFromConfig }] =
          await Promise.all([
      ...
      await delegateRawInputToWorkflow(
        nlInput,
        `NL recovery: ${nlInput.slice(0, 60)}`,
      );
    }
    if (jsonOutput) {
      process.stdout.write(
  ...
  const { formatDraftTask, draftTaskToInput } =
    await import("./draft/intent-to-tasks.js");
  const { irOperationToJsonDraftTask } =
    await import("../../input/ir-to-draft.js");
...
function formatPathTokenForDiagnostics(p: string): string {
...
if (subcommand === "ui") {
  ...
  const serverPath   = r(d(fileURLToPath(import.meta.url)), "../../../server.js");
  const serverScript = pathToFileURL(serverPath).pathname.replace(/^\/([A-Za-z]:)/, "$1");

  process.stdout.write("\n  UseSteady -- Starting UI...\n");
  const child = await import("node:child_process").then(m =>
    m.spawn(process.execPath, [serverScript], { stdio: "inherit", env: process.env })
  );
  child.on("exit", (code: number | null) => process.exit(code ?? 0));
  process.on("SIGINT",  () => (child as import("node:child_process").ChildProcess).kill("SIGINT"));
  process.on("SIGTERM", () => (child as import("node:child_process").ChildProcess).kill("SIGTERM"));
  ...
  try   { await runSetupSubcommand(setupCtrl); }
  finally { setupCtrl.shutdown(); }
  process.exit(0);
  ...
  if (adminSub === "friction") {
    try {
      ...
      const { runAdmin } = await import("./friction-admin.js") as { runAdmin: (a: string[]) => Promise<void> };
  ...
  const { renderCapabilitiesJson, renderCapabilitiesText } =
  ...
  const {
    ...
    renderAuthHelpText,
    runAuthCommand,
  } = await import("./entitlement-cache.js");
  ...
  const unknownFlag = rest.some(
    (tok) => tok.startsWith("-") && tok !== "--help" && tok !== "-h",
  );
  if (unknownFlag) {
  ...
  // No governed effect. Never executes, never approves, never loads specs.
  // Temporary D5 store probe is created and removed. D17 reports store durability.
  const wantsHelp = rest.includes("--help") || rest.includes("-h");
  if (wantsHelp) {
  ...
  if (auditVerb === "summary") {
    const {
      parseAuditSummaryArgs,
      runAuditSummary,
      renderAuditSummaryHelpText,
      renderAuditSummaryUsageError,
    } = await import("./audit-summary.js");
  ...
  const {
    ...
    runAuditExport,
    renderAuditExportHelpText,
    renderMissingRunIdError,
  ...
  // P2 — local usage summary from usage-events.jsonl (may persist bookkeeping).
  const {
    parseUsageArgs,
    runUsageCommand,
    renderUsageHelpText,
  } = await import("./usage-summary.js");
  ...
  const {
    renderTemplatesListJson,
    renderTemplatesListText,
    renderTemplateDetailJson,
    renderTemplateDetailText,
    renderUnknownTemplateText,
    isKnownTemplateName,
  ...
  const { renderExamplesJson, renderExamplesText } =
  ...
  if (verb === "inspect") {
    ...
    const { inspectWorkflowSpec, renderInspectionJson, renderInspectionText } =
      await import("./workflow-inspect.js");
  ...
  const { runResumeInfo, renderResumeInfoText, renderResumeInfoJson } =
    await import("./workflow-resume-info.js");
  ...
  if (internalArg !== null) {
    ...
    let runResultPath: string | undefined;
