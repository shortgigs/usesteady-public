 ...
 */
export function contributionsForIROperation(op: Operation): readonly TargetContribution[] {
  switch (op.type) {
    case "create":
 ...
 */
export function contributionsForNonDeterministicTask(
  targetFiles: readonly string[] | undefined,
): readonly TargetContribution[] {
  if (!targetFiles) return [];
  const out: TargetContribution[] = [];
...
const KIND_RANK: Readonly<Record<TargetEntryKind, number>> = Object.freeze({
...
function mergeKind(a: TargetEntryKind, b: TargetEntryKind): TargetEntryKind {
...
export function buildTargetTree(
  contributions: readonly TargetContribution[],
): readonly TargetEntry[] {
  const byPath = new Map<string, { kind: TargetEntryKind; ops: Set<string> }>();
 ...
 */
export function renderTargetTreeText(entries: readonly TargetEntry[]): readonly string[] {
  if (entries.length === 0) return Object.freeze<string[]>([]);

  // Build a hierarchical view. We treat each entry as a leaf at its
  // path; directories along the way are inferred. Directories that ARE
  // explicit entries (kind: "dir") render their basename at their parent
  // level; deeper leaves under them indent further.
  //
  // We need a uniform node model:
  //   node = { name, kind, ops, children: Map<name, node> }
  type Node = {
    name:     string;
    kind:     TargetEntryKind | "intermediate";
    ops:      readonly string[];
    children: Map<string, Node>;
  };

  const root: Node = { name: "", kind: "intermediate", ops: [], children: new Map() };
  ...
  for (const e of entries) {
    ...
    if (tail.length > widestBasename) widestBasename = tail.length;
  }
