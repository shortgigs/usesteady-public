...
import type { StructuredFields } from "./ir-to-spec-fields.js";

// ─── Public types ────────────────────────────────────────────────────────────

/**
 * Re-export of the shared structured-fields shape so callers that import
 * from `spec-nl-synth.ts` don't have to reach into `ir-to-spec-fields.ts`.
 */
export type SynthesizedTaskFields = StructuredFields;

export type SynthesisResult =
  | { readonly ok: true;  readonly fields: SynthesizedTaskFields }
  | { readonly ok: false; readonly reason: string; readonly code?: CliErrorCode };
 ...
 */
export function isNlRecoveryEligibleFailure(
  result: Extract<SynthesisResult, { ok: false }>,
): boolean {
 ...
 */
export function synthesizeStructuredFieldsFromNL(input: string): SynthesisResult {
