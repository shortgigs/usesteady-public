...
export type TemplateEntry = {
  ...
  readonly exampleValues: readonly { readonly field: string; readonly example: string }[];
...
export type TemplatesCatalog = {
  readonly templates: readonly { readonly name: TemplateName; readonly purpose: string }[];
};
 ...
 */
const TEMPLATE_REGISTRY: Readonly<Record<TemplateName, TemplateEntry>> = Object.freeze({
  "append-to-file": Object.freeze({
    name: "append-to-file",
    purpose:
      "Append a block of text to an existing file without touching anything that is already there.",
    requiredFields: Object.freeze(["file", "to"]) as readonly string[],
    optionalFields: Object.freeze([]) as readonly string[],
    exampleValues: Object.freeze([
      Object.freeze({ field: "file", example: "NOTES.md" }),
      Object.freeze({ field: "to",   example: "hello from usesteady" }),
    ]) as readonly { field: string; example: string }[],
  ...
  "safe-rename": Object.freeze({
    name: "safe-rename",
    purpose:
      "Rename a single file with explicit approval. Atomic at the filesystem level; reversible by a reverse rename.",
    requiredFields: Object.freeze(["from", "to"]) as readonly string[],
    optionalFields: Object.freeze([]) as readonly string[],
    exampleValues: Object.freeze([
  ...
  "multi-file-replace": Object.freeze({
    ...
    purpose:
      "Apply the same exact-text replacement across an explicit, hand-listed set of files. Each operation is approved individually.",
    requiredFields: Object.freeze(["file", "from", "to"]) as readonly string[],
    optionalFields: Object.freeze([]) as readonly string[],
    exampleValues: Object.freeze([
  ...
  "non-destructive-cleanup": Object.freeze({
    ...
    purpose:
      "Organize existing files into a new directory by renaming them. Creates the directory first; moves files via rename. No deletions.",
    requiredFields: Object.freeze(["path", "from", "to"]) as readonly string[],
    optionalFields: Object.freeze([]) as readonly string[],
    exampleValues: Object.freeze([
  ...
  "git-safe-review-flow": Object.freeze({
    ...
    purpose:
      "Capture review notes in a tracked file (REVIEW.md) without modifying any source code. Creates the file if missing; appends additional notes on subsequent runs.",
    requiredFields: Object.freeze(["file", "to"]) as readonly string[],
    optionalFields: Object.freeze([]) as readonly string[],
    exampleValues: Object.freeze([
  ...
  "bump-version": Object.freeze({
    name: "bump-version",
    purpose:
      "Bump a project version: replace the `\"version\"` field in package.json and append a matching CHANGELOG.md entry. Two operations across two files; does NOT run git, npm, or any command.",
    requiredFields: Object.freeze(["file", "from", "to"]) as readonly string[],
    optionalFields: Object.freeze([]) as readonly string[],
    exampleValues: Object.freeze([
  ...
  "add-import": Object.freeze({
    name: "add-import",
    purpose:
      "Prepend an import statement to the top of a source file. Single operation using `prepend`, which writes to the very start of the file.",
    requiredFields: Object.freeze(["file", "to"]) as readonly string[],
    optionalFields: Object.freeze([]) as readonly string[],
    exampleValues: Object.freeze([
...
export function getTemplate(name: string): TemplateEntry | undefined {
  if (!isKnownTemplateName(name)) return undefined;
 ...
 */
export function isKnownTemplateName(value: unknown): value is TemplateName {
 ...
 */
export function getAllTemplateOperationTypes(): readonly string[] {
  const types: string[] = [];
 ...
 */
export function allTemplateOperationsUseRegisteredOps(): boolean {
  const known = new Set<string>(ALL_OPERATION_TYPES as readonly string[]);
 ...
 */
export function renderTemplatesListJson(): string {
 ...
 */
export function renderTemplatesListText(): string {
 ...
 */
export function renderTemplateDetailJson(name: string): string | undefined {
 ...
 */
export function renderTemplateDetailText(name: string): string | undefined {
  const entry = getTemplate(name);
  if (entry === undefined) return undefined;
  const lines: string[] = [];
  lines.push("");
  ...
  if (entry.requiredFields.length > 0) {
    for (const f of entry.requiredFields) lines.push(`      - ${f}`);
  } else {
    lines.push("      (none)");
  }
  lines.push("");
  ...
  if (entry.optionalFields.length > 0) {
    for (const f of entry.optionalFields) lines.push(`      - ${f}`);
  } else {
    lines.push("      (none)");
  }
  lines.push("");
 ...
 */
export function renderUnknownTemplateText(name: string): string {
