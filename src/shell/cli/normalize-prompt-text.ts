 ...
 */
const OUTER_SMART_PAIRS: ReadonlyArray<readonly [string, string]> = [
  ["\u201C", "\u201D"],
  ["\u2018", "\u2019"],
];

export function hasMatchedOuterAsciiPair(s: string): boolean {
...
export function hasMatchedOuterSmartPair(s: string): boolean {
  if (s.length < 2) return false;
  const first = s.charAt(0);
  const last  = s.charAt(s.length - 1);
  for (const [open, close] of OUTER_SMART_PAIRS) {
    if (first === open && last === close) return true;
  }
  return false;
}
