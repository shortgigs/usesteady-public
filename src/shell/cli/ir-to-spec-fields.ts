...
export type StructuredFields = Pick<
  ...
  | "command"
  | "structuredReplace"
  // usesteady-public#45 — diagnostics/preview only; never read by the
  // executor. Populated by the replace branch when the user explicitly
  // specified an occurrence directive (NL always; JSON only when the
  // public op carried `occurrence`).
  | "requestedOccurrence"
>;

export type MapResult =
  | { readonly ok: true;  readonly fields: StructuredFields }
  | { readonly ok: false; readonly reason: string };

export function mapIROpToStructuredFields(op: Operation): MapResult {
  switch (op.type) {
    case "create_dir":
      return {
        ok: true,
    ...
    case "replace": {
      if (op.args.occurrence !== "first") {
        ...
        return {
          ok: false,
          reason:
            `replace with "${occText}" is not yet supported. The executor ` +
