...
function print(lines: readonly string[]): void {
  for (const line of lines) process.stdout.write(sanitizeForConsole(line) + "\n");
}
...
function ask(rl: SessionController, prompt: string): Promise<string> {
...
/**
 * Return value for every interactive prompt that can be escaped via "exit".
 * Consistent shape across the onboarding module so callers can propagate
 * user-cancellation cleanly without throwing across readline boundaries.
 *
 * Interactive exit contract ("interactive session exits"). Pre-fix, typing
 * "exit" at the example-approval prompt re-prompted forever because the
 * keyword wasn't recognised — the user was trapped until they typed a or r.
 */
type ExampleOutcome = "approved" | "exit";
 ...
 */
function isExitInput(raw: string): boolean {
...
async function showApproved(): Promise<void> {
 ...
 */
export async function runOnboarding(rl: SessionController): Promise<string | null> {
