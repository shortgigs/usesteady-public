...
import type { ConsensusAuditRecord } from "../../claude/adapters/multi-llm-types.js";
...
export const USAGE_EVENT_SCHEMA = "usage-event.v1" as const;
 ...
 */
export type ObservedProviderLabel =
  | "anthropic"
  | "openai-compatible"
  | "openai"
  | "xai";
...
function normalizeUsageEvent(parsed: Record<string, unknown>): UsageEventRecord | null {
  if (parsed["schemaVersion"] !== USAGE_EVENT_SCHEMA) return null;
  if (typeof parsed["eventId"] !== "string") return null;
  if (parsed["eventType"] !== "workflow_run" && parsed["eventType"] !== "provider_call") return null;
  ...
  return {
    ...
    ...(typeof parsed["provider"] === "string"
      ? { provider: parsed["provider"] as ObservedProviderLabel }
      : {}),
...
function loadKnownEventIds(events: readonly UsageEventRecord[]): Set<string> {
  return new Set(events.map((e) => e.eventId));
}

/** D12: only read numeric token/cost fields explicitly present on a source object. */
export function readRecordedUsageFields(
  source: Record<string, unknown>,
): { readonly recordedTokens: number | null; readonly recordedCostUsd: number | null } {
  ...
  let recordedTokens: number | null = null;
...
function mapObservedConsensusPlugin(pluginName: string): ObservedProviderLabel | null {
  if (pluginName === "claude") return "anthropic";
  if (pluginName === "openai-compatible") return "openai-compatible";
  if (pluginName === "openai") return "openai";
  if (pluginName === "xai") return "xai";
  return null;
}

function hasObservedClaudeDelivery(
  session: NonNullable<ReturnType<typeof getWorkflowAuditRecord>>["tasks"][number]["session"],
): boolean {
...
function providerCallEvent(
  partial: Omit<UsageEventRecord, "schemaVersion" | "recordedAt" | "recordedTokens" | "recordedCostUsd" | "callCount">,
  recordedAt: string,
  source: Record<string, unknown> = {},
): UsageEventRecord {
...
function parseConsensusAuditLines(content: string): ConsensusAuditRecord[] {
  const records: ConsensusAuditRecord[] = [];
  for (const line of content.split("\n")) {
    if (line.trim().length === 0) continue;
    try {
      const stripped = line.startsWith("[consensus-audit] ")
        ...
        : line;
      const parsed = JSON.parse(stripped) as ConsensusAuditRecord;
...
export function materializeUsageEvents(
  ...
  for (const summary of getWorkflowHistories(storeDir)) {
    ...
    for (const task of audit.tasks) {
      if (!hasObservedClaudeDelivery(task.session)) continue;
  ...
  if (existsSync(consensusPath)) {
    ...
    for (const record of parseConsensusAuditLines(consensusContent)) {
      for (const round of record.rounds) {
        ...
        }, recordedAt, round as unknown as Record<string, unknown>));
        knownIds.add(eventId);
      }
    }
  }
...
export function aggregateUsageSummary(
  ...
  const providerDistribution: Record<ObservedProviderLabel, number> = {
    anthropic:         0,
    "openai-compatible": 0,
    openai:            0,
    xai:               0,
  ...
  for (const event of events) {
    ...
    if (event.eventType !== "provider_call" || event.provider === undefined) continue;
...
export function formatRecordedCostUsd(value: number | null): string {
  if (value === null) return "unknown";
  return `$${value.toFixed(2)}`;
}

export function formatRecordedTokens(value: number | null): string {
  if (value === null) return "unknown";
  return formatUsageInteger(value);
}

export function renderUsageSummaryText(summary: UsageSummary): string {
  const lines = [
    "",
    "  Usage summary:",
    `  Runs: ${formatUsageInteger(summary.runs)}`,
    `  Recorded provider-call events: ${formatUsageInteger(summary.providerCalls)}`,
    ...
    "",
    "  Tokens and cost are unknown unless directly recorded in store artifacts. Recorded events are not total usage.",
    "",
  ];
  return lines.join("\n");
}
