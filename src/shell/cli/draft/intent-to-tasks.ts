...
export type DraftTask = {
  ...
  readonly from?:                string;   // source text / filename
  readonly to?:                  string;   // replacement text / new filename / command
  readonly file?:                string;   // target file path
  readonly needs_confirmation?:  true;     // set when clause could not be parsed
  readonly rawText?:             string;   // always set — original clause text
};
...
export function translateIntent(text: string): DraftTask[] {
 ...
 */
export function formatDraftTask(task: DraftTask): string {
 ...
 */
function quotePathToken(path: string | undefined): string {
  ...
  if (path === undefined) return String(path);
 ...
 */
export function draftTaskToInput(task: DraftTask): string {
  if (task.needs_confirmation && task.rawText) return task.rawText;
  switch (task.action) {
...
// ─── JSON op types (Structured CLI Automation Mode) ──────────────────────────
...
export type JsonOp = {
  ...
  readonly from?: string;   // rename: old path; replace: text to find
  readonly to?:   string;   // rename: new path; replace: replacement; run: command
  readonly file?: string;   // replace / create / delete: target file path
};
 ...
 */
export function jsonOpToDraftTask(raw: unknown): DraftTask | null {
  ...
  switch (op["type"]) {
    case "rename":
      ...
      if (from.trim().length === 0 || to.trim().length === 0) return null;
      return { action: "rename", from: from.trim(), to: to.trim(), rawText: `rename ${from.trim()} to ${to.trim()}` };
    case "replace":
      if (from === undefined || to === undefined || !file) return null;
    ...
    case "append":
      if (to === undefined || !file) return null;
...
function parseClause(clause: string): DraftTask {
  ...
  const rename =
    raw.match(/^(?:rename|mv)\s+("[^"]+"|'[^']+'|\S+)\s+to\s+("[^"]+"|'[^']+'|\S+)$/i) ??
