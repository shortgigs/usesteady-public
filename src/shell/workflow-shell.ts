 ...
 *     task_scope:  answerWorkflowScope(idx) → task_ready (fresh approval required)
 ...
 */
function recordRunApprovalProvenance(run: WorkflowRun, storeDir: string): void {
  ...
  if (storeDir.trim().length === 0) return;
  try {
    recordApprovalBasis(
      {
 ...
 */
async function deliverAndAdvance(
  run:      WorkflowRun,
  plugins:  WorkflowPlugins,
  storeDir: string,
  policies: WorkflowShellPolicies,
  deps?:    ClaudeGateDeps,
): Promise<WorkflowRun> {
  const delivered = await deliverWorkflowTask(
    run,
    plugins,
    storeDir,
    deps,
    policies.workspaceRoot !== undefined
  ...
  if (delivered.phase === "running") {
    return bindEffectiveResourceOnReady(
      ...
      policies.workspaceRoot,
    );
  }

  return bindEffectiveResourceOnReady(delivered, policies.workspaceRoot);
}
...
export async function advanceWorkflowOnConfirm(
  run:      WorkflowRun,
  yes:      boolean,
  plugins:  WorkflowPlugins,
  storeDir: string,
  policies: WorkflowShellPolicies,
  deps?:    ClaudeGateDeps,
  advisoryResolution?: AdvisoryConfirmResolution,
): Promise<WorkflowRun> {
  ...
  if (run.phase === "reviewing") {
    if (!yes) return cancelWorkflow(run);
    ...
    return bindEffectiveResourceOnReady(started, policies.workspaceRoot);
  }
  ...
  if (run.phase === "task_advisory") {
    if (
      ...
      const retired = retireWorkflowAdvisory(
        run,
        {
 ...
 *   answerWorkflowScope → task_ready with a newly bound receiver.
 *   Fresh approval is required; this does not deliver.
 ...
 */
export async function advanceWorkflowOnChoice(
  run:       WorkflowRun,
  choiceIdx: number,
  plugins:   WorkflowPlugins,
  storeDir:  string,
  ...
  if (run.phase === "task_scope") {
    const answered = answerWorkflowScope(run, choiceIdx, policies.workspaceRoot);
    if (answered === run) return run;  // out-of-range — re-prompt
    return answered;
  }
  ...
  if (run.phase === "task_failed") {
    ...
    const actionMap: Record<number, "stop" | "skip" | "retry"> = {
      1: "stop",
      2: "skip",
      3: "retry",
    };
    const action = actionMap[choiceIdx];
    if (!action) return run;  // out-of-range — re-prompt

    const next = applyFailureAction(run, action);
    if (next === run) return run;  // retry at max limit — re-prompt
