...
import type { WorkflowState, WorkflowStatus, TuiStep, TuiConsensusEntry } from "./types.js";
 ...
 */
function deriveBlockedReason(run: WorkflowRun): string {
  const note = (run.display.failureNote ?? "").toLowerCase();
  if (note.includes("scope"))                                    return "Scope of the change is unclear";
  if (note.includes("no agreement") || note.includes("quorum")) return "Models did not agree on execution";
  if (note.includes("blocked") || note.includes("policy"))      return "Models did not agree on execution";
  if (note.includes("exec") || note.includes("error"))          return "Execution could not be completed";
...
function deriveStatus(run: WorkflowRun): WorkflowStatus {
  switch (run.phase) {
    case "reviewing":
      return "Reviewing";
...
function deriveStepStatus(outcome: WorkflowRun["tasks"][number]["outcome"]): TuiStep["status"] {
  switch (outcome) {
...
function toWorkflowState(
  run:      WorkflowRun,
  consensus: readonly TuiConsensusEntry[] | undefined,
): WorkflowState {
  const steps: TuiStep[] = run.tasks.map(task => ({
    id:          task.index,
 ...
 */
export function setTuiState(
  run:       WorkflowRun,
  consensus?: readonly TuiConsensusEntry[],
): void {
  try {
 ...
 */
export function readTuiState(): WorkflowState | null {
  try {
