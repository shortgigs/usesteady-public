...
export function renderReconstructHelpText(): string {
  return (
    "usesteady reconstruct — rebuild what happened from a UCP root\n" +
    "  (no governed effect; may persist bookkeeping)\n\n" +
    ...
    "  Does not execute, approve, or replay a workflow.\n" +
    "  May persist a missing intent-root envelope in the audit store.\n" +
    "  Not a no-state-mutation command.\n" +
    "  Default store is ephemeral (OS temp). Set USESTEADY_STORE_DIR\n" +
    "  to a directory outside the OS temp folder for configured local storage; retention is not guaranteed.\n\n"
  );
}

export function renderReconstructJson(report: ReconstructReport): string {
...
export function renderReconstructText(report: ReconstructReport): string {
  const lines: string[] = [];

  lines.push("RECONSTRUCT (no governed effect; may persist bookkeeping)");
  ...
  lines.push("");
  lines.push(
    "  Reconstruction does not execute, approve, or replay. It may persist bookkeeping.",
  );
  lines.push("");
