...
import { readEnvelopeAt, readAllEnvelopes } from "./reader.js";
import type {
  UCPEnvelope,
  UCPType,
  ArtifactPayload,
 ...
 */
export function getEnvelopeById(
  storeDir: string,
  id:       string,
...
export function getChain(
  ...
  if (intentOffset !== undefined) {
    ...
    if (intentEnv !== null) results.push(intentEnv);
  }
 ...
 */
export function getArtifactByRunId(
  storeDir: string,
  runId:    string,
  ...
  for (const id of ids) {
    ...
    if (env.type !== "ucp.artifact.v1") continue;

    // Type-narrow: payload has runId because type === "ucp.artifact.v1"
    const payload = env.payload as ArtifactPayload;
    if (payload.runId === runId) return env as ArtifactEnvelope;
  }
 ...
 */
export function getTraceByArtifactId(
  storeDir:       string,
  artifactEnvId:  string,
): ExecutionTraceEnvelope | null {
  const index    = loadIndex(storeDir);
  ...
  for (const id of childIds) {
    ...
    if (env.type === "ucp.execution_trace.v1") {
      return env as ExecutionTraceEnvelope;
    }
  }
 ...
 * Returns null if not found.
 */
export function getReplayByArtifactId(
  storeDir:       string,
  artifactEnvId:  string,
): ReplayReportEnvelope | null {
  const index    = loadIndex(storeDir);
  const childIds = index.byParent[artifactEnvId] ?? [];
  ...
  for (const id of childIds) {
    ...
    if (env.type === "ucp.replay_report.v1") {
      return env as ReplayReportEnvelope;
    }
  }
 ...
 */
export function getReminderExecutionByResponseId(
  storeDir:      string,
  responseEnvId: string,
): ReminderExecutionEnvelope | null {
  const index    = loadIndex(storeDir);
  ...
  for (const id of childIds) {
    ...
    if (env.type === "ucp.reminder_execution.v1") {
      return env as ReminderExecutionEnvelope;
    }
  }
...
export function getTimeline(
  ...
  let intent:            UCPEnvelope<unknown> | null = null;
  let response:          UCPEnvelope<unknown> | null = null;
  let artifact:          ArtifactEnvelope | null     = null;
  let reminderExecution: ReminderExecutionEnvelope | null = null;

  for (const env of chain) {
    switch (env.type) {
      case "ucp.intent.v1":              intent            = env; break;
      case "ucp.response.v1":            response          = env; break;
      case "ucp.artifact.v1":            artifact          = env as ArtifactEnvelope; break;
      case "ucp.reminder_execution.v1":  reminderExecution = env as ReminderExecutionEnvelope; break;
    }
  }
...
// Re-export payload types so callers can type-narrow without importing types.ts directly
