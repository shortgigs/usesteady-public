 ...
 */
export function persistEnvelope(storeDir: string, envelope: UCPEnvelope<unknown>): void {
  try {
    const offset = appendEnvelope(storeDir, envelope);
    updateIndex(storeDir, envelope, offset);
  } catch (err) {
    console.warn(
      `[ucp-persist] Failed to persist envelope id=${envelope.id.slice(0, 8)}… type=${envelope.type}:`,
      err,
    );
  }
}
 ...
 */
export function persistEnvelopeOrThrow(storeDir: string, envelope: UCPEnvelope<unknown>): void {
  const offset = appendEnvelope(storeDir, envelope);
  updateIndex(storeDir, envelope, offset);
}
