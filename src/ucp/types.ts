...
// ─── Imports (for payload type references only) ───────────────────────────────
...
// ─── Envelope type literal union ──────────────────────────────────────────────

export type UCPType =
  // ── Intake / understanding pipeline (Phases 1–2) ───────────────────────────
  | "ucp.intent.v1"
  | "ucp.prv.v1"
  | "ucp.safety.v1"
  | "ucp.context.v1"
  | "ucp.disambiguation.v1"
  | "ucp.completion.v1"
  | "ucp.intent_interpretation.v1"
  | "ucp.change_interpretation.v1"
  | "ucp.response.v1"
  | "ucp.interaction_contract.v1"
  | "ucp.interaction_event.v1"
  | "ucp.debug_trace.v1"
  // ── Post-execution territory (Phase 3) ────────────────────────────────────
  | "ucp.artifact.v1"
  | "ucp.execution_trace.v1"
  | "ucp.replay_report.v1"
  // ── Reminder execution (Phase 6) ─────────────────────────────────────────
  | "ucp.reminder_execution.v1"
  // ── Cursor integration (Phase 8) ─────────────────────────────────────────
  | "ucp.cursor_handoff.v1"    // delivery gate: before Cursor receives the artifact
  | "ucp.cursor_receipt.v1"    // delivery gate: Cursor acknowledged (kind: accepted)
  | "ucp.cursor_refused.v1"    // delivery gate: Cursor refused (scope or execution error)
  // ── Claude Managed Agents integration (Phase 8B) ──────────────────────────
  | "ucp.claude_handoff.v1"    // delivery gate: before Claude receives the artifact
  | "ucp.claude_receipt.v1"    // delivery gate: Claude accepted (session started)
  | "ucp.claude_refused.v1"    // delivery gate: Claude refused (scope or execution error)
  // ── Model advisory positions (P3 Phase 2) ─────────────────────────────────
  | "ucp.model_advisory.v1"    // delivery gate: Claude emitted a structured advisory (parked)
  // ── Model evidence-basis provenance (P4) ──────────────────────────────────
  | "ucp.model_evidence_basis.v1" // delivery gate: system-derived evidence basis for a model challenge
  // ── Model-position retirement (P5 V1) ─────────────────────────────────────
  | "ucp.model_position_retirement.v1" // append-only: O retired by named evidence
  | "ucp.retired_position_reassertion.v1" // recorded resurrection attempt; non-standing
  // ── Outcome verification (P6 V1) ───────────────────────────────────────────
  | "ucp.executor_report.v1"             // X: delivery/execution self-report
  | "ucp.outcome_observation.v1"         // O: independent observation, if any
  | "ucp.outcome_reconciliation.v1"      // R: new fact about X given O
  // ── Structured CLI shadow envelopes (Phase 12A) ───────────────────────────
  | "ucp.command.v1"           // normalized CLI structured command (shadow-only)
  | "ucp.execution_result.v1"  // terminal workflow result projection (shadow-only)
  // ── Multi-session Workflow (Phase 9C) ─────────────────────────────────────
  | "ucp.workflow_run.v1";     // workflow terminal record: links all session envelopes
...
// ─── Payload types ────────────────────────────────────────────────────────────
// Each payload maps EXACTLY from the corresponding source type.
// No fields are invented. No fields may be removed.
...
export type InteractionEventPayload = {
  readonly type:
    ...
    | "unsupported_recovered"
    | "color_intent_observed"
    | "text_intent_observed"
    | "config_intent_observed";
...
// ─── Phase 3 payload types — post-execution territory ────────────────────────
//
// Design principle:
//   Phase 3A answers: "What was produced?"     → ArtifactPayload, ExecutionTracePayload
//   Phase 3B answers: "Was it valid?"           → ReplayReportPayload
//
// These are kept separate because they are epistemically different questions.
// An artifact envelope exists after execution. A replay envelope exists after
// a subsequent verification pass. They link via refs.parentId.
...
// ─── Phase 6 payload type — reminder execution ────────────────────────────────
...
// ─── Phase 8 payload types — Cursor integration ───────────────────────────────
...
// ─── Phase 8B payload types — Claude Managed Agents ─────────────────────────
...
// ─── Phase 9C / 9F payload types — Multi-session Workflow ────────────────────
...
export type WorkflowRunPayload = {
  readonly workflowRunId:  string;
  /**
   * P-MED / F-02 — immutable execution-instance identity. Distinct from
   * `workflowRunId` (spec identity). Absent on pre-slice envelopes;
   * consumers treat missing as `workflowRunId` (legacy single id).
   */
  readonly executionInstanceId?: string;
  /**
   * P-MED / F-02 — monotonic persist ordinal for `--last`. Assigned at
   * persist time (max existing + 1). Absent on legacy envelopes (sort as 0).
   */
  ...
  readonly sessionRefs: readonly {
    ...
    // planning_reviewed is a V1 additive member; earlier typed unions omitted it.
