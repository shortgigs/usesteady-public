 ...
 */
export async function proposeCandidateUnderstanding(
  input: string,
  classify: ClassifyFn = classifyIntent,
): Promise<CandidateUnderstanding | null> {
  let classification;
  try {
  ...
  if (rewrite === undefined || rewrite.trim() === "") return null;
  ...
  if (normalizeNLToIR(rewrite, "prompt").kind !== "ok") return null;
