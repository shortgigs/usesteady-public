 ...
 *   - `run_command` process exit code, stdout/stderr (intrinsically execute-time)
 *
 * Post-M3 widening (R0-B / #1086 — live run-command authority):
 *
 *   - `run_command` command text outside the frozen one-executable-plus-
 *     literal-argv contract → `command_policy_violation` (validate-stage).
 *     This is syntax/authority validation only; process outcome evidence remains
 *     execute-time. The executor repeats the same policy immediately before
 *     process creation as defense in depth.
 *
...
import type { IR, Operation, ReplaceOccurrence } from "./ir.js";
...
} from "../safety/filename-safety.js";
import { findExtensionBearingParentSegment } from "../safety/path-segment-semantics.js";
import { parseAuthorizedRunCommand } from "../safety/run-command-policy.js";
import {
  checkEffectiveResourceForOp,
  type EffectiveResourceFs,
...
export type ValidatorFs = {
  ...
  /** P-F01 — optional so hop tests can inject resolution without OS symlinks. */
  readonly realpathSync?: (path: string) => string;
  /** Entry-aware inspection; resolution failures must not look absent. */
  readonly lstatSync?: EffectiveResourceFs["lstatSync"];
};

const defaultFs: ValidatorFs = {
  existsSync,
...
//   - run.args.command is not a path. R0-B command-policy validation is a
//     separate authority check and independently refuses NUL before approval.
...
function renderPathForMsg(p: string): string {
 ...
 */
function checkPathBytes(op: Operation): Omit<ValidateError, "index"> | null {
  switch (op.type) {
    case "create": {
      if (pathHasNullByte(op.args.path)) return makeInvalidPathError(op, op.args.path, "file");
      return null;
    }
    case "create_dir": {
      if (pathHasNullByte(op.args.path)) return makeInvalidPathError(op, op.args.path, "path");
      return null;
    }
    case "delete": {
      if (pathHasNullByte(op.args.path)) return makeInvalidPathError(op, op.args.path, "file");
    ...
    case "rename": {
      if (pathHasNullByte(op.args.from)) return makeInvalidPathError(op, op.args.from, "from");
      if (pathHasNullByte(op.args.to))   return makeInvalidPathError(op, op.args.to,   "to");
      return null;
    }
    case "replace": {
      if (pathHasNullByte(op.args.file)) return makeInvalidPathError(op, op.args.file, "file");
      return null;
    }
    case "append": {
      if (pathHasNullByte(op.args.file)) return makeInvalidPathError(op, op.args.file, "file");
...
//
// Out of scope:
//   - run.args.command — not a path; it is handled by the separate R0-B
//     literal-argv command policy.
//   - Windows junction / reparse-point hops (ADV-V2-009) — not folded
//     into #36 or P-F01; POSIX/file symlink hops are P-F01.
 ...
 */
function checkEscape(p: string): boolean {
  if (typeof p !== "string" || p.length === 0)        return false;
  if (isAbsolute(p))                                  return true;
  if (DRIVE_ROOTED.test(p))                           return true;
  if (DRIVE_RELATIVE.test(p))                         return true;
  if (p.startsWith(UNC_BACKSLASH))                    return true;
  if (p.startsWith(UNC_FORWARD_SLASH))                return true;
...
function pathEscapesWorkspace(p: string): boolean {
  // Original-form check (existing behavior, preserved verbatim).
  if (checkEscape(p)) return true;
  ...
  try {
    ...
    if (err instanceof CanonicalizationError) return true;
    return true;
  }
  ...
  if (canonical !== p && checkEscape(canonical)) return true;

  return false;
}

const OUTSIDE_WORKSPACE_MSG =
  "Path is outside the workspace. Use a relative path inside the current workspace.";

function makeOutsideWorkspaceError(
  op: Operation,
  path: string,
  field: string,
): Omit<ValidateError, "index"> {
 ...
 * actionable for the human who wrote the op.
 */
function checkPathContainment(op: Operation): Omit<ValidateError, "index"> | null {
  switch (op.type) {
    case "create": {
      if (pathEscapesWorkspace(op.args.path)) return makeOutsideWorkspaceError(op, op.args.path, "file");
      return null;
    }
    case "create_dir": {
      if (pathEscapesWorkspace(op.args.path)) return makeOutsideWorkspaceError(op, op.args.path, "path");
      return null;
    }
    case "delete": {
      if (pathEscapesWorkspace(op.args.path)) return makeOutsideWorkspaceError(op, op.args.path, "file");
    ...
    case "rename": {
      if (pathEscapesWorkspace(op.args.from)) return makeOutsideWorkspaceError(op, op.args.from, "from");
      if (pathEscapesWorkspace(op.args.to))   return makeOutsideWorkspaceError(op, op.args.to,   "to");
      return null;
    }
    case "replace": {
      if (pathEscapesWorkspace(op.args.file)) return makeOutsideWorkspaceError(op, op.args.file, "file");
      return null;
    }
    case "append": {
      if (pathEscapesWorkspace(op.args.file)) return makeOutsideWorkspaceError(op, op.args.file, "file");
...
function makeInvalidFilenameError(
  op: Operation,
  path: string,
  field: string,
  detail: string,
  ...
  const summary =
    validatorCode === "invalid_filename_reserved_name"
  ...
  const code: "invalid_filename_chars" | "invalid_filename_reserved_name" =
    validatorCode === "invalid_filename_reserved_name"
      ? "invalid_filename_reserved_name"
      : "invalid_filename_chars";
  return {
 ...
 *      bidi controls".
 */
function makeProhibitedPathError(
  op: Operation,
  path: string,
  field: string,
  detail: string,
...
function verbForOp(op: Operation): string {
  switch (op.type) {
...
function makeExtensionBearingParentError(
  op: Operation,
  path: string,
  field: string,
  segment: string,
  ...
  return {
    ...
    code: "invalid_path",
    message:
      `Refusing to ${verbForOp(op)}: ${field} has parent directory "${segment}" ` +
...
function checkExtensionBearingParentForOp(
  op: Operation,
  ...
  switch (op.type) {
    case "create": {
      ...
      return makeExtensionBearingParentError(
        op, op.args.path, "file", hit.segment, hit.extension,
      );
    }
    case "create_dir": {
      const hit = findExtensionBearingParentSegment(op.args.path);
      if (hit === null) return null;
      return makeExtensionBearingParentError(
        op, op.args.path, "path", hit.segment, hit.extension,
      );
    }
    case "rename": {
      ...
      if (hit === null) return null;
      return makeExtensionBearingParentError(
        op, op.args.to, "to", hit.segment, hit.extension,
      );
    }
    case "delete":
    case "replace":
...
function checkFilenameSafetyForOp(op: Operation): Omit<ValidateError, "index"> | null {
  switch (op.type) {
    case "create": {
 ...
 *     not commands. `run` is governed by the separate R0-B literal-argv
 *     command-policy authority check.
 */
function checkProtectedPathForOp(op: Operation): Omit<ValidateError, "index"> | null {
  switch (op.type) {
    case "create": {
...
// ─── Per-op validators ───────────────────────────────────────────────────────
//
// One function per op variant with a validate-stage check. Each returns a
// `ValidateError` (without `index`, which the IR walker fills in) or `null`
// if the op is feasible at the validate stage.
//
// Filesystem validators are path-level. R0-B additionally validates the
// static `run_command` authority grammar before approval; process outcome
// evidence remains execute-time.

type PerOpResult = Omit<ValidateError, "index"> | null;

function validateCreate(op: Operation & { type: "create" }, ctx: ValidatorCtxResolved): PerOpResult {
...
function validateCreateDir(op: Operation & { type: "create_dir" }, ctx: ValidatorCtxResolved): PerOpResult {
 ...
 */
function formatRequestedOccurrence(
  occ: ReplaceOccurrence,
): { readonly display: string; readonly machine: string } {
  if (occ === "first") {
 ...
 */
function validateReplace(op: Operation & { type: "replace" }): PerOpResult {
  ...
  return {
    ...
    message:
      `Refusing to apply replace in "${op.args.file}": you requested ` +
...
function validateDelete(op: Operation & { type: "delete" }, ctx: ValidatorCtxResolved): PerOpResult {
  const abs = resolvePath(ctx.workspaceRoot, op.args.path);
...
function validateRename(op: Operation & { type: "rename" }, ctx: ValidatorCtxResolved): PerOpResult {
...
/**
 * R0-B / #1086 — static live run-command authority check.
 *
 * The policy parser does not execute or rewrite anything. It decides only
 * whether the proposal represents one executable plus literal argv. A refusal
 * here prevents preview/approval from granting authority to shell syntax that
 * the executor must not interpret. The adapter repeats this exact decision
 * immediately before process creation.
 */
function validateRun(op: Operation & { type: "run" }): PerOpResult {
  const policy = parseAuthorizedRunCommand(op.args.command);
...
// `append` and `prepend` have NO M3 validator entries:
//
//   - `append` / `prepend` are create-if-missing — no missing-target check
//     by design (rule 4 above).
//   - `run` has no filesystem-feasibility probe, but R0-B adds the static
//     command-authority validation above. Exit code/stdout/stderr remain
//     intrinsically execute-time.
//
// `replace`:
//   - M3: no validator entry. The file_not_found / old_value_not_found /
//     ambiguous_match checks require reading file content — execute-time
//     only (§6.5.1 rule 1). Unchanged in this PR.
//   - usesteady-public#45 (post-M3 widening): `validateReplace` refuses
//     at validate-stage when the IR carries an explicit non-"first"
//     `requestedOccurrence` directive. No file read; the IR field alone
//     is enough to know the executor will not honor the directive.
//     Public `ambiguous_match` reused (§6.5.1 rule 3 / §6.6.1 rule 7).

// ─── IR walker ───────────────────────────────────────────────────────────────

type ValidatorCtxResolved = { readonly workspaceRoot: string; readonly fs: ValidatorFs };

function resolveContext(ctx: ValidatorContext): ValidatorCtxResolved {
 ...
 *
 * The returned error set includes the historical filesystem-feasibility codes
 * plus policy widenings such as `invalid_path` and R0-B's
 * `command_policy_violation`. All are documented in `cli-error.ts`.
 */
export function validateOperation(
  ...
  if (pathBytesError !== null) return pathBytesError;
  ...
  if (extensionParentError !== null) return extensionParentError;
  ...
  if (filenameSafetyError !== null) return filenameSafetyError;
  ...
  if (protectedPathError !== null) return protectedPathError;

  const ctx = resolveContext(context);

  // P-F01 — effective-resource containment. Lexical #36 already passed.
  // Walk existing prefixes; a POSIX/file symlink hop to an outside
  // realpath is outside_workspace. An unreadable/cyclic hop is
  // effective_resource_unresolved. Both refuse before preview / effect.
  // Windows junction/reparse is not claimed closed by this check.
  const effectiveFs: EffectiveResourceFs = {
    lstatSync: ctx.fs.lstatSync ?? lstatSync,
    realpathSync: ctx.fs.realpathSync ?? realpathSync,
  };
  const effectiveError = checkEffectiveResourceForOp(op, ctx.workspaceRoot, effectiveFs);
  ...
  switch (op.type) {
    ...
    case "run":        return validateRun(op);
  }
}
