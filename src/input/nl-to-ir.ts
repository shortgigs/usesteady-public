...
export type NLToIRResult =
  | { readonly kind: "ok"; readonly ir: IR }
  | { readonly kind: "error"; readonly error: NormalizeError };
 ...
 */
export function normalizeNLToIR(
  input: string,
  surface: NLSurface,
): NLToIRResult {
  const raw = input;
  ...
  if (trimmed.length === 0) {
    return normalizeErr(
      "parse_error",
      "empty input: natural-language request is required",
      irSurface,
      raw,
    );
  }
  ...
  if (containsCompoundConjunction(impolite)) {
    return normalizeErr(
      "parse_error",
      "compound NL not supported — use batch or --json for multi-op inputs",
      irSurface,
      raw,
    );
  }
 ...
 */
export function detectDuplicatePromptFlag(
...
function toIRSourceSurface(surface: NLSurface): IRSourceSurface {
...
const POLITE_PREFIX_RE =
  /^(?:please\s+|can\s+you\s+|could\s+you\s+|i\s+(?:need|want)\s+(?:you\s+)?to\s+|would\s+you\s+)/i;
 ...
 */
function containsCompoundConjunction(text: string): boolean {
  let inDouble = false;
  let inSingle = false;
...
type DispatchResult =
  | { readonly kind: "ok"; readonly op: Operation }
  | {
      readonly kind: "error";
      readonly code: NormalizeError["code"];
      readonly message: string;
    };

function dispatchToOp(text: string): DispatchResult {
  // Order matters:
  ...
  if (asReplace !== null) return asReplace;

  const asRename = matchRename(text);
  if (asRename !== null) return asRename;

  const asCreateDir = matchCreateDir(text);
  if (asCreateDir !== null) return asCreateDir;

  const asCreate = matchCreate(text);
  if (asCreate !== null) return asCreate;

  const asDelete = matchDelete(text);
  if (asDelete !== null) return asDelete;

  const asAppend = matchAppend(text);
  if (asAppend !== null) return asAppend;

  const asPrepend = matchPrepend(text);
  if (asPrepend !== null) return asPrepend;

  const asRun = matchRun(text);
  if (asRun !== null) return asRun;

  const asFix = matchFix(text);
  if (asFix !== null) return asFix;
  ...
  const bareVerb = text.match(
    /^(replace|change|update|set|rename|mv|mkdir|touch|create|delete|rm|remove|append|prepend|run)\s*$/i,
  );
  if (bareVerb) {
    return {
      kind: "error",
      code: "parse_error",
      message:
        `bare verb "${bareVerb[1]}" with no target — provide the full NL request ` +
  ...
  return {
    ...
    code: "parse_error",
    message:
      "input did not match any supported NL shape. " +
 ...
 */
function matchReplace(text: string): DispatchResult | null {
  ...
  const patterns: RegExp[] = [
    new RegExp(
      `^replace\\s+"([^"]+)"\\s+with\\s+"([^"]*)"\\s+in\\s+(${PATH_TOKEN_RE})${occurrenceTail}\\s*$`,
      "i",
    ),
    new RegExp(
      `^replace\\s+'([^']+)'\\s+with\\s+'([^']*)'\\s+in\\s+(${PATH_TOKEN_RE})${occurrenceTail}\\s*$`,
    ...
    new RegExp(
      `^change\\s+"([^"]+)"\\s+to\\s+"([^"]*)"\\s+in\\s+(${PATH_TOKEN_RE})${occurrenceTail}\\s*$`,
      "i",
    ),
    new RegExp(
      `^change\\s+'([^']+)'\\s+to\\s+'([^']*)'\\s+in\\s+(${PATH_TOKEN_RE})${occurrenceTail}\\s*$`,
    ...
    new RegExp(
      `^(?:update|set)\\s+"([^"]+)"\\s+to\\s+"([^"]*)"\\s+in\\s+(${PATH_TOKEN_RE})${occurrenceTail}\\s*$`,
      "i",
    ),
    new RegExp(
      `^(?:update|set)\\s+'([^']+)'\\s+to\\s+'([^']*)'\\s+in\\s+(${PATH_TOKEN_RE})${occurrenceTail}\\s*$`,
    ...
    new RegExp(
      `^replace\\s+([^\\s"'\`]+)\\s+with\\s+([^\\s"'\`]+)\\s+in\\s+(${PATH_TOKEN_RE})${occurrenceTail}\\s*$`,
      "i",
    ),
    new RegExp(
      `^(?:change|update|set)\\s+([^\\s"'\`]+)\\s+to\\s+([^\\s"'\`]+)\\s+in\\s+(${PATH_TOKEN_RE})${occurrenceTail}\\s*$`,
      "i",
    ),
  ];
  ...
  for (const re of patterns) {
    ...
    if (from.length === 0) {
      ...
      return {
        ...
        code: "parse_error",
        message:
          "replace \"from\" value is empty — replacing an empty string is undefined",
      };
    }
    ...
    if (occurrence === null) {
      ...
      return {
        ...
        message:
          "replace requires an explicit occurrence clause — " +
 ...
 */
function parseOccurrenceClause(clause: string): ReplaceOccurrence | null {
  ...
  const digitOrd = s.match(
    /^(\d+)(?:st|nd|rd|th)(?:\s+(?:occurrence|instance))?$/,
  );
  if (digitOrd) {
  ...
  const wordOrdinals: Readonly<Record<string, number>> = {
    second: 2,
    third: 3,
    fourth: 4,
    fifth: 5,
  ...
  const wordOrd = s.match(
    /^(second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth)(?:\s+(?:occurrence|instance))?$/,
  );
  if (wordOrd) {
...
function matchRename(text: string): DispatchResult | null {
  ...
  const withTo = text.match(
    new RegExp(
      `^(?:rename|mv|move)\\s+${typeNoun}(${PATH_TOKEN_RE})\\s+to\\s+(${PATH_TOKEN_RE})\\s*$`,
      "i",
    ),
  );
  ...
  const withoutTo = !withTo
    ? text.match(
        new RegExp(`^mv\\s+(${PATH_TOKEN_RE})\\s+(${PATH_TOKEN_RE})\\s*$`, "i"),
      )
    : null;
...
function matchCreateDir(text: string): DispatchResult | null {
  ...
  const m = text.match(
    new RegExp(
      `^(?:mkdir|create\\s+(?:folder|directory|dir))\\s+(${PATH_TOKEN_RE})\\s*$`,
      "i",
    ),
  );
  if (!m) return null;
 ...
 */
const VAGUE_DELETE_TARGETS: ReadonlySet<string> = new Set([
...
function isVagueDeleteTarget(path: string): boolean {
  const normalized = path.trim().toLowerCase().replace(/\s+/g, " ");
  return VAGUE_DELETE_TARGETS.has(normalized);
}

function matchDelete(text: string): DispatchResult | null {
  ...
  const m = text.match(
    new RegExp(
      `^(?:delete|rm|remove)\\s+(?:${articleNoun})?(${PATH_TOKEN_RE})\\s*$`,
      "i",
    ),
  );
  if (!m) return null;
  ...
  if (isVagueDeleteTarget(path)) {
    ...
    return {
      ...
      code: "parse_error",
      message:
        `delete target "${path}" is too vague — UseSteady cannot approve a ` +
...
function matchAppend(text: string): DispatchResult | null {
  return matchAppendOrPrepend(text, "append");
}

function matchPrepend(text: string): DispatchResult | null {
  return matchAppendOrPrepend(text, "prepend");
}

function matchAppendOrPrepend(
  text: string,
  verb: "append" | "prepend",
): DispatchResult | null {
 ...
 */
const VAGUE_OPERATIONAL_TARGETS: ReadonlySet<string> = new Set([
...
function isVagueOperationalTarget(target: string): boolean {
 ...
 */
const KNOWN_RUN_TOOLS: ReadonlySet<string> = new Set([
 ...
 */
function isCommandShaped(command: string): boolean {
  ...
  if (/^\.{0,2}[\\/]/.test(firstToken) || firstToken.includes("/")) return true;
  return KNOWN_RUN_TOOLS.has(firstToken.toLowerCase());
}

// ── run ─────────────────────────────────────────────────────────────────────

function matchRun(text: string): DispatchResult | null {
  ...
  if (isVagueOperationalTarget(command)) {
    return {
      ...
      code: "parse_error",
      message:
        `run target "${command}" is too vague — name the concrete command to run ` +
  ...
  if (!isCommandShaped(command)) {
    return {
      ...
      code: "parse_error",
      message:
        `run target "${command}" is not a recognizable command — start with the ` +
 ...
 */
function matchFix(text: string): DispatchResult | null {
  ...
  if (isVagueOperationalTarget(target)) {
    return {
      ...
      code: "parse_error",
      message:
        `fix target "${target}" is too vague — name the concrete thing to fix ` +
