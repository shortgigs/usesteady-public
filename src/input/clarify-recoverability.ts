...
const TYPE_NOUNS: ReadonlySet<string> = new Set([
  "file",
  "folder",
  "directory",
  "dir",
 ...
 */
const VAGUE_TARGETS: ReadonlySet<string> = new Set([
...
function isVague(token: string): boolean {
...
const MISSING_DEST_RE = new RegExp(
  `^(?:rename|mv|move)\\s+(?:(?:file|folder|directory|dir)\\s+)?(${TOKEN})\\s*$`,
  "i",
);
 ...
 */
export function classifyClarifyRecoverability(
  rawInput: string,
): ClarifyRecoverability {
  const trimmed = rawInput.trim();
  if (trimmed.length === 0) return { kind: "not_recoverable" };
 ...
 */
function normalizeTypeAnswer(answer: string): "file" | "folder" | null {
  const a = answer.trim().toLowerCase();
  if (a === "file" || a === "f") return "file";
 ...
 */
export function reconstructClarifiedInput(
  rec: ClarifyRecoverability,
  answer: string,
): string | null {
  switch (rec.kind) {
