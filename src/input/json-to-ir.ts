...
import type { IR, IRSourceSurface, Operation, ReplaceOccurrence } from "./ir.js";
import { isKnownOperationType } from "./op-registry.js";
import { PUBLIC_JSON_SCHEMA } from "./public-json-schema.js";

// ─── Occurrence parser (usesteady-public#45 / JSON surface) ──────────────────
//
// Parse the optional `occurrence` field on a public JSON replace op into
// the IR shape `ReplaceOccurrence`. Returns a discriminated result so the
// caller can distinguish:
//   - absent      → no explicit directive (downstream IR keeps the
//                   adapter-side default of "all" for §5.1 compliance).
//   - parsed      → user provided a recognized shape; the value is the
//                   user's explicit directive.
//   - invalid     → field is present but malformed (e.g. wrong type,
//                   non-positive index, unknown string). Rejected at the
//                   adapter so the JSON op fails to convert (the caller
//                   short-circuits per the existing `null`-return
//                   contract). Same posture as any other malformed field
//                   on the public schema.
type OccurrenceParse =
  | { readonly kind: "absent" }
  | { readonly kind: "parsed"; readonly value: ReplaceOccurrence }
  | { readonly kind: "invalid" };

function parseJsonOccurrence(raw: unknown): OccurrenceParse {
...
export type JsonToIRResult =
  ...
  | { readonly kind: "error"; readonly index: number; readonly raw: unknown };
...
function strField(op: Record<string, unknown>, name: string): string | undefined {
  const v = op[name];
 ...
 *
 *   - ADV-V2-037: optional create.contents is preserved exactly. Known
 *     unsupported content/to aliases and malformed contents refuse.
 *
 ...
 */
export function jsonOpToIROperation(raw: unknown): Operation | null {
  ...
  if (!isKnownOperationType(typeRaw)) return null;
  ...
  switch (typeRaw) {
    ...
    case "replace": {
      if (from === undefined || to === undefined || file === null || file === undefined) return null;
      ...
      if (occParse.kind === "invalid") return null;
      const occurrence: ReplaceOccurrence =
        occParse.kind === "parsed" ? occParse.value : "all";
      const requestedOccurrence: ReplaceOccurrence | undefined =
        occParse.kind === "parsed" ? occParse.value : undefined;
      return {
        type: "replace",
        args: {
          file,
    ...
    case "create": {
      ...
      // Do not silently discard a caller's content directive.
      if (Object.hasOwn(op, "content") || Object.hasOwn(op, "to")) return null;
      if (Object.hasOwn(op, "contents") && typeof op["contents"] !== "string") return null;
      return {
        type: "create",
        args: { path: file, ...(Object.hasOwn(op, "contents") ? { contents: op["contents"] as string } : {}) },
      };
    }
    ...
    case "run": {
      if (to === undefined || to.trim().length === 0) return null;
      return { type: "run", args: { command: to.trim() } };
    }

    case "append": {
      if (to === undefined || file === null || file === undefined) return null;
 ...
 */
export function buildIRFromJsonOps(
  ops: readonly unknown[],
  rawSource: string,
  surface: IRSourceSurface & ("json" | "batch"),
): JsonToIRResult {
 ...
 */
export function diagnoseInvalidJsonOp(raw: unknown): string | null {
  ...
  if (typeRaw === "create") {
    if (Object.hasOwn(op, "content") || Object.hasOwn(op, "to")) {
      return 'op type "create" does not accept "content" or "to"; use string field "contents".';
    }
    if (Object.hasOwn(op, "contents") && typeof op["contents"] !== "string") {
      return 'op type "create" requires "contents" to be a string when provided.';
    }
  }
...
function formatMissingFieldMessage(opType: string, fieldName: string): string {
  return (
    `op type "${opType}" requires field "${fieldName}" but it was not provided. ` +
    `See \`usesteady capabilities\` for the full per-op JSON schema.`
  );
}

function formatNonStringFieldMessage(
  opType: string,
  fieldName: string,
  actualType: string,
): string {
  return (
    `op type "${opType}" requires field "${fieldName}" to be a string, ` +
