 ...
 */
const VAGUE_NAMES: ReadonlySet<string> = new Set([
 ...
 */
const DESTINATION_TYPE_NOUN =
  /\b(?:to|into|in|inside|within|onto|from)\s+(?:(?:the|a|an|this|that|my|our|your)\s+)?(?:folders?|director(?:y|ies)|dirs?|files?)\b/i;
 ...
 */
export function canonicalizeNearMiss(text: string): string {
  const trimmed = text.trim();
  if (trimmed.length === 0) return text;

  if (!CREATE_VERB.test(trimmed)) return text;
  ...
  if (DESTINATION_TYPE_NOUN.test(trimmed)) return text;
 ...
 */
function extractSingleName(text: string): string | null {
...
function sanitizeName(raw: string): string | null {
  ...
  if (!PATH_SAFE_TOKEN.test(t)) return null;
  if (t.includes("..")) return null; // no path traversal

  const low = t.toLowerCase();
  if (VAGUE_NAMES.has(low) || NOISE_WORDS.has(low)) return null;
