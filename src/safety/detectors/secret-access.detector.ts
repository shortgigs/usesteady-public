...
type LabeledPattern = { readonly regex: RegExp; readonly label: string };
 ...
 */
const ROTATION_MGMT_RE =
  /\b(rotat\w*|renew(?:al|ing|ed)?|reissue\w*|regenerat\w*|revok\w*|invalidat\w*|deactivat\w*|expir(?:e|ed|ing|ation)?|cycl(?:e|ed|ing)|roll(?:(?:\s+(?:over|back))?(?:ing|ed)?))\b/i;
 ...
 */
const DISCLOSURE_VERBS_RE =
  /\b(read|show|dump|print|expos\w*|reveal|output|display)\b/i;
 ...
 */
function isRotationWithoutDisclosure(input: string): boolean {
...
export const secretAccessDetector: SafetyDetector = {
  ...
  detect(input: string): SafetyResult | null {
    ...
    if (isRotationWithoutDisclosure(input)) return null;
