/**
 * POST /api/executor/replay-worker-io/metadata/:job_id — single-path metadata I/O only.
...
import {
  ...
  validateReplayNamespacePath,
} from "../../replay-workers/io/metadata/index.js";
import {
...
export const EXECUTOR_REPLAY_WORKER_IO_METADATA_ROUTE_PREFIX =
  "/api/executor/replay-worker-io/metadata";

const SIDE_EFFECT_SCOPE_FOR_REDERIVE = "descriptive_log" as const;
const MUTATION_SCOPE_FOR_REDERIVE = "replay_namespace_single_path_metadata" as const;
...
function parseReplayNamespacePath(body: unknown): string | undefined {
  if (body === null || typeof body !== "object") return undefined;
  const raw = (body as Record<string, unknown>)["replay_namespace_path"];
  return typeof raw === "string" ? raw : undefined;
}

function parseMetadataPatch(body: unknown): unknown {
  if (body === null || typeof body !== "object") return undefined;
  return (body as Record<string, unknown>)["metadata_patch"];
}
...
function handleReplayWorkerIoMetadata(req: Request, res: Response): void {
  ...
  const replay_namespace_path = parseReplayNamespacePath(req.body);
  if (replay_namespace_path === undefined) {
    res.status(422).json(
      failureEnvelope(
        "path_missing",
        "replay_namespace_path is required in the request body.",
      ),
    );
    return;
  }

  if (Array.isArray((req.body as Record<string, unknown>)?.["replay_namespace_paths"])) {
    res.status(422).json(
      failureEnvelope(
        "path_batch_forbidden",
        "Multiple paths (replay_namespace_paths) are not permitted — exactly one replay_namespace_path.",
      ),
    );
    return;
  }

  const pathCheck = validateReplayNamespacePath(replay_namespace_path);
  if (!pathCheck.ok) {
    res.status(422).json(
      failureEnvelope(pathCheck.cause, pathCheck.reason),
    );
    return;
  }

  const metadata_patch = parseMetadataPatch(req.body);
  const patchCheck = validateMetadataPatch(metadata_patch);
  if (!patchCheck.ok) {
    res.status(422).json(
      failureEnvelope(patchCheck.cause, patchCheck.reason),
    );
    return;
  }
  ...
  const io = updateReplayNamespaceSinglePathMetadata({
    replay_worker_per_scope_io_eligibility,
    operator_confirmation: true,
    replay_namespace_path,
    metadata_patch: patchCheck.patch,
  });

  res.status(200).json({ ok: true, io });
}

export function registerExecutorReplayWorkerIoMetadataRoute(app: Express): void {
  app.post(
    `${EXECUTOR_REPLAY_WORKER_IO_METADATA_ROUTE_PREFIX}/:job_id`,
    handleReplayWorkerIoMetadata,
  );
}
