/**
 * POST /api/executor/replay-worker-io/trace/:job_id — trace-line append I/O only.
...
import {
  ...
  validateReplayTraceTarget,
  validateTraceLine,
} from "../../replay-workers/io/trace/index.js";
import {
...
export const EXECUTOR_REPLAY_WORKER_IO_TRACE_ROUTE_PREFIX =
  "/api/executor/replay-worker-io/trace";

const SIDE_EFFECT_SCOPE_FOR_REDERIVE = "descriptive_log" as const;
const MUTATION_SCOPE_FOR_REDERIVE = "replay_namespace_trace_line_append" as const;
...
function parseReplayTraceTarget(body: unknown): string | undefined {
  if (body === null || typeof body !== "object") return undefined;
  const raw = (body as Record<string, unknown>)["replay_trace_target"];
  return typeof raw === "string" ? raw : undefined;
}

function parseTraceLine(body: unknown): unknown {
  if (body === null || typeof body !== "object") return undefined;
  return (body as Record<string, unknown>)["trace_line"];
}
...
function handleReplayWorkerIoTrace(req: Request, res: Response): void {
  ...
  const replay_trace_target = parseReplayTraceTarget(req.body);
  if (replay_trace_target === undefined) {
    res.status(422).json(
      failureEnvelope(
        "target_missing",
        "replay_trace_target is required in the request body.",
      ),
    );
    return;
  }

  if (Array.isArray((req.body as Record<string, unknown>)?.["replay_trace_targets"])) {
    res.status(422).json(
      failureEnvelope(
        "target_batch_forbidden",
        "Multiple targets (replay_trace_targets) are not permitted — exactly one replay_trace_target.",
      ),
    );
    return;
  }

  const targetCheck = validateReplayTraceTarget(replay_trace_target);
  if (!targetCheck.ok) {
    res.status(422).json(
      failureEnvelope(targetCheck.cause, targetCheck.reason),
    );
    return;
  }

  const trace_line = parseTraceLine(req.body);
  const lineCheck = validateTraceLine(trace_line);
  if (!lineCheck.ok) {
    res.status(422).json(
      failureEnvelope(lineCheck.cause, lineCheck.reason),
    );
    return;
  }
  ...
  const result = appendReplayNamespaceTraceLine({
    replay_worker_per_scope_io_eligibility,
    operator_confirmation: true,
    replay_trace_target,
    trace_line: lineCheck.line,
  });

  res.status(200).json({
    ok: true,
    io: result.io,
    trace_line_id: result.trace_line_id,
  });
}

export function registerExecutorReplayWorkerIoTraceRoute(app: Express): void {
  app.post(
    `${EXECUTOR_REPLAY_WORKER_IO_TRACE_ROUTE_PREFIX}/:job_id`,
    handleReplayWorkerIoTrace,
  );
}
