/**
 * POST /api/executor/replay-worker/:job_id — descriptive replay worker dispatch only.
...
export const EXECUTOR_REPLAY_WORKER_ROUTE_PREFIX =
  "/api/executor/replay-worker";
...
function handleReplayWorkerDispatch(req: Request, res: Response): void {
  ...
  if (operator_confirmation !== true) {
    res.status(422).json(
      failureEnvelope(
        ...
        "Replay worker dispatch requires operator_confirmation: true in the request body.",
      ),
    );
    return;
  }
  ...
  const dispatch = dispatchReplayWorker({
    job_id,
    replay_worker_eligibility,
    operator_confirmation: true,
  });

  res.status(200).json({ ok: true, dispatch });
}

export function registerExecutorReplayWorkerRoute(app: Express): void {
  app.post(
    `${EXECUTOR_REPLAY_WORKER_ROUTE_PREFIX}/:job_id`,
    handleReplayWorkerDispatch,
  );
}
