...
const CONTEXT_DEPENDENCY_PATTERNS: ReadonlyArray<RegExp> = [
  /\bagain\b/i,
  /\bcontinue\b/i,
  /\bprevious\b/i,
...
export function requiresContext(input: string): boolean {
  const stripped = stripQuotedContent(input);
  return CONTEXT_DEPENDENCY_PATTERNS.some((pattern) => pattern.test(stripped));
}

export function hasRequiredContext(ctx: { readonly hasPriorSession: boolean }): boolean {
