...
import { PROVIDERS, type AIProvider } from "../config/providers.js";
 ...
 */
export function isProviderLive(p: AIProvider): boolean {
  if (!p.liveModeEnv) return true;
...
export function runtimeToProviderId(runtime: string): string | null {
  for (const p of PROVIDERS) {
    if (p.runtimes.includes(runtime)) return p.id;
  }
  return null;
}
 ...
 */
export function buildRequiredProviderIds(
 ...
 */
export function getProviderStatuses(): readonly ProviderStatusDto[] {
