...
const COLOR_WORDS_RE =
  /\b(blue|red|green|yellow|orange|purple|pink|black|white|gray|grey|teal|indigo|violet|cyan|magenta|coral|slate|stone|zinc|neutral|amber|lime|emerald|sky|rose|darker|lighter|dark|light|colors?|colours?)\b/i;

const UI_COLOR_TARGETS_RE =
  /\b(button|background|header|footer|nav|sidebar|icon|badge|label|link|card|chip|tag|border|text|heading|banner|hero|modal|overlay|avatar)\b/i;
...
export const colorIntentInterpreter: IntentInterpreter = {
  id: "color_intent",
  priority: 20,

  matches(input): boolean {
    if (DARK_MODE_GUARD_RE.test(input)) return false;
    return COLOR_WORDS_RE.test(input);
  },

  interpret(input): IntentInterpretation | null {
    if (DARK_MODE_GUARD_RE.test(input)) return null;
    const colorMatch = COLOR_WORDS_RE.exec(input);
    if (colorMatch === null) return null;

    const basis: string[] = [`matched color word: ${colorMatch[1]}`];
