 ...
 */
const COMMIT_SIGNAL_RE =
  /\b(git\s+commit|save\s+(?:my\s+)?changes|commit)\b/i;
 ...
 */
const COMMIT_NOUN_CONTEXT_RE =
  /\b(?:show|view|see|check|inspect|display|list|log)\b.{0,30}\bcommits?\b|\b(?:last|latest|recent|previous|prior|earliest|newest|oldest)\s+commits?\b|\bcommit\s+(?:history|log|message|hash|id|sha|diff)\b/i;
 ...
 */
const SEQUENTIAL_SIGNAL_RE =
  /\b(?:and\s+then|then\s+(?:commit|deploy|push|build|test)|(?:build|test|run|compile|check|verify|clean|lint)\s+and\s+(?:then\s+)?(?:commit|deploy|push|build|test|clean|remove|delete))\b/i;

export const workflowIntentInterpreter: IntentInterpreter = {
  id: "workflow_intent",
  priority: 5,
