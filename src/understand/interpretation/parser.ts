...
function hasCompoundSuffix(input: string, matchEnd: number): boolean {
...
const REPLACE_PATTERN =
  /^\s*replace\s+"([^"]+)"\s+with\s+"([^"]*)"\s+in\s+("[^"]+"|'[^']+'|\S+)/i;

const PATCH_PATTERN =
  /^\s*patch\s+("[^"]+"|'[^']+'|\S+)\s+search="([^"]+)"\s+replace="([^"]*)"/i;

// ─── Filesystem operation patterns ───────────────────────────────────────────

// create (folder|directory) <path>
const CREATE_DIR_PATTERN =
  /^\s*create\s+(?:folder|directory)\s+("[^"]+"|'[^']+'|\S+)/i;

// create file <path>
const CREATE_FILE_PATTERN =
  /^\s*create\s+file\s+("[^"]+"|'[^']+'|\S+)/i;

// rename <path> to <newPath>
const RENAME_PATTERN =
  /^\s*rename\s+("[^"]+"|'[^']+'|\S+)\s+to\s+("[^"]+"|'[^']+'|\S+)/i;

// (delete|remove) file|folder|directory <path>  — explicit keyword, any path token
const DELETE_WITH_KEYWORD_PATTERN =
  /^\s*(?:delete|remove)\s+(?:file|folder|directory)\s+("[^"]+"|'[^']+'|\S+)/i;
...
const DELETE_BARE_PATTERN =
  /^\s*(?:delete|remove)\s+((?:"[^"]+"|'[^']+'|\S+))/i;
...
export function parseChange(rawInput: string): ParsedChange | null {
  ...
  if (COMPOUND_SUFFIX_RE.test(input)) return null;
  ...
  if (intent !== null) return intentToChange(intent);
  ...
  if (replaceMatch !== null) {
    if (hasCompoundSuffix(input, replaceMatch[0].length)) return null;
  ...
  if (patchMatch !== null) {
    if (hasCompoundSuffix(input, patchMatch[0].length)) return null;
  ...
  if (createDirMatch !== null) {
    if (hasCompoundSuffix(input, createDirMatch[0].length)) return null;
  ...
  if (createFileMatch !== null) {
    if (hasCompoundSuffix(input, createFileMatch[0].length)) return null;
  ...
  if (renameMatch !== null) {
    if (hasCompoundSuffix(input, renameMatch[0].length)) return null;
    const filePath = stripWrappingQuotes(renameMatch[1]!);
    const newPath = stripWrappingQuotes(renameMatch[2]!);
    if (filePath.length === 0 || newPath.length === 0) return null;
  ...
  if (deleteKwMatch !== null) {
    if (hasCompoundSuffix(input, deleteKwMatch[0].length)) return null;
  ...
  if (deleteBareMatch !== null) {
    if (hasCompoundSuffix(input, deleteBareMatch[0].length)) return null;
