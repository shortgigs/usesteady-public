...
const NON_LITERAL_PATTERNS: ReadonlyArray<RegExp> = [
...
const CONTEXT_REFERENCE_PATTERNS: ReadonlyArray<RegExp> = [
...
export function runContextAlignment(
  input: string,
  ...
  if (CONTEXT_REFERENCE_PATTERNS.some((p) => p.test(input))) {
    if (!ctx.hasPriorSession) {
      return {
        kind: "hard_mismatch",
        reason:
          "Input references a prior session or result, but no prior context is available.",
      };
    }
  }
