...
const OVERLOADED_TERMS: ReadonlyArray<OverloadedTermEntry> = [
...
function findMatch(input: string): OverloadedTermEntry | null {
  return OVERLOADED_TERMS.find((entry) => entry.pattern.test(input)) ?? null;
}

export const overloadedTermDetector: AmbiguityDetector = {
  id: "overloaded_term",
  priority: 20,
