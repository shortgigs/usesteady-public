...
const INVESTIGATION_RE =
  /\b(?:debug|bug|repro(?:duce)?|flak[iy]|smoke[\s-]test|mock(?:ing)?|test\s+(?:coverage|fixtures?|cases?)|coverage\s+report(?:ing)?|analyz\w+|monitor|verify|inspect|trac(?:e|ing)|diagnos\w+|troubleshoot\w*)\b/i;
 ...
 */
const OPERATION_RE =
  /\b(?:deploy(?:ment)?|rollback|restart|reboot|pipeline|migrat(?:e|ions?)|health[\s-]check|launch|roll[\s-]out|export|sync|add\s+(?:a\s+|an\s+)?(?:column|index)|update\s+the\s+schema|schema\s+change|run\s+(?:the\s+)?(?:pipeline|job|server|service|migrations?)|optimize\s+(?:the\s+)?(?:query|queries|slow\s+query)|scale\s+(?:the\s+)?(?:service|app|cluster|workers?|fleet|replicas?|pods?|instances?|nodes?)|suspend|assign|escalat\w+|merge|lock|rotat\w+\s+(?:(?:\w[\w-]*)\s+){0,3}(?:keys?|secrets?|credentials?|tokens?|certs?|certificates?|api[\s-]keys?)|upgrade|provision\w*|failover|drain|patch)\b/i;

/**
 * Copy, UX text, product descriptions, taglines, user flows.
 * Only high-signal content/product vocabulary — avoids over-matching.
 */
const CONTENT_ITERATION_RE =
  /\b(tagline\b|compelling\b|testimonial(?:s)?\b|onboarding\b|user[\s-]flow\b|checkout[\s-]flow\b|product[\s-]description\b|improve\s+the\s+(?:user|product|content|copy|experience|flow|ux)\b|make\b.{0,25}\bmore\s+(?:compelling|engaging)\b)\b/i;
 ...
 */
const BOUNDARY_RE =
  /\b(?:redesign|rearchitect)\b|\brewrite\s+(?:the|all|everything|from\s+scratch)\b|\b(?:write|add|generate)\s+(?:unit\s+)?(?:tests?|specs?|test\s+cases?)\s+(?:for|in|to)\b|\b(?:implement|build|set\s+up|scaffold)\s+(?:a|an|the)\b|\badd\s+a\s+new\s+\w+|\bmake\s+(?:it|the|my|everything)\s+(?:faster|better|smarter|more\s+efficient|more\s+performant)\b/i;
 ...
 */
export function selectSilentGuidanceMode(input: string): SilentGuidanceMode {
  if (INVESTIGATION_RE.test(input))     return "investigation";
  if (OPERATION_RE.test(input))         return "operation";
  if (CONTENT_ITERATION_RE.test(input)) return "content_iteration";
  if (BOUNDARY_RE.test(input))          return "boundary";
  return "unknown";
}
