...
function isCompound(input: string): boolean {
  return COMPOUND_RE.test(input);
}

import type { CompletionRule, CompletionResult } from "./types.js";
 ...
 */
export const rerunPreviousRule: CompletionRule = {
  id: "rerun_previous",
  priority: 5,
  matches(input, ctx): boolean {
    return (
      /\b(run\s+again|redo|repeat\s+last)\b/i.test(input) &&
      ctx.hasPriorSession === true &&
      ctx.lastInput !== undefined &&
      ctx.lastInput.length > 0
    );
  },
  evaluate(_input, ctx): CompletionResult | null {
    if (!ctx.hasPriorSession || !ctx.lastInput) return null;
...
export const runTestsRule: CompletionRule = {
  id: "run_tests",
  priority: 21,
...
export const readFileRule: CompletionRule = {
  id: "read_file",
  priority: 22,
  matches(input): boolean {
...
export const replaceExactRule: CompletionRule = {
  id: "replace_exact",
  priority: 23,
  matches(input): boolean {
    if (isCompound(input)) return false;
...
export const commitWithMessageRule: CompletionRule = {
...
export const patchStructuredRule: CompletionRule = {
  id: "patch_structured",
  priority: 25,
  matches(input): boolean {
    if (isCompound(input)) return false;
...
export const gitOpsRule: CompletionRule = {
  id: "git_ops",
  priority: 26,
  matches(input): boolean {
 ...
 */
export const renameExactRule: CompletionRule = {
  id: "rename_exact",
  priority: 27,
  matches(input): boolean {
    if (isCompound(input)) return false;
    ...
    return (
      /^\s*(?:rename|move)\s+(?:the\s+(?:file|folder|directory)\s+)?\S+\s+to\s+\S+/i.test(input) ||
 ...
 */
export const createFileRule: CompletionRule = {
  id: "create_file",
  priority: 28,
  matches(input): boolean {
    if (isCompound(input)) return false;
 ...
 */
export const createDirRule: CompletionRule = {
  id: "create_dir",
  priority: 29,
  matches(input): boolean {
    if (isCompound(input)) return false;
 ...
 */
export const deleteExactRule: CompletionRule = {
  id: "delete_exact",
  priority: 30,
  matches(input): boolean {
    if (isCompound(input)) return false;
    return (
      /^\s*(?:delete|remove)\s+(?:file|folder|directory)\s+\S+/i.test(input) ||
 ...
 */
export const runCommandRule: CompletionRule = {
  id: "run_command",
  priority: 33,
  matches(input): boolean {
    // Compound workflow ("run tests and then commit") → stays in guide mode
    if (/\b(and\s+then|then\s+\w|followed\s+by)\b/i.test(input)) return false;
    // Context-dependent re-run phrases ("run again", "run it") → handled by rerunPreviousRule
    if (/^\s*run\s+(?:again|that|it|this|same)\s*$/i.test(input)) return false;
    ...
    if (/^\s*run\s+(?:the|a|an|my|our|all)\s/i.test(input)) return false;
 ...
 */
export const changeCssClassMissingFileRule: CompletionRule = {
 ...
 */
export const commitMissingMessageRule: CompletionRule = {
 ...
 */
export const showLatestChangeRule: CompletionRule = {
 ...
 */
export const renameMissingTargetRule: CompletionRule = {
 ...
 */
export const deleteBareNounRule: CompletionRule = {
  id: "delete_bare_noun",
  priority: 54,
  matches(input): boolean {
    // Matches delete/remove verb but is NOT caught by deleteExactRule
    return (
      /^\s*(?:delete|remove)\b/i.test(input) &&
 ...
 */
export const runNoCommandRule: CompletionRule = {
  id: "run_no_command",
  priority: 56,
  matches(input): boolean {
...
export const vagueStyleChangeRule: CompletionRule = {
  id: "vague_style_change",
  priority: 75,
  matches(input): boolean {
    if (DARK_MODE_COMPOUND_RE.test(input)) return false;
    if (CSS_CLASS_TOKEN_RE.test(input))    return false;
 ...
 */
export const vaguePatchRule: CompletionRule = {
  id: "vague_patch",
  priority: 80,
  matches(input): boolean {
 ...
 */
export const vagueTaskRule: CompletionRule = {
  id: "vague_task",
  priority: 90,
  matches(input): boolean {
 ...
 */
export const fixVagueRule: CompletionRule = {
  id: "fix_vague",
  priority: 91,
  matches(input): boolean {
...
export const boundaryRule: CompletionRule = {
  id: "boundary",
  priority: 95,
  matches(input): boolean {
    return (
...
export const ALL_COMPLETION_RULES: ReadonlyArray<CompletionRule> = [
