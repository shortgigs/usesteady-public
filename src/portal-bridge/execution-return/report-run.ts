...
import { buildExecutionReturnPayload, type ExecutionReturnInput } from "./build-payload.js";
import { resolvePortalReporting, type PortalReportingDisabledReason } from "./opt-in.js";
import { sendExecutionReturn, type SendResult } from "./transport.js";
import type {
  ExecutionReturnDecisionSummary,
  ExecutionReturnDeliveryReportSummary,
  ExecutionReturnOutcomeVerification,
} from "./types.js";
 ...
 */
export function outcomeVerificationFromTasks(
  verifications: readonly (OutcomeVerification | undefined)[],
): ExecutionReturnOutcomeVerification | undefined {
  const present = verifications.filter((v): v is OutcomeVerification => v !== undefined);
  if (present.length === 0) return undefined;
  const chosen =
    present.find((v) => v.status === "disagreement") ??
...
export type ReportOutcome =
  | { readonly reported: true; readonly result: SendResult }
  | { readonly reported: false; readonly reason: PortalReportingDisabledReason | string };
 ...
 */
export function unavailableDecisionSummary(
  totalSteps: number,
  breakGlass: boolean,
): ExecutionReturnDecisionSummary {
  return {
 ...
 */
export function decisionSummaryFromExplicitRatification(
 ...
 */
export function deliveryReportSummaryFromOutcomes(
 ...
 */
export function decisionSummaryFromOutcomes(
  outcomes: readonly string[],
  breakGlass: boolean,
): ExecutionReturnDecisionSummary {
...
export async function reportRunToPortal(input: {
  ...
  try {
    ...
    if (!cfg.enabled) return { reported: false, reason: cfg.reason };
    ...
    if (!result.ok) return { reported: false, reason: result.reason };
