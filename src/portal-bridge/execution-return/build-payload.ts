...
import {
  ...
  type ExecutionReturnReplayRef,
  type ExecutionReturnChainVerification,
  type ExecutionReturnResumeTokenMeta,
  type ExecutionReturnOutcomeVerification,
} from "./types.js";

const OUTCOMES: readonly ExecutionOutcome[] = ["success", "failure", "partial"];
const APPROVAL_MODES: readonly ApprovalMode[] = ["per_step", "break_glass"];
const CHANGE_TYPES = ["create", "update", "delete", "rename"] as const;
 ...
 */
export function buildExecutionReturnPayload(
  input: ExecutionReturnInput,
): ExecutionReturnPayloadV1 {
  const chainCount = Math.max(0, Math.trunc(input.chainCount));

  const base: ExecutionReturnPayloadV1 = {
  ...
  const withReplay: ExecutionReturnPayloadV1 = input.replayRef
    ? {
        ...base,
        replay_ref: {
          reconstructable: input.replayRef.reconstructable === true,
          checksum:
            input.replayRef.reconstructable === true && isNonEmptyString(input.replayRef.checksum)
  ...
  const withChainVerification: ExecutionReturnPayloadV1 =
    cv && Array.isArray(cv.entry_ids) && cv.entry_ids.length > 0
      ? {
          ...withReplay,
          chain_verification: {
  ...
  const withDelivery: ExecutionReturnPayloadV1 = drs
    ? {
        ...withOutcome,
 ...
 */
export function validateExecutionReturnPayload(
  ...
  if (!isNonEmptyString(payload.ucp_root_id)) errors.push("ucp_root_id must be a non-empty string");
  ...
  if (!ds || typeof ds !== "object") {
    ...
    for (const k of ["total_steps", "approved", "rejected", "executed", "skipped"] as const) {
      if (!isNonNegativeInt(ds[k])) errors.push(`decision_summary.${k} must be a non-negative integer`);
    }
    if (typeof ds.break_glass !== "boolean") errors.push("decision_summary.break_glass must be a boolean");
    const availabilityBases = ["explicit_human_decision", "not_available"] as const;
  ...
  if (payload.delivery_report_summary !== undefined) {
    ...
    if (!drs || typeof drs !== "object") {
      ...
      for (const k of ["total", "accepted", "rejected", "skipped", "pending", "stopped", "other"] as const) {
        if (!isNonNegativeInt(drs[k])) {
          errors.push(`delivery_report_summary.${k} must be a non-negative integer`);
        }
      }
      if (
        isNonNegativeInt(drs.total) &&
        ...
        errors.push(
          "delivery_report_summary.accepted+rejected+skipped+pending+stopped+other must equal total",
        );
      }
    }
  }
  ...
  if (!ar || typeof ar !== "object") {
    ...
    if (
      ar.authority_status !== undefined &&
      ar.authority_status !== "portal_signed_verified" &&
      ar.authority_status !== "self_asserted"
    ) {
      errors.push(
        "approval_record.authority_status must be portal_signed_verified|self_asserted when present",
      );
    }
  }
  ...
  if (!cr || typeof cr !== "object") {
    ...
    if (typeof cr.available !== "boolean") errors.push("chain_ref.available must be a boolean");
    if (!isNonNegativeInt(cr.count)) errors.push("chain_ref.count must be a non-negative integer");
    if (cr.fetch !== "on_demand") errors.push('chain_ref.fetch must be "on_demand"');
  }
  ...
  if (payload.outcome_verification !== undefined) {
    ...
    const statuses = ["verified", "disagreement", "unknown", "unverified"] as const;
    const reports = ["accepted", "failed"] as const;
    const observations = ["agree", "disagree", "unknown", "not_observed"] as const;
    const verdicts = ["agree", "disagree", "unknown"] as const;
  ...
  if (payload.affected_resources !== undefined) {
    ...
    for (const r of payload.affected_resources) {
      if (!isNonEmptyString(r.path)) errors.push("affected_resources[].path must be a non-empty string");
    ...
    if (
      typeof payload.affected_resources_total === "number" &&
