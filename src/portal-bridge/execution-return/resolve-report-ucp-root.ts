...
import { computeChainVerification, type ChainVerification } from "../../ucp/chain-verification.js";
...
const INTENT_TYPE = "ucp.intent.v1" as const;
...
function primaryTaskInput(run: WorkflowRun): string {
  const fromTask = run.tasks.find(
    t => typeof t.spec.input === "string" && t.spec.input.trim().length > 0,
  )?.spec.input;
  if (fromTask) return fromTask;
  return run.spec.tasks[0]?.input ?? "";
}

function selectedReportRoot(run: WorkflowRun): {
  readonly selectedId: string | null;
  readonly input: string;
} {
  const fromTask = run.tasks.find(
    t => typeof t.intentId === "string" && t.intentId.length > 0,
  );
  if (fromTask && typeof fromTask.intentId === "string") {
    const taskInput =
      typeof fromTask.spec.input === "string" && fromTask.spec.input.trim().length > 0
        ? fromTask.spec.input
 ...
 */
export function resolveReportUcpRootId(run: WorkflowRun, storeDir: string): string {
  ...
  if (selected.selectedId !== null && envelope.id !== selected.selectedId) {
    throw new CertificationReportSnapshotError(
      "root_identity_mismatch",
      "selected report root does not match the deterministic intent envelope",
    );
  }
  ...
  if (readBack === null || readBack.id !== rootId || readBack.type !== INTENT_TYPE) {
    throw new CertificationReportSnapshotError(
      "root_readback_mismatch",
      "exact report-root read-back failed",
    );
  }
 ...
 */
export function captureCertificationChainSnapshot(
  storeDir: string,
  ucpRootId: string,
): CertificationChainSnapshot {
  const chain = getChain(storeDir, ucpRootId);
  const entryIds = Object.freeze(chain.map((env) => env.id).slice());
  if (entryIds.length === 0) {
    throw new CertificationReportSnapshotError(
      "empty_snapshot",
      "certification chain snapshot is empty",
    );
  }
  if (entryIds[0] !== ucpRootId) {
    throw new CertificationReportSnapshotError(
      "snapshot_root_mismatch",
      "certification chain snapshot does not begin with the selected root",
    );
  }
  ...
  for (const id of entryIds) {
    if (typeof id !== "string" || id.trim().length === 0) {
      throw new CertificationReportSnapshotError(
        "invalid_snapshot_id",
        "certification chain snapshot contains an empty id",
      );
    }
    if (seen.has(id)) {
      throw new CertificationReportSnapshotError(
        "duplicate_snapshot_id",
        "certification chain snapshot contains a duplicate id",
      );
    }
    seen.add(id);
  }
  ...
  if (chainVerification === null) {
    throw new CertificationReportSnapshotError(
      ...
      "could not compute certification chain verification",
    );
  }
  if (chainVerification.entry_ids.length !== entryIds.length) {
    throw new CertificationReportSnapshotError(
      "snapshot_count_mismatch",
      "chain verification length diverged from the copied snapshot",
    );
  }
 ...
 */
export function prepareCertificationReportSnapshot(
  run: WorkflowRun,
  storeDir: string,
): CertificationChainSnapshot {
