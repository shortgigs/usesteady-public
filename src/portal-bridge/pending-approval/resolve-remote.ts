...
import {
  validateMappedStoreDir,
  type MappedStorePolicy,
} from "../../shell/defaults.js";
import {
  runPendingApprovalBridge,
  type BridgeOptions,
} from "./bridge.js";
...
export type RemoteApprovalDecision =
  | { readonly decision: "yes"; readonly entry: ApprovalDecisionEntry; readonly authority: RemoteAuthorityEvidence }
  | { readonly decision: "no"; readonly entry: ApprovalDecisionEntry; readonly authority: RemoteAuthorityEvidence };
...
export type ResolveRemoteApprovalOptions = {
  ...
  readonly retirementReadback?: {
    ...
    readonly readEnvelopeById:
      (storeDir: string, envelopeId: string) => UCPEnvelope<unknown> | null;
  };
...
export function verifyRemoteEntryAuthority(input: {
  ...
  const parsedV2 = parsedV3.ok ? { ok: false } as const : parseDecisionOriginAuthorityAssertionV2(raw);
  ...
  const advisoryExpectation =
    input.gate.modelAdvisories !== undefined && input.gate.modelAdvisories.length > 0;
  if (
    (isV2 || isV3) &&
  ...
  const expected: ExpectedAuthorityBinding = {
  ...
  if (isV3) {
    if (input.surface === "web") {
      ...
      const verified = verifyRetirementAuthorityAssertionV3(
        raw,
        resolution.trust,
        expected,
        {
    ...
    const policy = (input.mappedStorePolicy ?? validateMappedStoreDir)(
      input.retirementReadback.storeDir,
    );
    if (!policy.ok) return { status: "verification_failed", reason: policy.code };
    const artifactId = input.gate.modelAdvisories?.[0]?.artifactId;
    if (!artifactId) return { status: "verification_failed", reason: "binding_mismatch:artifact_id" };
    const result = verifyRetirementAuthorityAssertionV3(
      raw,
      resolution.trust,
      expected,
      {
    ...
    if (!result.ok) return { status: "verification_failed", reason: result.reason };
 ...
 */
export async function resolveRemoteApprovalDecision(
  run: WorkflowRun,
  config: Extract<PortalApprovalsConfig, { enabled: true }>,
  opts: ResolveRemoteApprovalOptions = {},
): Promise<RemoteApprovalDecision | null> {
  const nowIso = opts.nowIso ?? (() => new Date().toISOString());
  const onStatus = opts.onStatus;

  const input = pendingApprovalInputFromRun(run, nowIso());
  if (!input) return null; // no projectable gate -> local approval flow
...
export async function resolveWebConfirmYes(input: {
  ...
  const { retirementReadback: _forbiddenReader, ...safeRemoteOpts } =
    input.remoteOpts ?? {};
