...
export const APPROVAL_DECISION_SCHEMA = "ucp.approval_decision.v1" as const;
...
export type DecisionParseResult =
  | { readonly ok: true; readonly value: ApprovalDecisionResponseV1 }
...
export type DecisionDirective =
  ...
  | { readonly kind: "fallback_local"; readonly reason: "expired" }
  | { readonly kind: "withdrawn" };

const STATUSES: readonly ApprovalDecisionStatus[] = [
  "pending",
  "approved",
  "rejected",
  "expired",
...
const VERBS: readonly ApprovalDecisionVerb[] = ["approve", "reject"];
 ...
 */
export function parseApprovalDecisionResponse(body: unknown): DecisionParseResult {
  const errors: string[] = [];
  ...
  const entries: ApprovalDecisionEntry[] = [];
  if (!Array.isArray(rawDecisions)) {
    ...
    for (const raw of rawDecisions) {
      ...
      if (!STATUSES.includes(raw["status"] as ApprovalDecisionStatus)) {
      ...
      if (
        ...
        !VERBS.includes(decisionRaw as ApprovalDecisionVerb)
      ) {
      ...
      entries.push({
        step_index: raw["step_index"] as number,
        status: raw["status"] as ApprovalDecisionStatus,
        decision:
          decisionRaw === null || decisionRaw === undefined
            ? null
            : (decisionRaw as ApprovalDecisionVerb),
        decided_at:
          raw["decided_at"] === undefined ? null : (raw["decided_at"] as string | null),
        decided_by:
          raw["decided_by"] === undefined ? null : (raw["decided_by"] as string | null),
        ...
        ...(raw["gate_cycle"] !== undefined && raw["gate_cycle"] !== null
          ? { gate_cycle: raw["gate_cycle"] as number }
          : {}),
        ...(raw["decision_relation"] !== undefined
          ? { decision_relation: raw["decision_relation"] as string | null }
          : {}),
        ...(Array.isArray(raw["resolving_evidence_ids"])
          ? { resolving_evidence_ids: raw["resolving_evidence_ids"] as string[] }
          : {}),
        ...(Array.isArray(raw["retirement_basis_relations"])
          ? {
              retirement_basis_relations:
                raw["retirement_basis_relations"] as RetirementBasisRelationV1[],
            }
          : {}),
      });
    }
  ...
  return {
    ...
    value: {
      ...
      run_id: (body["run_id"] as string).trim(),
      decisions: entries,
    },
  };
}
 ...
 */
export function selectDecisionForStep(
  response: ApprovalDecisionResponseV1,
  stepIndex: number,
): ApprovalDecisionEntry | null {
  let best: ApprovalDecisionEntry | null = null;
  for (const d of response.decisions) {
    if (d.step_index !== stepIndex) continue;
    if (best === null || (d.gate_cycle ?? 0) > (best.gate_cycle ?? 0)) best = d;
  }
  return best;
}
 ...
 */
export function selectDecisionForStepAndCycle(
  response: ApprovalDecisionResponseV1,
  stepIndex: number,
  gateCycle: number,
): ApprovalDecisionEntry | null {
  return (
    response.decisions.find(
      (d) => d.step_index === stepIndex && (d.gate_cycle ?? 0) === gateCycle,
    ) ?? null
  );
}
 ...
 */
export function interpretDecisionStatus(status: ApprovalDecisionStatus): DecisionDirective {
  switch (status) {
    case "pending":
      return { kind: "wait" };
    case "approved":
 ...
 */
export function interpretDecision(entry: ApprovalDecisionEntry | null): DecisionDirective {
  if (!entry) return { kind: "wait" };
