/**
 * Decision Record Bridge (DECISION_RECORD_BRIDGE_V1) - HTTPS transport.
 *
 * `sendDecisionRecord` POSTs a payload to the Portal's
 * `POST /api/v1/decision-records` endpoint with the entitlement-derived bearer
 * token. It is:
 *   - VALIDATE-FIRST, FAIL-CLOSED: an invalid payload is never sent.
 *   - BEST-EFFORT: it NEVER throws. Every network/abort/parse error is caught and
 *     returned as a structured result, so a transport failure can never affect the
 *     local decision (reporting is a side-channel).
 */

import { resolveOutboundFetch } from "../outbound-fetch.js";

import { validateDecisionRecordPayload } from "./build-payload.js";
import type { DecisionRecordPayloadV1 } from "./types.js";
...
export type SendOptions = {
  /** Portal base URL, e.g. https://app.usesteady.dev (no /api/v1/decision-records). */
  readonly url: string;
...
function joinUrl(base: string): string {
  return `${base.replace(/\/+$/, "")}/api/v1/decision-records`;
}

export async function sendDecisionRecord(
  payload: DecisionRecordPayloadV1,
  opts: SendOptions,
): Promise<SendResult> {
  // Fail-closed gate: never put an invalid payload on the wire.
  const validation = validateDecisionRecordPayload(payload);
  if (!validation.ok) {
