...
export type PostCoreUnderstandResult =
  | { readonly ok: true; readonly ucpRootId: string }
...
export async function postCoreUnderstandWorkflow(input: {
  ...
  if (!trimmed) return { ok: false, reason: "empty_intent" };

  const base = input.coreBaseUrl.trim().replace(/\/+$/, "");
  if (!base) return { ok: false, reason: "no_core_url" };
  ...
  try {
    ...
    const body = (await response.json()) as { ok?: unknown; ucp_root_id?: unknown };
